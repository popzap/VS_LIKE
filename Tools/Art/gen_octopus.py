...
BASE_W = [7.0, 6.4, 6.0, 5.6, 4.6, 3.2]  # 뿌리 굵기
...
def draw_lash(i):
    ...
    # 중심의 빛 — 촉수가 어디서 나오는지 알려 준다
    cr = (10 + i * 1.4)
    d.ellipse([cx - cr * FXS, cy - cr * FXS, cx + cr * FXS, cy + cr * FXS],
              fill=GLOW + (int(al * 0.75),))
...
for i in range(6):
    ...
    rr = np.hypot(xs + 0.5 - 128, ys + 0.5 - 128).max()
    worst = max(worst, rr)
    print(f'  lash f{i} 바깥 반지름 {rr:6.2f}px  알파최대 {cell.max()}')
