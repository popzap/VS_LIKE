...
| **B12** | 2026-09-05 | DEV(코드 읽다 발견) | **DEV** | **칸이 꽉 찬 상태에서 새 아이템을 상점에서 사면 골드만 사라진다.** 콘솔에는 **구매 성공으로 찍힌다** — `CanAcquire` 가 상점 경로 어디에도 없다. ⚠️ 등재 시점엔 **목격한 적이 없었다**(실플레이 3판에서 상점 구매가 0건이라 아무도 안 밟았다) | ✅ `수정됨(D71)` |
| **B13** | 2026-09-05 | DEV(코드로 확정) | **DEV** | **`base`/`meta` 의 `Luck` 이 최종 스탯에 안 들어간다.** `PlayerStats.RecalculateStats` 가 **필드별 나열**이라 `Luck` 한 줄이 빠졌다. ⚠️ **아직 안 터진다** — `Upgrades.csv` 의 `StatKey` 7종에도, `Economy.csv` 에도 `Luck` 이 없다. **둘 중 하나만 생기면 그 순간 터진다** | ✅ `수정됨(D83 — B안: `StatBlock` 쪽을 고쳐 다음 필드에서 또 안 밟게)` |
| **B14** | 2026-09-05 | DEV(사용자 플레이 로그) | **DEV** | **`EnemyVisual` 이 매 `LateUpdate` 마다 예외를 던진다** — 8초 동안 **47건**(`ArgumentNullException: dest`). 🔑 원인은 **플레이 중 재컴파일(도메인 리로드)** — `_sr`(Object 참조)은 복원되는데 `_mpb`(순수 C# 객체)는 null 이 되고 `Awake`/`Setup` 은 다시 안 돈다. 🟡 **빌드에서는 안 난다** | ✅ `수정됨(D84 — 지연 생성 프로퍼티로. 스스로 낫는다)` |
| **B15** | 2026-09-05 | DEV(사용자 플레이 로그) | **DEV** | **`ExpDrop.Collect` 이 `NullReferenceException` 을 던진다**(2건). `ExperienceManager.Instance` 가 null 인 순간에 구슬이 회수됐다. 🔴 **결과가 조용하지 않다** — 예외가 `Collect()` 중간에서 터져 `_collected = true` 는 이미 선 채로 **반납이 건너뛰어져 구슬이 영원히 남는다** | ✅ `수정됨(D84 — 예외가 아니라 순서가 핵심이었다)` |
...
| **B20** | 2026-09-07 | DEV(D106 잉크 측정 중) | **DEV** | **보스 오프스크린 화살표가 HUD 글자를 덮는다.** 화살표는 화면 테두리를 타는데 HUD 글자(`HPText`·`LevelText`·`XPText`·`TimerText`·`KillText`·`CurrencyText`)가 **전부 화면 위쪽**에 몰려 있다 ⇒ 보스가 위쪽 절반에 있으면 겹친다. **12방향 중 5방향**(30°~150°)에서 겹침, 최대 **566px²** | ✅ `수정됨(D106)` — 위쪽만 안으로 미는 `topHudBand` 신설(**210**, 글리프로 실측). 여유는 `arrowSize` 에서 계산한다(회전 사각형의 AABB 는 대각선을 못 넘는다). **글리프 겹침 0/12**. 🟡 곁일로 **보스 체력 바 겹침**을 새로 봤다 → `B21` |

| **B21** | 2026-09-07 | DEV(B20 판정 화면 확인 중) | **DEV** | **보스 화살표가 보스 체력 바에 묻힌다 — 둘 다 빨강이다.** `B20` 을 고쳐 글자는 다 피했는데, 위쪽 방향에서 화살표(`1,0.28,0.22`)가 **보스 체력 바 위에 겹쳐** 윗부분이 바에 섞인다. 🔑 `B20` 은 **글자만** 봤고 `Image` 는 안 봤다 | ✅ `수정됨(D106)` — **색을 안 바꾸고 어두운 외곽선**을 깔았다(색 판단은 CONTENT 몫 `요청-61`, 붉은색이 지고 있는 "위험"이라는 뜻도 남는다). 같은 메시 안에서 **외곽선 삼각형을 먼저** 넣으면 뒤에 깔린다 — 새 오브젝트 0개. 판정 **바 위 평균 색차 39 → 76**(대조군 = 외곽선만 끈 같은 화면) · 덤으로 잉크 425 → 576px(등가 한 변 20.6 → **24.0**) · `B20` 회귀 **0/12** |
...
## B21 — ✅ **해결됨 (D106)** · 화살표가 보스 체력 바에 묻힌다 (둘 다 빨강)

**발견 경위:** `B20` 을 고치고 **화면으로 판정하다가** 나왔다. 숫자는 0/12 통과였다.

`B20` 판정은 **글자만** 봤다(`TextMeshProUGUI`). 그래서 글자는 다 피했는데
**보스 체력 바(`Image`)** 는 검사 대상이 아니었고, 화살표가 그 위에 겹친다.
둘 다 빨강(`arrowColor 1,0.28,0.22` / 바도 붉은 계열)이라 **화살표 윗부분이 바에 섞인다.**

⚠️ 대비가 이미 **1.66 : 1** 로 낮은 상태다(→ [`REQ/CONTENT.md`](REQ/CONTENT.md) `요청-61`).
거기에 같은 색 위에 겹치기까지 하면 **"보스가 어디 있는지 알린다"는 존재 이유가 깎인다.**

### 왜 `B20` 과 같이 안 고쳤나

띠를 더 내리면 되긴 한다(바 아래끝 **252** ⇒ 화살표가 화면 위에서 139px). 안 한 이유:

1. **`B20` 의 범위 밖이다** — 그건 *"글자를 덮는다"* 였고 그건 닫혔다
2. 🔴 **전체 폭 띠 모델이 여기서 깨진다.** 같이 잰 `Frame`(왼쪽 아이템 칸)은 **356** 까지 내려온다.
   그것까지 피하면 화살표가 **화면 1/3 지점**에 앉아 "테두리 표시"가 아니게 된다.
   ⇒ 진짜 해법은 띠를 더 내리는 게 아니라 **영역별로 피하는 것**이거나, 아니면
   **색을 갈라서**(`요청-61`) 겹쳐도 읽히게 하는 것이다. 후자가 값이 더 크다

### ✅ 수정 (D106) — **피하지 말고 갈라 보이게 했다**

띠를 더 내리는 길(피하기)을 버리고 **어두운 외곽선**을 깔았다.
**색은 안 바꿨다** — 그건 CONTENT 판단이고(`요청-61`) 붉은색이 지고 있는
*"위험"* 이라는 뜻도 그대로 남는다. 외곽선은 팔레트가 아니라 **윤곽**이라
어느 배경 위에서도 형태가 남는다.

🔑 **오브젝트를 안 늘렸다.** 같은 메시 안에서는 **나중 삼각형이 위에 그려지므로**,
외곽선용 삼각형을 먼저 넣고 본체를 나중에 넣는 것만으로 "뒤에 깔린 테두리"가 된다.
정점 8 · 삼각형 4 · 새 `GameObject` **0**.

🔴 **`B20` 이 다시 열리지 않게 막았다.** 외곽선은 `rect` **밖으로 나간다** —
그걸 빼먹으면 화살표가 다시 글자를 덮는다. `OutlineScale()` 을 공개해
`B20` 의 여유 계산에 곱했다:
...
### 판정 — 대조군은 **외곽선만 끈 같은 화면**

| | 외곽선 없음(대조군) | 외곽선 6 |
|---|---|---|
| 바 위 겹친 픽셀 | 32px | 62px |
| **바 위 평균 색차** | **39** /765 | **76** /765 (**1.9배**) |
| 잉크 총량 | 428px | **576px** (등가 한 변 20.6 → **24.0**) |

`B20` 회귀 — 글리프 겹침 **0/12** (유지).

🟢 곁효과: 잉크가 늘어 **통과선 20px 여유가 커졌다**. 다만 대비 1.66:1 자체는
그대로라 `요청-61`(색 판단)은 여전히 열려 있다.

---

## B20 — ✅ **해결됨 (D106)** · 보스 화살표가 HUD 글자를 덮는다 (위쪽 절반 전체)

**발견 경위:** `D106` 에서 *"화살표를 잉크 기준으로 재 달라"* 를 하다가 나왔다.
캡처를 확대해 보니 화살표가 `0 G` 와 `0 Kills` 위에 얹혀 있었다. **지적 밖에서 나왔다.**

### 원인 — 화살표는 테두리를 타는데 글자가 전부 위에 있다

`BossOffscreenArrowUI` 는 보스 방향으로 **화면 테두리**에 화살표를 붙인다.
그런데 HUD 글자 여섯(`HPText`·`LevelText`·`XPText`·`TimerText`·`KillText`·`CurrencyText`)이
**전부 화면 위쪽 띠**에 있다 ⇒ 보스가 위쪽에 있으면 **반드시** 그 위를 탄다.

12방향 스윕 (보스 22유닛 · 게임뷰 856×498):

| 방향 | 겹치는 글자 | 넓이 |
|---|---|---|
| 30° | `KillText` `CurrencyText` | 553px² |
| 60° | `XPText` | 541px² |
| 90° | `TimerText` | 566px² |
| 120° | `LevelText` | 541px² |
| 150° | `HPText` | 403px² |
...
⇒ **12방향 중 5개(위쪽 절반). 아래쪽 절반은 깨끗하다.**

### 고치는 법 (제안 · 아직 안 고쳤다)

테두리에서 **안쪽으로 밀어 넣는 여백**을 위쪽만 크게 준다 (HUD 띠 높이만큼).
화살표를 옮기는 쪽이 안전하다 — 글자를 옮기면 `D85` 가 잡아 놓은 배치가 다시 흔들린다.

> 🔴 **재는 데 한 번 실패했다.** `GetWorldCorners` 에서 모서리 **2개**(`c[0]`·`c[2]`)만 써서
> 화면 AABB 를 만들었는데, 화살표는 **회전한 사각형**이라 그러면 폭이 절반으로 나온다
> (19×51 로 잡혔고 실제는 54×50 이었다). **잉크 bbox 실측(39×32)과 안 맞아서 잡혔다** —
> 픽셀 측정이 대조군 노릇을 했다. **회전하는 것은 모서리 4개를 다 봐야 한다.**

### ✅ 수정 (D106) — 위쪽만 안으로 민다

`topHudBand` 를 신설해 **위로 갈 때만** 한계를 다르게 준다. *"언제 뜨는가"(화면 안 판정)는
안 건드렸다* — 그건 이 버그 밖이고 같이 바꾸면 화살표가 뜨는 조건까지 달라진다.

```csharp
float arrowHalfMax = 0.5f * Mathf.Sqrt(w * w + h * h);   // 회전해도 대각선을 못 넘는다
...
이미 44 → 96 으로 한 번 바뀌었다.

**판정: 글리프 겹침 0/12** (수정 전 5 · 중간값 100 일 때 2)
...
| 2차 | **212** | 글자의 `RectTransform` 으로 쟀다 — **rect 는 가로로 넓은데 글자는 왼쪽에만 있다.** 12방향 중 하나가 헛경보였다 |
| 3차 | **210** ✅ | `ForceMeshUpdate` 뒤 `characterInfo[i].isVisible` 로 **실제 글리프** 외곽을 냈다 (실측 206 + 여유) |

> 🔴 **2차를 고쳤는데도 화면이 안 바뀌어 한참 헤맸다.** 코드 기본값을 212 로 올렸는데
...
🟡 곁일로 **보스 체력 바 겹침**을 새로 봤다 → `B21` (고치지 않았다).

---

---

## B19 — ✅ **해결됨 (D102)** · 도감이 승급 조건을 다 안 적는다 — 선행 직업이 빠졌다 (사용자 실플레이 · 2026-09-06)
...
## B18 — ✅ **해결됨 (D101)** · 제단 반경(2.2)이 건물 설치 링(최대 2.51)보다 좁다 (사용자 실플레이 · 2026-09-06)
...
## B17 — ✅ **해결됨 (D95)** · 상점에 오래 있다 레벨업하면 게임이 상태 기계째 꼬인다 (사용자 실플레이 · 2026-09-06)
...
## B16 — ✅ **해결됨 (D91)** · HUD 아이템 아이콘이 테두리 밖으로 나온다 (칸은 50인데 아이콘이 64)
...
## B15 — ✅ **해결됨 (D84)** · `ExpDrop.Collect` 이 `NullReferenceException` 을 던진다 (사용자 플레이 중 실측)
...
## B14 — ✅ **해결됨 (D84)** · `EnemyVisual` 이 매 `LateUpdate` 마다 예외를 던진다 (사용자 플레이 중 47건)
...
## B13 — ✅ **해결됨 (D83 · B안)** · `base`/`meta` 의 `Luck` 이 최종 스탯에 안 들어간다 (코드로 확정 · 터지기 전에 막았다)
