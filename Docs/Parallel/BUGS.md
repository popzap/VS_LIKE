...
| **B8** | 2026-09-02 | DEV(D27 조사 중) | **DEV** | **적이 뭉치면 무리 분리(separation)가 스스로 약해진다.** `EnemyBase.GetSeparation()` 의 이웃 버퍼가 **12칸 고정**인데 이 오버로드는 넘치는 이웃을 **경고 없이 버린다.** 뭉칠수록 더 뭉치는 자기강화 루프 | `열림(코드 근거만 — 런타임 미측정)` |
...
🔴 **런타임으로는 아직 한 번도 안 재 봤다.** 위는 전부 코드와 Unity API 규약에서 나온 추론이다.
`D27` 의 부하 하네스로 적 200/400/800 을 세우고 `n` 의 분포를 찍는 것이 첫 판정이다.
