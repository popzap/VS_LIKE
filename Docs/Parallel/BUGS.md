...
| **B16** | 2026-09-06 | DEV(D88 화면 검증 중) | **DEV** | **HUD 아이템 칸의 아이콘이 테두리 밖으로 삐져나온다.** `Prefab_ItemChip` 의 `Icon` 이 `m_SizeDelta 64x64` **고정**이라 칸 크기를 안 따라간다. `D87` 이 HUD 칸을 36→**50** 으로 바꿔 **50 칸에 64 아이콘**이 됐다 (사방 7 캔버스px = 화면 **3.2px**). 빈 칸은 `icon a=0.00` 이라 안 보일 뿐 **크기는 같다** | `열림` |
