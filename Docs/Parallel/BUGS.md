...
|---|---|---|---|---|---|
| **B1** | 2026-08-30 | 사용자(실플레이) | ⚠️ **결정 필요** — 로직 버그인데 파일은 CONTENT 소유. §B1 | 엘리트 외곽선이 실루엣과 어긋난다. **원인 확정** — 외곽선이 시트의 *옆 프레임* 알파를 빨아들인다 | `확인중` |
| **B2** | 2026-08-30 | 사용자(실플레이) | **CONTENT** | **원인 확정** — Goblin·Slime 걷기 시트가 깨졌다. 16칸 중 12칸이 프레임이 아니라 **작은 캐릭터 9~16마리 뭉치** | `수정됨(C1) — 임포트·검증 대기` |
...
| **B4** | 2026-08-30 | CONTENT | **DEV** | 일시정지·옵션창 글자 6곳이 **빈칸으로 나온다.** 씬에 한글이 남아 있는데 폰트가 Static 115자라 한글이 없다 | `열림` |
