...
| **B20** | 2026-09-07 | DEV(D106 잉크 측정 중) | **DEV** | **보스 오프스크린 화살표가 HUD 글자를 덮는다.** 화살표는 화면 테두리를 타는데 HUD 글자(`HPText`·`LevelText`·`XPText`·`TimerText`·`KillText`·`CurrencyText`)가 **전부 화면 위쪽**에 몰려 있다 ⇒ 보스가 위쪽 절반에 있으면 겹친다. **12방향 중 5방향**(30°~150°)에서 겹침, 최대 **566px²** | `열림` |
...
## B20 — 🔴 **보스 화살표가 HUD 글자를 덮는다** (위쪽 절반 전체)
