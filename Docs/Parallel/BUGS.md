...
| **B21** | 2026-09-07 | DEV(B20 판정 화면 확인 중) | **DEV** | **보스 화살표가 보스 체력 바에 묻힌다 — 둘 다 빨강이다.** `B20` 을 고쳐 글자는 다 피했는데, 위쪽 방향에서 화살표(`1,0.28,0.22`)가 **보스 체력 바 위에 겹쳐** 윗부분이 바에 섞인다. 🔑 `B20` 은 **글자만** 봤고 `Image` 는 안 봤다. 대비가 이미 1.66:1 로 낮은데(→ `요청-61`) 같은 색 위에 겹치면 **화살표의 존재 이유가 깎인다** | `열림` |
