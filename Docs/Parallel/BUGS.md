...
| **B19** | 2026-09-06 | 사용자 실플레이 | **DEV** | **도감 `CLASSES` 탭의 승급 조건에 선행 직업이 빠졌다.** `Aegis` 가 *"Turret Lv.5 + Armor Lv.3"* 만 보여 줘서, 재료를 갖췄는데 승급이 안 되는 것처럼 보인다 — 실제로는 **`Sentinel` 이어야** 한다. 🔑 `EVOLUTION` 탭 본문에는 *"Requires class:"* 줄이 있는데 **`CLASSES` 탭에만 없었다** — 같은 사실을 두 곳에서 따로 만들다 한쪽이 빠졌다. **틀린 게 아니라 모자랐다** | `수정됨(D102)` |
| **B18** | 2026-09-06 | 사용자 실플레이 | **DEV** | **`Aegis` 승급이 안 된다(도감 9/10).** 🔑 레시피는 멀쩡하다 — 끝까지 승급시켜 확인했다. 🔴 **숫자 둘이 안 맞았다**: 건물은 `placeDistance 1.8` 링 위에 놓여 실측 거리가 **1.18~2.51** 로 흩어지는데 제단 반경(`altarRadius`)이 **2.2** 였다. ⇒ 갓 세운 건물이 **자리에 따라** E 에 닿기도 안 닿기도 한다 — 같은 조작이 될 때도 안 될 때도 있었다 | `수정됨(D101)` |
...
## B19 — 🔴 **도감이 승급 조건을 다 안 적는다 — 선행 직업이 빠졌다** (사용자 실플레이 · 2026-09-06)

**증상:** 사용자 — *"터렛 5 아머 5 레벨인데 안되는데 잘못 적혀있는거야?"*

도감 `CLASSES` 탭의 `Aegis` 조건 줄:

```
Turret Lv.5  +  Armor Lv.3
```

**재료는 맞다. 그런데 `Aegis` 는 `Sentinel` 이어야만 승급된다** — 그 줄이 없다.
⇒ 사용자는 재료만 갖추면 되는 줄 알고 **몇 판을 헛돌았다.**

### 🔑 같은 사실을 두 곳에서 따로 만들고 있었다

| 탭 | 선행 직업 표시 |
|---|---|
| `EVOLUTION` (`BuildEvolutions`) | ✅ 본문에 `"Requires class: ..."` (`CodexPanel.cs:439`) |
| `CLASSES` (`BuildClasses`) | 🔴 **없다** — `Recipe(ce.Ingredients, ...)` 만 쓴다 |

`Recipe()` 는 **재료 배열만** 받는 함수라 `FromClass` 를 알 방법이 없다.
`EVOLUTION` 쪽은 그 밖에서 따로 한 줄을 덧붙였고, `CLASSES` 쪽은 안 붙였다.

**틀린 게 아니라 모자랐다.** 그래서 더 나쁘다 — 읽는 사람은 그게 전부라고 믿는다.

## ✅ 해결 (D102, 2026-09-06)

`BuildClasses` 의 조건 줄 앞에 선행 직업을 붙인다. 미발견 직업이면 `???` 로 가려
**도감의 감추기 규칙을 조건 줄에서도 지킨다.**

```
Aegis    조건: Sentinel  >  Turret Lv.5  +  Armor Lv.3
```

### 검증 (플레이 모드 · CLASSES 탭 전수)

```
Sentinel   조건: Gun Lv.5  +  Turret Lv.3
Doomlord   조건: Bomb Lv.5  +  Bombard Lv.3
Warden     조건: Sword Lv.5  +  Village Lv.3
Aegis      조건: Sentinel  >  Turret Lv.5  +  Armor Lv.3     <- 선행 직업이 붙었다
```

🔑 **선행 직업이 있는 것은 `Aegis` 하나뿐이라 나머지 셋은 그대로다** — 대조군이 같이 나왔다.

### ⚠️ `B18` 과 겹쳐 있었다

같은 증상(*"승급이 안 된다"*)에 **원인이 둘**이었다:

| | |
|---|---|
| `B18` | 제단 반경(2.2)이 건물 설치 링(최대 2.51)보다 좁아 **E 가 안 닿는다** |
| `B19` | 도감이 **선행 직업을 안 알려 준다** |

🔑 **하나를 고치고 끝냈으면 사용자는 여전히 못 했을 것이다.**
사용자가 *"터렛 5 아머 5 인데"* 라고 **자기 상태를 숫자로** 알려 준 덕에 두 번째가 드러났다.

---

## B18 — 🔴 **제단 반경(2.2)이 건물 설치 링(최대 2.51)보다 좁다** (사용자 실플레이 · 2026-09-06)

**증상:** 사용자 — *"이직업이 진화가 안되네"*. 도감 `CLASSES` 가 **9/10**, 안 열린 하나가 `Aegis`.

### 🔑 먼저 확인한 것 — 레시피는 멀쩡하다

| | |
|---|---|
| `Aegis` | `FromClass = Sentinel` · 재료 `Turret Lv5` + `Armor Lv3` · 제단 **Turret** |
| 배선 | `classEvolutions` 4개 전부 배선됨 (`Aegis` 포함) |
| 조건 | 조건을 갖추니 `IsClassSatisfied(Aegis) = True` · `GetReadyClassEvolutions = 1` |

**끝까지 승급시켜 봤다** — 사슬이 `Sentinel > Aegis` 가 된다. ⇒ **데이터도 코드도 문제가 없다.**

### 🔴 원인 — 두 숫자가 서로를 모른다

| | 값 | 어디 |
|---|---|---|
| 건물 설치 거리 | `placeDistance` **1.8** · 링 위 실측 **1.18 / 1.99 / 2.40 / 2.51** | `BuildingManager` (`Economy.csv`) |
| 제단이 닿는 거리 | `altarRadius` **2.2** | `EvolutionManager` (CSV 에 없었다) |

건물은 플레이어 둘레의 **링 위 빈 자리**에 놓이므로 **어디 놓일지는 그때그때 다르다.**
그중 **2.2 를 넘는 자리**에 놓이면 **그 자리에서 E 를 눌러도 아무 일이 없다.**

🔑 **이게 "가끔 된다" 를 만든다.** 사용자가 `Sentinel`·`Doomlord`·`Warden` 은 승급했는데
`Aegis` 만 못 한 것도 이것으로 설명된다 — **조작을 모르는 게 아니라 운이었다.**
0.2 차이라 "가까이 가면 된다" 는 것도 눈치채기 어렵다.

## ✅ 해결 (D101, 2026-09-06)

`altarRadius` 를 `Economy.csv` 로 끌어올리고 **2.2 → 3** 으로. 링 최대 2.51 을 덮는다.

```
EvolutionManager,altarRadius,3,제단(건물 앞 E)이 닿는 거리. placeDistance 링(최대 약 2.5)보다 커야 한다
```

🔑 **CSV 에 올린 게 절반이다.** 원래 이 값은 코드 기본값 + 씬에만 있어서
`placeDistance`(CSV)와 **나란히 놓고 볼 수가 없었다.** 두 값이 서로를 모르면 또 어긋난다.
CSV 주석에 **"링을 전부 덮어야 한다"** 는 관계를 같이 적었다.

### 검증

```
거리 2.40 -> FindAltarClassEvolution = null   (재현)
거리 0.80 -> Aegis · TryEvolveAtAltar = True · 사슬 Sentinel > Aegis
altarRadius = 3 · 실측 링 거리 1.18~2.51 전부 반경 안
```

🔴 **시험이 한 번 오염됐다** — 앞선 판정의 잔여 상태(직업·터렛 3개)가 남아
`TryEvolveAtAltar = False` 와 `사슬 = Sentinel > Aegis` 가 **동시에** 나왔다.
판을 새로 켜고 한 명령 안에서 다시 재고서야 원인이 갈렸다 (`D85` 와 같은 함정).
