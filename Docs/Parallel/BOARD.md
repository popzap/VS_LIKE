...
| CONTENT | `C33` | `Assets/Game/Sprites/{Projectiles,UI}` · `Tools/Art/` · `Docs/DESIGN_ART.md` · `REQ/DEV.md` | 2026-09-04 | `대기(REQ→DEV)` — **탄환 2종 + UI 9-slice 프레임 3종** (`DESIGN_ART.md` §6 의 1·2번). **코드·씬 변경 0.** 🔴 `Proj_Bullet` 과 `Proj_EnemyBolt` 가 **같은 스프라이트 파일**을 쓰고 있었다 — 색 신호가 이미 셋(엘리트 외곽선·피격 플래시·적탄 틴트)이라 넷째를 색으로 얹으면 안 읽힌다 ⇒ **방향성으로 갈랐다**. 월드 크기는 기존과 동일(`256÷512=0.5유닛`)이라 게임플레이 변화 0. ⚠️ **두 번 다시 그렸다** — ① 대칭 렌즈라 방향이 안 보임 → 혜성 꼬리 ② 매끈한 글로우가 `Arrow.png` 옆에서 겉돌아 → **픽셀아트**(실측 4px 블록·논리 64²). 🔴 **`spriteBorder` 40/28/20 이 안 들어가면 9-slice 가 통째로 안 되는데 증상이 "설정 누락"으로 안 보인다** → [`REQ/DEV.md`](REQ/DEV.md) **요청-25** · [`DONE/C33.md`](DONE/C33.md) |
| CONTENT | `C34` | `Assets/Game/Sprites/{Passives,Effects}` · `Assets/Game/Balance/Upgrades.csv` · `Tools/Art/` · `Docs/{DESIGN_ART,TUNING}.md` | 2026-09-04 | `대기(REQ→DEV)` — **메타 강화 아이콘 + 승급·레벨업 연출 그림** (`DESIGN_ART.md` §6 의 3·4번). 🔑 **두 번 다 "만들지 않는 쪽"이 답이었다** — 아이콘 7장 중 **6장은 패시브 재사용**(같은 스탯이라 다른 그림이면 오히려 헷갈린다. 재사용 가능 여부는 알파 실측 후 결정 — 안 쟀으면 `I-41` 6장을 붙일 뻔했다) · 승급/레벨업은 **흰색 2장을 틴트로** 가른다. 🔴 **승급에 연출이 0 이었다** — 보스 등장엔 흔들림, 엘리트 처치엔 히트스톱이 있는데 **가장 큰 성취인 승급엔 아무것도 없어 비중이 거꾸로**다. ⚠️ 링을 한 번 다시 그렸다(폭 0.35 → **0.12**) — **두꺼운 링은 도넛이고 도넛은 안 퍼진다.** **코드는 3줄뿐** → [`REQ/DEV.md`](REQ/DEV.md) **요청-26** · [`DONE/C34.md`](DONE/C34.md) |
| CONTENT | `C35` | `Docs/DESIGN_ART.md` · `REQ/DEV.md` | 2026-09-04 | `대기(REQ→DEV)` — **상점 주인 · 메인 메뉴 배경 요청**(`DESIGN_ART.md` §6 의 **6번** · **애셋 변경 0**). §6 에서 **내가 못 만드는 유일한 항목**이다(인물·배경은 절차적 생성 불가). 🟢 **요청 전에 꽂을 자리부터 봤고 둘 다 코드 0줄이다** — 바뀌지 않는 그림은 참조를 들 이유가 없다. 안 봤으면 DEV 가 안 해도 될 코드를 짰다. 🔑 **상점 주인의 성격은 이미 정해져 있었다** — `ShopUI.cs:47` 의 대사 5줄이 *"For a price."* 라 **너스레 떠는 장사꾼**이다. "상인을 그려 달라"로 넘기면 그림과 대사가 어긋나므로 **대사를 인용해서** 넘겼다. 🔴 배경 밝기는 취향이 아니라 **글자 가독성**이라 숫자로 못 박았다(중앙 세로 띠 평균 ≤ 0.25). ⚠️ `C28` 함정 4개(알파 255 · PPU · 빈 파일 · `mtime`)를 붙였고 **판정 3건은 내가 다시 잰다** → [`REQ/DEV.md`](REQ/DEV.md) **요청-27** · [`DONE/C35.md`](DONE/C35.md) |
