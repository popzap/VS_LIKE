...
| DEV | `D37` | `Assets/Scripts/Meta/EventManager.cs` · `Assets/Scripts/UI/EventUI.cs`(신규) · `Assets/Scripts/Wave/WaveManager.cs` · `Assets/Game/Balance/Events.csv` · `Assets/Editor/BalanceImporter.cs` · 씬 | 2026-09-03 | `진행중` — **이벤트 3종** `E7 Exchange` · `E11 Minefield` · `E9 Field Promotion` (사용자 선택). 🔴 **선행 2건이 먼저다** — ① **이벤트 UI**(지금 `Debug.Log` 뿐이라 선택형이 성립을 안 한다) ② **`StageType.Event` 웨이브 분기**(없어서 전투형이 존재할 수 없다) |
...
|---|---|
| 상태 | `BUSY` |
| 점유 이슈 | `D37` (이벤트 UI · Event 웨이브 분기 · E7/E9/E11) |
| 컴파일 에러 | `0` (2026-09-03 **D36 종료 시** 확인 — 플레이 정지 · 잔류 오브젝트 0 · 멈춰 둔 컴포넌트 복구 확인) |
