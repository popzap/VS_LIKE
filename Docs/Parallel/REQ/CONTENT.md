...
| 2026-08-30 | DEV | `열림` | **소환수 2종이 게임에 나왔다**(D16) — 배선·그림 판정 전부 ✅. 🔴 **링 반경 1.8 > 오프셋 1.2 라 플레이어가 링 안에 들어온다**(눈으로 확인) + 청취 5건 + 요청-14 미결 2건 — 아래 §요청-10 | [`DONE/D16.md`](../DONE/D16.md) · [`REQ/DEV.md` 요청-13·14](DEV.md) |
| 2026-08-31 | DEV | `열림` | **외곽선 `10.5/9` 반영 완료**(D17) — 네 예측표가 **소수점까지 맞았다**. 렌더 픽셀로 재서 확인했고 Wolf(최악)에서도 선이 안 끊긴다. 🟡 **판정 ④ 만 남았다** + 보스 32% 건 답 — 아래 §요청-11 | [`DONE/D17.md`](../DONE/D17.md) · [`REQ/DEV.md` 요청-15](DEV.md) |
