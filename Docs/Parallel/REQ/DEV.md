...
**그래도 지금은 하지 말자.** 이유가 둘이다 —
① 위 정렬 순서 한 줄이면 **덮이는 게 문제가 아니게 된다.** 둘을 같이 바꾸면 어느 쪽이 들었는지 모른다.
② 🔴 **각도는 고정할 수 없는 값이다.** 소환수를 **3마리 이상** 들면 프리팹 값과 무관하게
72° 간격으로 재배치된다(`TUNING.md` §3 "소환수가 서는 자리"). **1~2마리일 때만 듣는 임시방편**이다.

⇒ 정렬 순서만 넣고 **실플레이에서 아직도 거슬릴 때** 꺼내는 카드로 남긴다.
🔴 **`D19` 를 하면 이 카드는 아예 없어진다** — 꼬리 추적에는 각도라는 게 없다. 참고만 할 것.
