...
>
> **최종 갱신:** 2026-08-30 (26차 — DEV 치트 패널 신설, D3)
>
...
- 🟡 **`OnGUI` 가 화면에 그려지는 모습은 눈으로 확인 안 됐다.** 백틱 입력을 MCP 로 주입해
  `Time.timeScale == 0` 을 외부 증거로 삼으려 했으나 **입력이 게임에 도달하지 않는다** —
  게임 뷰에 포커스가 없어서다. **도구의 한계이지 코드 문제가 아니다.**
  → 사용자가 플레이해서 백틱을 눌러 보는 것으로 판정한다.
