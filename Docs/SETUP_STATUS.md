...
>
> **최종 갱신:** 2026-09-05 (99차 — 세션 요약 + PLAYTEST_2, D63~D76)
>
...
## 2-150. ✅ **도전 석상 — 처음으로 "내가 고른 위험"** (D123, 2026-09-09 144차)

> 사용자: *"균열 + 도전 석상으로 가자"* (`D122` 와 한 쌍)

### 🔑 균열과 정반대여서 짝이 된다

| | 무엇을 말하나 | 안 하면 |
|---|---|---|
| **균열** (D122) | *"안 가면 나빠진다"* — 강제 | 소환 간격이 계속 짧아진다 |
| **도전 석상** (D123) | *"가면 걸 수 있다"* — 선택 | 아무 일도 안 일어난다 |

둘 다 **한 지점에 발을 묶는데 이유가 반대**다. 그래서 같이 놓였을 때
*"저 균열부터 닫을까, 지금 석상을 눌러도 될까"* 라는 판단이 생긴다.

🔑 **이 게임에 없던 것은 "내가 고른 위험"이다.** 지금까지 난이도는 층(`LayerScaling`)과
시간이 정했고 플레이어는 받기만 했다. `D121`(현상금)이 **패시브로** 연 문을
석상은 **자리로** 연다 — 준비됐다고 느낄 때, 원하는 순간에만 누른다.

### 🔵 엘리트를 새로 만들지 않았다

`WaveManager.SpawnMinion` 에 `isElite` 를 넘기는 것이 전부다. 그러면
체력 ×2.5 · 피해 ×1.5 · 경험치 ×3 · **보라 외곽선** · **사망 시 보물상자**가 통째로 따라온다.

무엇을 깨우는가는 `WaveManager.ChallengeEnemy()` 가 정한다 —
`EliteOverride` 가 있으면 그것, 없으면 **이 웨이브의 잡몹 하나**.
노말 웨이브는 `EliteOverride` 가 비어 있어서(`Waves.csv` 실측) 후자가 기본이고,
결과적으로 **그 층에 사는 놈의 엘리트판**이 나온다 — 층마다 도전 상대가 저절로 달라진다.

### 🔴 안내와 실제가 같은 함수를 본다

`PlayerController` 의 `E` 와 화면 안내가 **둘 다 `ChallengeStatue.NearestUsable`** 을 부른다.
갈라 두면 *"[E] 가 떴는데 눌러도 아무 일이 없다"* 가 반드시 생긴다.

🔵 안내는 **진화 안내와 같은 한 줄**을 쓴다. 새 줄을 만들면 두 시스템이 같은 픽셀을 놓고 다툰다 —
`D81` 이 실제로 그렇게 4px 겹쳤다. 대신 **석상을 맨 위 우선순위**로 두어
`PlayerController` 의 순서와 맞췄다.

🔴 반경은 **2.8** 로 넉넉하게 잡았다. `B18` 에서 제단 반경(2.2)이 건물 설치 링(최대 2.51)보다
좁아 **E 가 안 닿은** 적이 있다.

### 🔴 석상은 적이 아니라서 아무도 안 걷어 준다

`WaveManager.ClearWave` 는 `EnemyBase` 만 걷는다. 석상은 그냥 오브젝트라
**`ChallengeDirector` 가 웨이브 종료를 보고 직접 지운다** — 안 하면 다음 층에 지난 층 석상이 서 있다.
(균열은 적이라서 이 문제가 없다. 같은 시스템의 두 조각인데 **정리 주체가 다르다.**)

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/World/ChallengeStatue.cs` | 신규 · `Version=1` — 누르면 엘리트를 깨우고 한 번 쓰면 꺼진다 |
| `Assets/Scripts/World/ChallengeDirector.cs` | 신규 · `Version=1` — 언제·어디에 세우고 **언제 걷는지** |
| `Assets/Scripts/Wave/WaveManager.cs` | `ChallengeEnemy()` · `SpawnMinion(..., isElite)` · `Version 4→5` |
| `Assets/Scripts/Player/PlayerController.cs` | `E` 가 석상을 먼저 본다 (🔴 `return` 대신 `handled` — 뒤에 코드가 붙는 날 통째로 건너뛴다) |
| `Assets/Scripts/UI/EvolutionPromptUI.cs` | 안내 우선순위 맨 위에 `[E] CHALLENGE` |
| `Assets/Prefabs/ChallengeStatue.prefab` | 신규 (스프라이트 + 컴포넌트 · 스케일 3.5 → 화면에서 약 0.67×1.49) |
| `Assets/Game/Sprites/World/ChallengeStatue.png` | 신규 (**새 폴더** — 석상은 건물도 적도 아니다) |
| `Assets/Game/Balance/SceneWiring.csv` | `ChallengeDirector,statuePrefab` |

### 검증 로그 (플레이 모드 실측)

**① 디렉터가 스스로 세운다** — 개입 없이 **12초** 만에, 플레이어로부터 **10.6** 유닛 ✅
**② 반경** — 거리 **2.0 잡힘** vs **3.5 안 잡힘**(대조군 · 설계 2.8) ✅
**③ 도전** — `TryChallenge=True` · 깨어난 것 **Wolf** · 석상에서 **0.76** 유닛
...
**④ 한 번만** — 두 번째 `TryChallenge=False`(첫 번째는 `True`) ✅
**⑤ 보상** — 죽이기 **전 +0**(대조군) → 죽인 뒤 **+42** ✅
(= 현상금 40 + 늑대 자신의 `CurrencyDrop` 2) · 석상 자리에 **상자 1개**
**⑥ 정리** — 웨이브 종료 전 1개 → **종료 뒤 0개** ✅
**화면** — 안내가 실제로 그려졌다: `"[E] CHALLENGE — awaken an elite"` ·
`materialCount=1`(D104 기준) · 게임 카메라 직접 렌더로 석상 확인(플레이어보다 키가 크다)

🟡 **어두운 바닥에서 석상이 눈에 확 띄지는 않는다** — 룬이 화면에서 2px 남짓이다.
쓸 수 있을 때 숨 쉬는(밝아지는) 연출이 유일한 신호라 실플레이에서 확인이 필요하다 → `TODO.md`.

## 2-149. ✅ **균열 — 이 게임에 처음으로 "저기"가 생겼다** (D122, 2026-09-09 143차)

> 사용자: *"균열 + 도전 석상으로 가자"* (구역 유도 시스템 · 내가 낸 안 중에서 고름)

### 왜 오벨리스크·석상만으로는 안 되는가

설계 전에 `WaveManager` 를 먼저 읽었다. 적이 태어나는 자리는 이렇게 정해진다:
...
🔑 **적은 100 % 나를 중심으로 태어난다.** 그래서 이 아레나에는 **지리가 없다** —
어디에 서 있든 세상은 똑같이 생겼고, 위치는 오직 *"적과 나의 거리"* 로만 의미를 가졌다.
바닥에 점을 하나 찍는 것(오벨리스크·상호작용 석상)으로는 이게 안 바뀐다.
**적이 태어나는 자리를 옮겨야** 비로소 "저기"가 생긴다.

### 🎁 건물 9종에게 처음으로 지킬 대상이 생긴다

터렛·포대·바리케이드·지뢰는 전부 *"한 지점을 붙든다"* 인데,
이 게임에는 **붙들 지점이 없었다** — `D110`(미끼) · `D117`(벽) · `D120`(화톳불)이
세 번에 걸쳐 우회한 게 전부 이 빈칸이다. 균열이 그 빈칸을 채운다.

### 🔵 적으로 태어나게 한 것이 이 작업의 전부다

무기 13종은 전부 이렇게 때린다 — `OverlapCircle(Enemy)` → `GetComponent<EnemyBase>()` → `TakeDamage`.
균열을 새 타입으로 만들었다면 **어떤 무기도 균열을 못 봤을 것**이고 13곳을 고쳐야 했다.
`RiftEnemy : EnemyBase` 로 두니 피격·체력·사망·드랍·개체 상한·웨이브 종료 정리가 **전부 공짜**로 따라왔다.

대신 "적"이라서 따라오는 것 두 개를 꺼야 했다:

| 무엇 | 왜 | 어떻게 |
|---|---|---|
| **재배치** | `RecycleFarEnemies` 가 멀어진 적을 플레이어 곁으로 옮긴다 — 균열이 따라오면 **"저기"가 다시 "내 옆"이 된다** | `EnemyBase.IsAnchored` (보스 예외 옆에 나란히) |
| **넉백** | `ApplyKnockback` 이 `Rb.linearVelocity` 를 직접 민다 — 맞을 때마다 균열이 **뒤로 흘러간다** | `FixedUpdate` 를 덮어 매 물리 프레임 앵커로 되돌린다 |

### 🔴 방치하면 나빠진다

소환 간격이 열려 있는 시간에 비례해 줄어든다(`3.5 → 1.0`초, 초당 `-0.06`).
**무시할 수 있는 위협은 위협이 아니다** — 이 감쇠가 없으면 균열은 그냥 장식이다.

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/Enemy/RiftEnemy.cs` | 신규 · `Version=3` — 자리에 못 박힌 적 + 잡몹을 뱉는다 |
| `Assets/Scripts/Enemy/RiftDirector.cs` | 신규 · `Version=1` — **언제·어디에** 여는지 (보스전 제외 · 플레이어에서 9~14) |
| `Assets/Scripts/Enemy/EnemyBase.cs` | `IsAnchored` 가상 프로퍼티 · `Version 6→7` |
| `Assets/Scripts/Wave/WaveManager.cs` | `RandomCurrentEnemy()` · `SpawnMinion` 이 개체를 **반환**하고 `ignoreCap` 을 받는다 · 앵커는 재배치 제외 · `Version 3→4` |
| `Assets/Prefabs/Enemy_Rift.prefab` | 신규 (`Enemy_Goblin` 복제 → `EnemyBase` 를 `RiftEnemy` 로 교체 · `sortingOrder -1`) |
| `Assets/Game/Sprites/Enemies/Rift.png` | 신규 |
| `Assets/Game/Balance/{Enemies,SceneWiring}.csv` | `Rift` 행 + `RiftDirector,riftData` 배선 |

### 검증 로그 (플레이 모드 실측)

🔑 **대조군은 "균열과 같은 자리에 같은 순간 태어난 잡몹"** 이다 — 자리도 시각도 같으니
이후의 모든 차이는 **무엇이냐**에서만 온다.

| | 균열 | 대조군 잡몹 |
|---|---|---|
| ② 8초 뒤 이동 거리 | **0.00** | **12.31** ✅ |
| ④ 5회 피격 뒤 밀린 거리 | **0.00** | **0.28** ✅ |

**③ 뱉는가** — 뱉는 그 순간 균열 반경 2 안의 적 **2** vs **반대편 같은 거리 0**(대조군) ✅
**③ 방치 감쇠** — 8초 뒤 간격 `3.5 → 3.02` (설계 `8 × 0.06 = 0.48` 감소와 일치) ✅
**⑤ 닫으면** — `activeRifts 1 → 0` · 그 자리에 **상자 1개** ✅
**디렉터** — 개입 없이 웨이브 중 스스로 하나를 열었다(내가 띄운 것 1 + 디렉터 1 = 2) ·
`secondsToNext` 가 돌고 있음 ✅ · 콘솔 예외 0

🔴 **측정이 두 번 틀렸고 둘 다 진짜 결함을 잡아냈다.**

1. **"뱉은 수 2인데 근처 적 0"** — 8초 뒤에 세었기 때문이다. 잡몹은 태어나자마자
   플레이어에게 걸어가 버려서(그리고 멀어지면 재배치까지 된다) **8초면 아무도 안 남는다.**
   0 vs 0 은 아무것도 못 가른다 ⇒ **뱉는 그 프레임**에 세도록 고쳤다.
2. 🔴 **균열이 플레이어보다 작게 그려졌다**(폭 0.46 vs 1.00) — `OnInitialized` 를 재정의하면서
   **`base.OnInitialized()` 를 안 불렀다.** 그 함수가 그림·색·**크기**(`SizeScale 3.2`)·
   `EnemyVisual.Setup` 을 전부 넣는 자리다. 화면을 안 찍었으면 못 봤다.
   ⇒ 고친 뒤 `scale 3.20` · 화면 폭 **플레이어의 1.5배** 확인.

🔵 `spawned` 를 셀 때 `SpawnMinion` 의 **반환값**을 본다 — 개체 상한에 걸리면 조용히 `null` 이라
그냥 세면 "뱉었다"는 숫자가 실제로 태어난 수와 어긋난다(그래서 반환형을 `void → EnemyBase` 로 바꿨다).

## 2-148. ✅ **현상금 — 처음으로 값을 치르고 사는 패시브** (D121, 2026-09-09 142차)

> 사용자: *"적이 강해지지만 (체력 50%증가) 정산 골드 증가 패시브 하나 추가"*

### 패시브 15종이 전부 이득만 준다

`Damage`·`Armor`·`Luck`·`Shield`… 무엇을 골라도 **손해가 없다.**
그래서 레벨업 3택에서 고민하는 것은 *"무엇을 살까"* 가 아니라
*"지금 뭐가 제일 모자라나"* 였다 — 선택이라기보다 **정리**다.

**현상금은 이 게임에서 처음으로 값을 치르고 산다.** 적이 단단해지는 대신 골드가 는다.
🔑 그래서 이 패시브는 **다른 패시브와 상호작용한다** — 화력이 이미 충분할 때만 이득이고,
Lv5(적 +125 % · 골드 +150 %)까지 밀면 **한 판을 통째로 그 도박에 건다.**

### 🎁 보상 쪽은 코드가 0줄이었다

`GameManager.GrantMetaGold` 가 이미 이렇게 되어 있다:
...
즉 **런 골드와 정산 골드가 같은 배율을 먹는다** — `BonusGoldGain` 열에 값을 넣는 것만으로
사용자가 말한 *"정산 골드 증가"* 가 그대로 된다. 🔵 **새로 만든 것은 대가 쪽뿐**이다
(`StatBlock.EnemyHpBonus`).

🔴 `AddPendingMetaGold`(상점 골드 전환)에는 **일부러 안 곱한다** — 런 골드를 벌 때 이미
그 배율을 받았으므로 전환에서 또 곱하면 **두 번 먹는다**(D71 이 적어 둔 함정이다).

### 🔴 적 체력은 태어날 때 한 번만 곱한다

```csharp
MaxHp = data.MaxHp * hpMult * LayerScaling.HpMult * PlayerHpTax;
```

층 배율 **바로 옆**에 둔 이유는 성격이 같아서다 — 둘 다 *"이 판의 적이 얼마나 단단한가"* 이고,
등급 배율(엘리트·보스) **위에** 얹혀야 한다.

🔴 **소급하지 않는다.** 이미 나와 있는 적까지 단단해지면 패시브를 고른 직후
화면의 적이 갑자기 안 죽어 *"내가 뭘 잘못 눌렀나"* 가 된다.
`LayerScaling` 이 층을 넘을 때만 적용되는 것과 같은 규칙이다.

🔵 `PlayerStats.Current` 는 **정적 프로퍼티**이고 이 코드는 웨이브 도중에 도는 함수라
`Awake` 순서 함정(I-8·I-38)과 무관하다 — `ProjectileBase` 가 특전을 읽는 방식과 같다.

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/Player/StatBlock.cs` | `EnemyHpBonus` — 🔴 **필드·`Zero()`·`Add()` 세 곳**을 같이 고쳐야 한다 (I-21) |
| `Assets/Scripts/Passive/PassiveData.cs` | `BonusEnemyHp` 배열 + `GetEnemyHp(lv)` |
| `Assets/Scripts/Passive/PassiveEffect.cs` | 합산 1줄 |
| `Assets/Scripts/Enemy/EnemyBase.cs` | `PlayerHpTax` + `MaxHp` 곱 1개 · `Version 5→6` |
| `Assets/Editor/BalanceImporter.cs` | `BonusEnemyHp` 읽기·내보내기 |
| `Assets/Game/Sprites/Passives/Bounty.png` | 신규 (해골이 새겨진 금화를 발톱 장갑이 쥔 그림) |
| `Assets/Game/Balance/{Passives,Items,SceneWiring}.csv` | 열 1개 + 행 2개 · `allItems` 38→39 |

### 검증 로그 (플레이 모드 실측)

🔑 **대조군은 "먹기 전에 태어난 같은 적"** 이다 — 같은 웨이브·같은 종류로만 비교했다.

| 적 | 먹기 전 `HpMax` | 먹은 뒤 태어난 개체 | 배 |
|---|---|---|---|
| Goblin (기본 30) | **30** | **45** | ×1.5 ✅ |
| Zombie (기본 70) | — | **105** | ×1.5 ✅ |

**② 소급 안 됨** — 먹기 전에 이미 나와 있던 그 고블린은 6초 뒤에도 **`HpMax=30` 그대로** ✅
**③ 정산 골드** — `GrantMetaGold(100)` 이 먹기 전 **`+100`**, 먹은 뒤 **`+150`** ✅
**④ 스탯** — `GoldGain 1 → 1.5` · `EnemyHpBonus 0 → 0.5` (Lv1 설계값) ✅
**애셋** — 아이콘 알파 `0~255` · 네 귀 전부 `0`(I-41 의 가짜 투명 아님) ·
임포트 설정은 `HpRegen.png.meta` 복사(`ppu 512` · `Point` · `Single`) · 콘솔 0

## 2-147. ✅ **화톳불 — 건물 중 처음으로 "거기 있을 이유"를 만든다** (D120, 2026-09-09 141차)

> 사용자: *"화톳불 (근처에 있으면 데미지 증가)"*

### 건물 9종이 전부 "설치하면 알아서 일한다"였다

그래서 **플레이어가 그 자리에 있을 이유가 하나도 없었다** — 지어 놓고 걸어가면 끝이다.
`D110` 이 진단한 문제(*"건물은 못 걷는 유일한 것"*)를 **세 번째 각도**로 푼다:

| | 어떻게 푸나 |
|---|---|
| **미끼** (D110) | 적을 **부른다** — 전장이 건물에 온다 |
| **바리케이드** (D117) | 공간을 **자른다** — 지나갈 수 없게 한다 |
| **화톳불** (D120) | 플레이어를 **붙든다** — 있고 싶게 만든다 |

🔑 **여기가 이 게임에 없던 긴장이다.** 뱀서라이크에서 플레이어는 늘 움직인다.
화톳불은 *"안전하지만 약하게 도망칠까"* 와 *"강하지만 서 있을까"* 를 부딪히게 한다.
서 있으려면 적을 막아야 하고, 그래서 **미끼·벽·보호막을 쓸 이유**가 생긴다.

### 🔑 새 갱신 루프를 안 만들었다

`BuildingBase` 의 쿨다운을 짧게(0.25초) 두고 **매 틱 버프를 다시 걸어 준다.**
벗어나면 아무도 안 걸어 주므로 **저절로 꺼진다** — `FreezerBuilding` 이 슬로우를 유지하는 방식과 같다.

🔴 **"나갔다"를 알아채는 코드가 없다.** 그런 코드는 늘 한쪽 경로를 빠뜨린다
(죽었을 때 · 건물이 부서졌을 때 · 씬이 바뀔 때…). 갱신이 끊기면 끝나는 쪽이 안전하다.

🔴 **지속시간은 주기보다 길어야 한다**(`holdMultiplier 1.8`). 정확히 주기만큼 주면
다음 갱신 직전에 한 프레임 꺼져 **피해가 깜빡인다.**

🔴 **매번 `RecalculateStats` 를 부르지 않는다.** 갱신이 초당 4번인데 재계산은
패시브·직업 사슬을 통째로 다시 더한다 — **값이 실제로 바뀔 때만** 돌린다.

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/Building/CampfireBuilding.cs` | 신규 · `Version=1` |
| `Assets/Scripts/Player/PlayerStats.cs` | `GrantCampfire` + `IsWarmed`/`CampfireDamage` + 합산 1줄 |
| `Assets/Prefabs/Building_Campfire.prefab` | 신규 (`Building_Decoy` 복제 + 컴포넌트·그림 교체) |
| `Assets/Game/Sprites/Buildings/Campfire.png` | 신규 |
| `Assets/Game/Balance/{Buildings,Items,SceneWiring}.csv` | 행 1개씩 + 머리말 · `allItems` 37→38 |

### 검증 로그 (플레이 모드 실측)

🔑 **대조군은 "같은 플레이어가 밖에 서 있을 때"** 다 — 다른 무엇도 안 바꾸고 **자리만** 옮겼다.

| | 밖 (대조군) | 안 |
|---|---|---|
| `Final.Damage` | **1.150** · `IsWarmed=False` | **1.350** · `IsWarmed=True` |
| 차이 | — | **정확히 0.200** (설계 `Output` 0.2) ✅ |
| 3.2초간 준 실제 피해 | **21.0** | **33.8** ✅ |

**② 나가면 돌아온다** — 다시 밖으로 나가니 `1.150` · `IsWarmed=False` ✅
(*"나갔다"를 알아채는 코드 없이* 갱신이 끊겨서 꺼진 것이다).
**③ 경계** — 반경 2.6 기준 **2.3 안 / 2.9 밖** 으로 정확히 갈렸다 ✅
**화면** — 게임 카메라 직접 렌더로 확인. 고리 48점 · 색 `FFBF4DE6`(따뜻한 색) ·
플레이어가 안에 있을 때 밝아진다 ✅

🔵 **④의 비 1.61 은 설계 1.20 보다 크다** — 검 쿨다운이 1.5초인데 측정 창이 3.2초라
**벤 횟수가 2회/3회로 갈렸다.** 회당 피해가 아니라 **횟수**가 비를 흔든 것이라
방향만 맞고 배율은 이 값으로 읽으면 안 된다. 정확한 값은 ①의 **0.200** 이다.

## 2-146. ✅ **지뢰 — 던지는 것과 두고 가는 것** (D119, 2026-09-09 140차)

> 사용자: *"지뢰"* (추천 목록에서 고름)

### 폭탄과 무엇이 다른가

`AoeWeapon`(폭탄·화염구)은 **가장 가까운 적**을 노려 날린다.
지뢰는 **플레이어가 지금 선 자리**에 놓는다 — 조준이 아니라 **내가 지나온 길**이 무기가 된다.

🔑 **그래서 이 게임의 핵심 동사와 짝이 맞는다.** 뱀서라이크에서 플레이어가 늘 하는 일은
**도망치는 것**인데, 지금까지 그 시간은 아무것도 낳지 않았다.
지뢰는 **도망친 거리만큼 피해가 된다** — 쫓아오는 무리가 제 발로 밟는다.

### 새 열을 하나도 안 만들었다

| 열 | 여기서 |
|---|---|
| `ProjectileCount` | 한 번에 **놓는 개수** |
| `Range` | 사거리가 아니라 **밟았다고 볼 반경**(기폭) |
| `ProjectileSize` | **폭발 반경**의 배수 |
| `ProjectileSpeed` | 유닛/초가 아니라 **수명(초)** |
| `ProjectilePrefab` | **지뢰 몸체**(`MineBody`) |
| `TravelPrefab` | 🔑 **터질 때 쓸 폭발**(`AoeProjectile`) — AoE 무기와 **반대로** 쓰인다 |

🔵 **폭발을 새로 만들지 않았다.** `AoeProjectile` 이 이미 *"반경만큼 커져 한 번 터지고 풀로
돌아가는"* 일을 한다 — `Initialize(dmg, radius, pool)` 을 그대로 빌려 썼다.

### 🔴 밟힘을 콜라이더로 잡지 않는다

`BarricadeBuilding`(D117)에서 배운 것과 같다 — 적 콜라이더가 `isTrigger` 라
지뢰에도 트리거를 달면 **플레이어·발사체·다른 건물까지 전부 들어온다.**
필요한 건 *"적이 반경 안에 있나"* 하나뿐이라 `OverlapCircle` 로 **적 레이어만** 본다.

### 🔴 세 가지 안전장치

| 무엇 | 왜 |
|---|---|
| `armDelay` 0.45초 | 🔴 **0 이면 즉발 폭탄과 똑같아진다** — 발밑에 놓는데 적이 붙어 있으면 바로 터진다 |
| `maxLive` 6 | 수명이 길고 쿨다운이 짧으면 **화면이 지뢰로 덮인다.** 오래된 것부터 회수 |
| `OnDisable` 회수 | 🔴 무기를 환불하면 **없는 무기의 지뢰가 필드에 남는다** (소환수가 겪은 일) |

무장 전에는 **어둡게** 보이고, 무장된 뒤에는 **깜빡인다** — 안 그러면 바닥 무늬와 구분이 안 된다.

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/Weapon/MineWeapon.cs` | 신규 · `Version=1` (놓기·개수 상한·회수) |
| `Assets/Scripts/Weapon/MineBody.cs` | 신규 · `Version=1` (무장·밟힘·수명·폭발) |
| `Assets/Prefabs/{Weapon_Mine,Proj_Mine}.prefab` | 신규 |
| `Assets/Game/Sprites/Weapons/Mine.png` | 신규 (아이콘 겸용) |
| `Assets/Game/Balance/{Weapons,Items,SceneWiring}.csv` | 행 1개씩 + 머리말 · `allItems` 36→37 |

### 검증 로그 (플레이 모드 실측 · 대조군은 폭탄)

| 무엇 | 결과 |
|---|---|
| ① 어디에 생기나 | **지뢰: 플레이어까지 0.00** vs **폭탄: 6.00**(적들이 선 반경) ✅ |
| ② 놓자마자 안 터지나 | 태어날 때 무장 **False** · **0.45초** 뒤 무장 (설계 0.45) ✅ |
| ③ 밟으면 터지나 | 지뢰 사라짐 · 적 hp **30.0 → 4.7** ✅ |
| ④ 수명이 걷어 가나 | 20초 관찰 **최대 4개** (상한 6에 안 참) ✅ |

### 🔴 측정이 세 군데 스스로 무너졌다 — 전부 고치고 다시 쟀다

**① 판정식이 틀렸다.** 적 셋을 같은 거리에 두고 `subs[0]` 하나만 기준으로 삼았는데,
폭탄은 **그중 아무나** 고른다. 가장 가까운 적으로 바꿔도 여전히 이상한 값(10.39)이 나왔는데,
알고 보니 **폭발한 자리의 적이 죽어서** 120° 옆 적까지의 거리(6√3 = 10.39)가 잡힌 것이었다.
🔑 **판정 문구는 끝내 못 고쳤지만 숫자가 결론을 말한다** — 지뢰 **0.00** vs 폭탄 **6.00**.

**② 표본이 매 프레임 바뀌었다.** 적을 붙여 놓고 무장 시각을 쟀더니
*"무장 → 즉폭 → 재설치"* 가 돌아 볼 때마다 **새 지뢰**였다(2초 타임아웃).
⇒ 무장 시각은 **적이 없을 때** 재야 한다. 고치니 정확히 **0.45초**가 나왔다.

**③ 잴 대상을 스스로 없앴다.** 수명을 재려고 무기를 뗐는데 `OnDisable` 이 지뢰를 **전부 걷어**
0개가 됐다. ⇒ **평형 개수**로 바꿨다 — 수명이 없으면 상한(6)까지 차오르고,
수명 8초 · 쿨다운 2.2초면 8/2.2 ≒ 3~4개에서 멈춘다. **최대 4개**가 나와 수명이 증명됐다.

## 2-145. ✅ **공전체 — 이 게임에서 유일하게 겨누지 않는 무기** (D118, 2026-09-09 139차)

> 사용자: *"공전체"* (추천 목록에서 고름)

### 왜 이게 빠져 있었나

무기 11종이 **전부 겨눠서 쏘거나 벤다.** 조준이 없고 늘 켜져 있는 것이 하나도 없어서
**등 뒤가 늘 열려 있었다.** 공전체가 파는 것은 피해가 아니라 **"뒤를 안 봐도 된다"** 이다.

### 새 열을 하나도 안 만들었다 — 여섯 개를 다시 읽었다

| 열 | 원래 뜻 | 여기서 |
|---|---|---|
| `ProjectileCount` | 동시 발사 수 | **구체 개수** |
| `Range` | 날아가는 거리 | **공전 반지름** |
| `ProjectileSpeed` | 유닛/초 | **회전 속도(도/초)** |
| `ProjectileSize` | 그림 크기 | 그림 크기 **겸 판정 반지름** 배수 |
| `Cooldown` | 발사 주기 | **같은 적을 다시 때리는 간격** |
| `ProjectilePrefab` | 발사체 | **구체 그림** (콜라이더도 강체도 없다) |

🔑 **`ProjectileCount` 를 개수로 읽은 것이 공짜 선물을 하나 줬다** —
`ScaledProjectileCount()` 를 그대로 쓰므로 **`DoubleProjectiles` 특전이 구체를 두 배로** 만든다.
따로 배선한 게 아니라 이름을 맞춘 결과다.

### 🔴 `Update` 를 선언하면 무기가 죽는다

`WeaponBase` 가 `Update` 로 쿨다운을 돌린다 — 파생 클래스에서 같은 이름을 쓰면 그걸 가려
**무기가 영영 발동하지 않는다.** 자리 잡기는 `LateUpdate` 에서 한다
(플레이어가 움직인 **뒤**라야 구체가 뒤늦게 끌려오지 않기도 하다).
`SummonWeapon` 이 같은 함정을 주석으로 남겨 둬서 처음부터 피했다.

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/Weapon/OrbitWeapon.cs` | 신규 · `Version=1` |
| `Assets/Prefabs/Weapon_Orbit.prefab` | 신규 (`Weapon_Sword` 복제 + 컴포넌트 교체) |
| `Assets/Prefabs/Proj_Orb.prefab` | 신규 · 🔴 **발사체가 아니다** — `ProjectileBase`·`Collider2D`·`Rigidbody2D` 를 다 뗐다 |
| `Assets/Game/Sprites/Weapons/Orb.png` | 신규 (아이콘 겸용) |
| `Assets/Game/Balance/{Weapons,Items,SceneWiring}.csv` | 행 1개씩 + 머리말 · `allItems` 35→36 |

### 검증 로그 (플레이 모드 실측)

🔑 **판정을 "사방 8방향"으로 짰다.** 겨누는 무기와 안 겨누는 무기의 차이는
**한 방향에서는 안 드러난다** — 둘러싸야 갈린다.
...
대조군이 정확히 **부채꼴 방향의 연속된 4마리**만 맞힌 것이 이 판정의 핵심이다.

**② 정말 도나** — 반지름 **1.41~1.60**(CSV 1.6 · 세로를 12 % 눌러 타원이므로 정상 범위),
2초간 돈 각도 합 **217°** (설계 110°/s ⇒ 220°) ✅
**③ 개수** — Lv1 2 · Lv3 3 · Lv5 4 로 **5/5 CSV 일치** ✅
`state=Wave` · `timeScale=1` 을 같이 찍어 얼어붙은 값이 아님을 확인했다. 콘솔 에러 0.

**화면** — 게임 카메라를 `RenderTexture` 에 직접 그려 확인. 구체 4개가 플레이어를 둘러싼다.
🟡 구체 지름이 **1.4 유닛**이라 플레이어보다 크다 — `ProjectileSize` 로 조절할 값이라 `TODO.md` 에 적었다.
(측정 시 `bounds` 가 **1.87** 로 나왔는데, 구체가 자전 중이라 축정렬 상자가 √2 배로 부푼 것이다 —
회전체의 `bounds` 를 크기로 읽으면 안 된다.)

## 2-144. ✅ **바리케이드 — 물리로는 막을 수가 없었다** (D117, 2026-09-09 138차)

> 사용자 원 요청(2026-09-08)의 건물 4종 중 **마지막 하나**:
> *"적들이 통과 못하고 도트데미지 주는 벽"*.
> 나머지 셋(멈춤=Siren · 슬로우=Freezer · 어그로=Decoy)은 `D108`·`D110` 에서 들어갔는데
> **이것만 빠져 있었다.**

### 🔴 물리로 막을 수가 없다 — 여기가 이 작업의 전부다

고치기 전에 콜라이더부터 읽었더니 **적의 콜라이더가 `isTrigger = true`** 였다
(접촉 피해를 `OnTriggerStay2D` 로 받는다). 트리거는 겹침을 **알려 줄 뿐 밀어내지 않는다** —
그래서 지금 **적은 모든 건물을 그냥 통과한다.**

콜라이더를 실체로 바꾸면 적이 플레이어·다른 건물·서로와 **전부 부딪히기 시작해**
이동과 접촉 피해가 통째로 달라진다. 벽 하나 만들자고 게임 전체를 흔들 수는 없다.

⇒ **이동 코드에서 막는다.** `ArenaBounds` 가 이미 `EnemyBase.FixedUpdate` 에서 쓰는
그 방식이고, **같은 자리에 같은 모양으로** 넣었다 —
early return 보다 위에 두는 이유도 같다(넉백으로 벽 안에 처박히는 게 제일 흔하다).

### 🔵 플레이어는 통과한다 — 공짜로 그렇게 됐다

요구가 *"**적들이** 통과 못하고"* 였고, 막는 코드가 `EnemyBase` 에만 있으므로
**아무것도 안 해도 플레이어는 지나간다.** 막았으면 제 벽에 갇히는 사고가 났을 것이다 —
놓는 자리를 사람이 못 고르기 때문이다.

### 미끼와 무엇이 다른가 — 동사가 다르다

| | 미끼 (D110) | 바리케이드 (D117) |
|---|---|---|
| 동사 | **끌어당긴다** (적이 제 발로 온다) | **막는다** (안 오는 게 아니라 못 지나간다) |
| 놓는 자리 | 어디든 일한다 | 🔑 **어디에 두느냐가 전부** |
| 방향 | 없다 | 🔴 **있다** — 가던 쪽을 가로막게 눕는다 |

방향은 CSV 가 아니라 **놓는 순간의 `PlayerController.LastMoveDir`** 이 정한다
(`D113` 이 텔레포트 때문에 만든 값을 그대로 재사용했다).

### 🔴 그림이 막는 범위를 속일 뻔했다

`sprite.bounds` 는 **텍스처 전체**지 그려진 잉크가 아니다. 벽 그림은 세로로 **26 %** 밖에
안 차 있어서 그대로 쓰면 **보이는 벽보다 막는 범위가 네 배**가 된다.
⇒ PNG 자체를 잉크 상자에 맞춰 잘랐다 (`1024x1024` → `992x277`).
**그림을 다시 만들면 반드시 다시 자를 것** — 안 자르면 조용히 이 함정으로 돌아온다.

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/Building/BarricadeBuilding.cs` | 신규 · `Version=1` |
| `Assets/Scripts/Enemy/EnemyBase.cs` | `Version 5→6` · `ArenaBounds` 바로 아래에 막기 6줄 |
| `Assets/Prefabs/Building_Barricade.prefab` | 신규 (`Building_Decoy` 복제 + 컴포넌트 교체 + 그림 교체) |
| `Assets/Game/Sprites/Buildings/Barricade{,_Broken}.png` | 신규 · 🔴 잉크에 맞춰 잘랐다 |
| `Assets/Game/Balance/{Buildings,Items,SceneWiring}.csv` | 행 1개씩 · `allItems` 34→35 |

### 검증 로그 (플레이 모드 실측 · 전부 대조군 동반)

| 무엇 | 대조군 | 결과 |
|---|---|---|
| ① 못 지나간다 | **벽 없음** → 적이 x 거리 **1.54** 까지 접근 | 벽 있으면 **3.07** 에서 멈춤 ✅ |
| ② 아프다 | **멀리 둔 적 3마리** → hp 변화 **0** | 미는 적 **3/3** hp 감소 ✅ |
| ③ 무너진다 | — | 표면에 붙들어 두고 9.87초 → 내구도 7.46 → **-0.02 · 부서짐=True** ✅ |
| ③ 부서지면 통과 | — | 두께방향 **-0.90 → +0.88**(부호 반전) · 길이방향 -0.78(|1.4| 안) ✅ |
| ④ 플레이어 통과 | — | 벽 너머로 옮겨도 **안 밀려남** ✅ |

**화면** — 게임 카메라를 `RenderTexture` 에 직접 그려 확인.
세로로 서서 +X 를 가로막고 가시가 바깥을 향한다.
`SpriteRenderer.bounds` **0.76 x 2.80** = 막는 크기와 정확히 같다 (그림이 안 속인다).

### 🔴 측정이 두 번 틀렸다 — 둘 다 "축을 하나만 봐서" 였다

**① 첫 판: "적이 벽을 뚫었다"** — 월드 x 만 찍었더니 적이 벽을 지나간 것처럼 보였다.
벽의 **로컬 좌표**(길이 방향 / 두께 방향)로 바꿔 찍었더니
길이방향이 `-0.27 → -2.35` 로 **벽 끝(|1.4|)을 넘은 뒤에야** 두께 부호가 바뀌었다 —
**뚫은 게 아니라 끝을 돌아간 것**이다. 🔑 이건 버그가 아니라 **원하는 동작**이다
(우회할 수 없으면 봉인이 된다).

**② 둘째 판: "내구도가 거의 안 닳는다"** — 3마리가 2.9초를 밀었는데 **0.32** 만 닳았다.
원인이 둘이었다:
- `PushOut` 이 적을 **정확히 경계**에 놓으므로 다음 프레임의 `Contains` 가 `<` 비교라
  false 가 된다 ⇒ 도트가 **격프레임으로 빠진다.** 접촉 여유(`contactSkin 0.06`)를 줬다.
- 🔴 **그러고도 0.30 이었다.** 진짜 이유는 다른 것이었다 —
  **적이 벽에 붙어 있질 않고 곧바로 끝으로 우회한다.**
  ⇒ 그래서 ③ 은 적을 **표면에 붙들어 두고** 따로 쟀다.
  *"닳는 속도가 맞나"* 는 기구가 아니라 **수치**이고, 플레이로 정할 값이다 → `TODO.md`.

🔵 곁일로, 프로브가 적을 `transform.position` 으로 옮기니 `EnemyBase` 가 읽는
`Rb.position` 과 한 프레임 어긋나 접촉이 **5번 중 1번**만 잡혔다. 강체 쪽으로 옮겨 고쳤다.

### 🔴 봉인 방지 논리를 실측이 바꿨다

설계할 때는 *"미는 적이 많을수록 빨리 부서지니 가두려 할수록 빨리 무너진다"* 로 잡았다.
실측은 다르게 말한다 — **적은 벽에 오래 붙어 있지 않고 곧바로 우회한다.**
⇒ 봉인을 막는 것은 **내구도가 아니라 우회**다. 내구도는 *"오래 두면 언젠가 무너진다"* 쪽이다.
길이(`AttackRange` 1.4~2.4)를 크게 올리면 이 균형이 깨진다 — **우회 거리가 곧 안전장치**다.

## 2-143. ✅ **SFX 4종 — 빌려 쓰던 소리를 전부 걷어냈다** (D116, 2026-09-08 137차)

> 사용자: *"SFX도"* (이펙트를 새로 만들라는 지시에 이어서)

`D112`·`D113` 이 넣은 것들이 **남의 소리를 빌려 쓰고 있었다.** 넷 다 뜻이 어긋난다:

| 언제 | 빌려 쓰던 것 | 무엇이 틀렸나 |
|---|---|---|
| 마이크 공격 | `WeaponSwing`(4) | **칼바람 소리**다. 마이크를 들고 있는데 칼 소리가 난다 |
| 순간이동 | `Magnet`(23) | **빨려 들어오는** 소리다. 순간이동은 반대로 빠져나가는 것이다 |
| 보호막 충전 | `BuffPickup`(26) | **먹는** 소리다. 아무것도 안 먹었는데 바닥을 쳐다보게 된다 |
| 보호막 막음 | `UiCancel`(41) | **UI 소리**다. 전투 중에 메뉴 취소음이 난다 |

### 자리를 먼저 재고 소리를 만들었다

`Tools/Audio/compare_sfx.py` 가 이미 **길이·어택·대역**으로 짝마다 거리를 낸다.
새 소리를 짓기 전에 그 표를 보고 **비어 있는 자리**를 골랐다.

🔴 **그 자를 먼저 고쳐야 했다** — `BombPickup`·`BuffPickup`·`GoldPickup` 셋이
`_Incoming/` 경로로 남아 **표에서 조용히 사라져 있었다.** 그 파일의 머리말이
*"이미 있는 소리가 목록에서 사라지는 게 이 자의 가장 위험한 고장"* 이라고 경고한
바로 그 일이 다시 나 있었다. 경로를 고치고 `PlayerHit`·`EnemyShoot`·`UiCancel` 도 표에 넣었다.

### 세 번 부딪히고 세 번 옮겼다 — 전부 자가 잡았다

| 소리 | 처음 | 무엇과 부딪혔나 | 어떻게 옮겼나 |
|---|---|---|---|
| `MicPulse` | 31/33/28/7 | 🔴 **`EnemyHit` 과 0.79** (목록 최악 · 대역 거리 **0.12**) | **200~800Hz 로 밀어 올렸다.** 거기 있는 것들(`UiCancel`·`WeaponFire`·`LevelUp`)은 전부 **길고 느리다** — 짧고 빠른 소리가 없는 자리다 |
| `ShieldUp` | A6/E7 · 0.42s | 🔴 `WeaponCast` 와 **0.65** → 한 옥타브 내렸더니 이번엔 `XpPickup` 과 **0.64** | 대역만 옮기다 **두더지잡기**가 됐다. 축을 바꿔 **길이가 빈 자리(0.70~0.85s)** 로 갔다 |
| `ShieldBlock` | 0.18s | 🔴 `GoldPickup` 과 **0.70** (길이·어택이 사실상 같았다) | **0.13s 로 끊고 저역을 확실히** 줬다 — 금화는 0~800Hz 가 **0%** 라 그게 유일한 손잡이다 |

🔴 **같은 실수를 세 번 했다** — 사인 층의 이득을 **눈대중**으로 넣었다.
사인은 결맞음이라 같은 진폭에서 잡음보다 에너지가 수십 배다. 첫 `MicPulse` 는
저역이 **81%** 로 나와 `Explosion` 자리를 통째로 먹었고, `Blink`·`ShieldBlock` 은
저역이 **2% · 0.3%** 로 파묻혀 `GoldPickup` 과 가르는 손잡이가 사라졌다.
⇒ 이제 **층마다 rms 를 재서 `g = sqrt(목표비율) / rms`** 로 잡는다. 세 스크립트에 다 적었다.

### 최종 자리

| 소리 | 길이 | 어택 | 0-200 / -800 / -3k / -9k | 가장 가까운 짝 |
|---|---:|---:|---|---|
| `MicPulse` (8) | 0.320s | 4.5ms | 6 / **53** / 30 / 10 | `ToxinSpill` **1.08** |
| `ShieldBlock` (18) | 0.130s | 4.0ms | **57** / 4 / 17 / 20 | 1.05 위 |
| `Blink` (28) | 0.260s | 39ms | 4 / 20 / **39** / 34 | 1.03 위 |
| `ShieldUp` (29) | 0.780s | 100ms | 0 / 0 / **66** / 23 | 🟡 `PlayerHit` **0.85** |

🟡 **`ShieldUp` 만 0.85 다.** 기존에 이미 있던 `LevelUp`↔`Magnet`(0.85)과 같은 수준이고
`Magnet`↔`EnemyShoot`(0.83)보다 멀다. 더 옮기면 또 다른 것에 붙는 게 두 번 확인됐으므로
여기서 멈추고 **귀 확인 항목**으로 남긴다 (자 자신이 *"판정은 귀로만 한다"* 고 적어 두었다).

### 🔴 마이크와 검은 **같은 클래스**를 쓴다 — 소리를 코드에 박으면 안 됐다

`MeleeWeapon.SwingCombo` 가 `AudioManager.Play(SfxId.WeaponSwing)` 을 **박아** 두고 있었다.
검과 마이크가 같은 컴포넌트를 쓰므로 그대로는 마이크에서도 칼 소리가 난다.
⇒ `[SerializeField] private SfxId swingSfx` 를 두어 **프리팹이 고르게** 했다.

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Tools/Audio/gen_{mic_pulse,blink,shield_up,shield_block}.py` | 신규 4개 |
| `Tools/Audio/compare_sfx.py` | 🔴 `_Incoming/` 로 사라져 있던 3개 경로 복구 + 4개 추가 |
| `Assets/Game/Audio/SFX_{MicPulse,Blink,ShieldUp,ShieldBlock}.wav` | 신규 4개 |
| `Assets/Game/Audio/AudioLibrary.asset` | 항목 28 → **32** |
| `Assets/Scripts/Audio/AudioId.cs` | `MicPulse 8` · `ShieldBlock 18` · `Blink 28` · `ShieldUp 29` |
| `Assets/Scripts/Weapon/MeleeWeapon.cs` | `swingSfx` 필드 — 🔴 박아 두면 마이크에서 칼 소리가 난다 |
| `Assets/Prefabs/Weapon_Mic.prefab` | `swingSfx = MicPulse` |
| `Assets/Scripts/Player/{PlayerShield,PlayerTeleport}.cs` | 빌려 쓰던 3곳 교체 |

### 검증 로그 (플레이 모드 실측)

🔴 **"소리가 났나"가 아니라 "그 클립이 났나"를 쟀다** — `AudioManager` 는 못 찾은 `SfxId` 를
조용히 무시할 수 있다. 울린 직후 **실제로 재생 중인 `AudioSource.clip` 이름**을 읽었다
(D104 의 *"뜬다 ≠ 보인다"* 와 같은 계열).

```
① 마이크      요청=MicPulse    → 실제 클립=SFX_MicPulse     ✅ vol=0.033 pitch=0.99
② 순간이동    요청=Blink       → 실제 클립=SFX_Blink        ✅ vol=0.053 pitch=1.03
③ 보호막 충전 요청=ShieldUp    → 실제 클립=SFX_ShieldUp     ✅ vol=0.064 pitch=1.00
④ 보호막 막음 요청=ShieldBlock → 실제 클립=SFX_ShieldBlock  ✅ vol=0.082 pitch=0.95
⑤ 대조군: 검  요청=WeaponSwing → 실제 클립=SFX_WeaponSwing  ✅ vol=0.035 pitch=1.01
```

**대조군을 같이 잰 이유** — 마이크만 재면 *"원래 그 소리였다"* 와 구분이 안 된다.
프리팹도 저장된 파일에서 다시 읽어 확인했다:
`Weapon_Mic swingSfx=MicPulse · halfAngle=32` / **`Weapon_Melee swingSfx=WeaponSwing · halfAngle=70`(그대로)**.

임포트 설정은 지어내지 않고 `SFX_GoldPickup.wav` 것을 읽어 복사했다
(`forceToMono` · `DecompressOnLoad` · `ADPCM` · 44100Hz — 넷 다 `ch=1` 로 확인).

## 2-142. ✅ **마이크 음파 원뿔 — 검격을 재탕하지 않는다** (D115, 2026-09-08 136차)

> 사용자: *"이펙트 재사용하지 말고 새로 만들어"*

`D112` 는 마이크 이펙트로 `Fx_SwingArc`(검격)를 복제하고 **보라 틴트만** 입혔다.
색으로 구분은 됐지만 **모양이 초승달 검격**이라 *"소리가 앞으로 퍼진다"* 로 안 읽혔다.

### 검격과 **반대로** 그렸다 — 그게 이 그림의 존재 이유다

| | 검격 | 음파 |
|---|---|---|
| 무엇이 움직이나 | 한 덩어리 초승달이 **호를 따라 쓸고 지나간다** | 여러 겹의 띠가 **중심에서 바깥으로 밀려 나간다** |
| 눈이 따라가는 방향 | **옆** | **앞** |

프레임이 넘어갈 때 시선이 도는 방향이 달라서, 색을 빼도 구분된다.

### `SwingArcFx` 의 약속 위에 얹었다 — 코드는 한 줄도 안 고쳤다

`SwingArcFx` 가 이미 *"6프레임을 30fps 로 돌리고 풀로 돌아간다"* 를 하고 있어서
**그 약속을 그대로 지키는 시트를 만들면 끝**이었다.

| 약속 | 값 | 왜 |
|---|---|---|
| 시트 | `1536 x 256` = 256칸 6개 (Multiple) | `frames` 배열이 6칸이다 |
| PPU | **100** | `spriteRadiusAtScaleOne` 계산이 여기 얹혀 있다 |
| 피벗 | `(0.5, 0.5)` · alignment 0 | 🔴 **회전 중심이 칸 중앙**이라야 `z` 회전이 방향과 맞는다 |
| 그림 위치 | **오른쪽 반쪽에만** | 위와 같은 이유 |
| 색 | **흰색** | 색은 프리팹의 `SpriteRenderer.color` 가 정한다 (`gen_fx.py` 와 같은 방식) |

🔴 **각도는 `Weapon_Mic.prefab` 의 `halfAngle`(32) 과 같은 값이라야 한다** —
안 그러면 **그림이 사거리를 속인다.** 생성 스크립트 머리말에 적어 두었다.

🔴 **바깥 반지름 118px → `spriteRadiusAtScaleOne = 1.18`.**
처음에 124 로 잡았더니 띠 두께(`BAND_W`)만큼 더 나가 **마지막 프레임이 칸에 걸렸다**
(118 × 1.075 = 126.9 < 128). 그리고 `r = 1.0` 에서 알파를 칼같이 끊었더니
소리가 퍼지다 만 게 아니라 **"잘린 그림"** 으로 보여서 바깥 경계도 부드럽게 했다.

🔵 **필터는 `Bilinear`** 다 — `SwingArc` 는 `Point` 인데 여기만 다르다.
픽셀아트가 아니라 **빛**이라 `Point` 로 두면 부드러운 띠 가장자리가 계단이 된다
(`gen_fx.py` 가 같은 이유로 `Bilinear` 을 골랐다).

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Tools/Art/gen_sound_cone.py` | 신규 · PIL 절차 생성 (`gen_fx.py` 와 같은 방식) |
| `Assets/Game/Sprites/Effects/SoundCone.png` | 신규 · `1536x256` 6프레임 시트 |
| `Assets/Prefabs/Fx_SoundCone.prefab` | `frames` 6칸 교체 · `spriteRadiusAtScaleOne` 1.074 → **1.18** |

### 검증 로그

**시트** — 프레임별 알파 경계 `bbox` 를 재서 **6칸 전부 칸 안**(x 최대 245 < 255)임을 확인.
`x` 최소가 131 이라 **중앙(128)보다 오른쪽**에만 그려졌다 ✅.

**게임 화면** — 🔴 `activeSelf` 로 판정하지 않았다 (D104).
호가 실제로 뜬 프레임에 **`timeScale` 을 0 으로 얼리고** 그 프레임을 찍었다:

```
호 발견 — 이름=Fx_SoundCone(Clone) · sprite=SoundCone_0 · renderer.enabled=True
        · color=B873FFF2 · scale=3.39 · 월드 폭=8.68 x 8.68 · 검격이 아닌가=True
```

`scale 3.39` = `Range 4 / 1.18` ✅ (그림이 사거리를 안 속인다).
게임 카메라를 **렌더 텍스처에 직접 그려** PNG 로 뽑아 눈으로 확인 —
플레이어에서 앞으로 퍼지는 보라 음파, 바깥 끝이 적(3유닛)을 지나 사거리(4)에 닿는다.

🔴 **`Unity_Camera_Capture` 에 인스턴스 ID 를 넘겼는데도 씬 뷰가 찍혔다.**
CLAUDE.md 가 경고한 함정인데 인자를 넘겨도 안 됐다 —
`Camera.main` 을 `RenderTexture` 에 직접 그리는 쪽으로 우회했다.

**대조군** — `Fx_SwingArc` 는 `frames[0]=SwingArc_0` · `radius=1.074` 로 **그대로**다.

## 2-141. ✅ **쿠나이 진화 특전 — "하나 더"의 뜻이 무기마다 다르다** (D114, 2026-09-08 135차)

> 사용자: *"쿠나이 진화 특전 추가해"*

`D112` 가 쿠나이를 넣으면서 `Perk` 를 **비워 뒀다.** 남은 3종
(`DoubleProjectiles`·`DoubleBuffDuration`·`DoubleGold`)이 전부 다른 진화에 쓰였고,
새 특전은 `EvolutionPerk` enum 추가라 코드 작업이라서였다.

### 무엇을 줄 것인가 — 쿠나이의 정체를 런 전체로 옮긴다

쿠나이가 파는 것은 **"첫 적에서 멈추지 않는다"** 이다.
그래서 특전도 그것이어야 한다 — `ExtraPierce`, **모든 발사체가 적을 하나 더 맞힌다.**

🔑 **값이 하나인데 무기마다 뜻이 다르다.**

| 무기 | "하나 더"가 무엇인가 |
|---|---|
| 직선 발사체 (화살·총알·수리검) | **하나 더 뚫는다** (`ProjectileBase` 의 관통) |
| 튕기는 발사체 (쿠나이) | **한 번 더 튕긴다** (`BouncingProjectile` 의 도약) |

둘 다 결과는 **같은 문장**이다 — *"한 발이 적을 하나 더 맞힌다."*
🔴 **한쪽 언어로만 구현하면 다른 쪽에서 뜻이 어긋난다** — 튕기는 발사체에게
관통 +1 은 *"직선으로 하나 더 뚫는 것"* 이라 쿠나이 자신에게는 거의 쓸모가 없다.
그래서 `BouncingProjectile.Initialize` 가 같은 값을 **도약 +1** 로 다시 읽는다.

🔴 **폭발·장판·근접에는 아무 일도 안 일어난다.** `AoeProjectile`·`BombProjectile`·
`MeleeWeapon` 은 `ProjectileBase` 를 안 쓴다 — 애초에 뚫을 것이 없다.
🔵 적의 발사체(`EnemyProjectile`)도 다른 클래스라 영향이 없다.

🔵 **무기가 아니라 발사체가 읽는다.** 특전은 런 도중에 켜지므로
무기 생성 시점에 한 번 읽어 두면 **그 무기만 영영 못 받는다.**

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/Evolution/EvolutionData.cs` | `EvolutionPerk.ExtraPierce = 4` — 🔴 정수 직렬화라 **뒤에만** 붙였다 |
| `Assets/Scripts/Player/PlayerStats.cs` | `BonusPierce` + `ApplyEvolutionPerk` 분기 1줄 |
| `Assets/Scripts/Weapon/ProjectileBase.cs` | `BonusPierceNow` + `Initialize` 에서 관통에 더한다 |
| `Assets/Scripts/Weapon/BouncingProjectile.cs` | 같은 값을 **도약**으로 읽는다 |
| `Assets/Game/Balance/Evolutions.csv` | `Kunai` 의 `Perk` = `ExtraPierce` + 머리말 |
| `Assets/Game/Balance/Weapons.csv` | *"Perk 를 비웠다"* 서술을 바로잡았다 |

### 검증 로그 (플레이 모드 실측)

🔴 **무기로 쏘지 않고 발사체를 하나만 직접 쏘았다.** 쿠나이는 `ProjectileCount 2` 라
무기로 쏘면 **두 발이 섞여** *"한 발이 몇을 맞히나"* 를 못 잰다.
🔴 **피해를 1 로 주었다.** 크게 주면 적이 죽어 풀로 돌아가고 **사슬이 거기서 끊긴다** —
특전 탓인지 죽어서인지 구분이 사라진다.
🔴 **특전은 끄는 경로가 없다**(D68 의 설계). 그래서 **대조군 둘을 먼저 다 재고** 그 다음에 켰다.

| 무엇 | 배치 | 대조군 | 특전 켠 뒤 |
|---|---|---|---|
| ① 직선 (수리검 · 관통 3) | 일렬 5마리 | **3/5** `[OOO..]` ✅ | **4/5** `[OOOO.]` ✅ |
| ② 튕김 (쿠나이 · 도약 3) | 지그재그 6마리 | **4/6** `[OOOO..]` ✅ | **5/6** `[OOOOO.]` ✅ |

**4/4 예상 일치.** 🔑 배치를 갈라 놓은 것이 판정의 핵심이다 —
일렬이라야 "관통"이 뜻을 갖고, 흩어져야 "튕김"이 뜻을 갖는다.
지그재그는 `y` 를 1 이상 벌려 **직선으로는 1마리만 맞도록** 잡았다.

`BonusPierce` 0 → **1** · `Perk` 4종 전부 정상(`ExtraPierce (4)`) ·
`state=Wave` · `timeScale=1` (얼어붙은 값이 아님) · 콘솔 에러 0.

## 2-140. ✅ **패시브 3종 — 수치를 키우는 것과 없던 동작을 켜는 것** (D113, 2026-09-08 134차)

> 사용자: *"패시브 추가 — 소환수 증가 · 일정 시간마다 텔레포트(캐릭터가 향한 방향으로) · 보호막(원거리 공격 막아주는 쉴드)"*
...
지금까지의 패시브 12종은 전부 **이미 있는 수치를 키운다**(피해 +10 % 식).
이 셋은 **없던 동작을 켜는다.** 그래서 새 범주를 만들고 싶어지는데, 그러지 않았다 —
**`StatBlock` 에 필드를 더하고 `0` 을 "기능 없음"으로 읽는다.**
그러면 CSV → `PassiveData` → `StatBlock` → `Final` 파이프라인을 **통째로 그대로 쓴다.**

🔴 **대신 함정이 하나 생겼다** — `TeleportInterval`·`ShieldInterval` 은 **작을수록 좋은** 값인데
`StatBlock.Add` 는 더한다. 지금은 쓰는 패시브가 각각 하나뿐이고 레벨 배열이
*"그 레벨일 때의 총값"* 이라 덮어쓰기처럼 동작하지만,
**두 번째 출처가 생기면 합쳐서 오히려 느려진다.** CSV 머리말과 필드 주석에 적어 두었다.

### ① 소환수 증가 — `SummonWeapon` 이 몸통을 **하나만** 낳고 있었다

`_body` 하나를 `_bodies` 목록으로 바꿨고, 공격도 **몸통마다 따로** 돌리도록 고쳤다.
🔴 대표 하나만 쏘게 두면 *"소환수 증가"* 가 **화면에만 늘고 세기는 그대로**라
패시브가 거짓말이 된다.

🔑 **마릿수를 매 프레임 다시 읽는다.** 패시브는 **판 도중에** 얻고 올린다 —
소환 시점에 한 번만 세면 이미 소환해 둔 드래곤은 영영 한 마리로 남는다.

🔴 기차 간격 계산도 같이 고쳤다 — 앞칸이 먹는 길이가 **마릿수만큼**이다.
안 곱하면 앞 무기의 2번째 몸통과 내 1번째가 **같은 자리에 겹친다.**

수치는 `1|1|2|2|3` 으로 잡았다 — 🔴 **Lv1 부터 1 을 준다.**
0 으로 시작하면 *"먹었는데 아무 변화가 없는"* 레벨이 생긴다.

### ② 텔레포트 — 이건 **방어 기술**이다

뱀서라이크에서 플레이어의 유일한 방어 동사는 *"걸어서 벗어난다"* 인데
둘러싸이면 걸을 곳이 없다. 순간이동은 **걷지 않고 벗어나는** 유일한 수단이라
포위를 푼다 (`D110` 의 미끼와 짝이다 — 그쪽은 *적을 다른 데로 보내고*, 이쪽은 *내가 빠진다*).

🔴 **바라보는 쪽을 새로 만들어야 했다** — `PlayerController` 가 방향을 안 들고 있었다.
그림에도 방향이 없다(걷기 시트가 전부 정면이다 · `D100`). `LastMoveDir` 을 더했는데
🔴 *"지금 누르고 있는 키"* 가 아니라 **마지막으로 움직인 쪽**이다 —
손을 떼면 입력은 0 이 되는데 그때도 캐릭터는 어딘가를 향하고 있어야 한다.

🔴 위치는 `Rigidbody2D.position` 으로 옮긴다. `transform.position` 만 바꾸면
물리가 다음 `FixedUpdate` 에 옛 자리로 되돌린다 (`D111` 에서 같은 자리를 밟았다).
그리고 `ArenaBounds.Clamp` 를 거친다 — 안 거치면 **순간이동만 벽 밖으로 나가**
적이 못 따라오는 자리가 생긴다.

### ③ 보호막 — **원거리만** 막는다

`TryTakeHit` 에 `ranged` 인자를 더했다. 🔴 **기본값이 `false` 인 것이 핵심이다** —
접촉 호출부(`EnemyBase`·`BossSlam`)는 **한 줄도 안 고쳤다.**
새 인자가 생겼다고 전부 `true` 로 채우면 보호막이 접촉까지 막아
**무적 픽업과 구분이 사라진다.** 고친 곳은 `EnemyProjectile` **한 줄**이다.

🔵 **막았을 때 무적 시간을 주지 않는다.** 주면 보호막 한 장이
그 뒤 0.5초의 공격까지 통째로 지워 버려 *"한 번 막는다"* 가 아니게 된다.

고리 연출은 애셋 없이 `LineRenderer` 로 그렸고,
🔴 셰이더는 **URP 2D 것**을 썼다 — 빌트인 `Sprites/Default` 은
`isSupported` 가 **true 인데도 URP 2D 렌더러가 한 픽셀도 안 그린다** (`D111`).

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/Player/StatBlock.cs` | 필드 3개 + `Zero()` + `Add()` — 🔴 **세 곳을 같이** 고쳐야 한다 (I-21 / B13) |
| `Assets/Scripts/Passive/PassiveData.cs` | `Bonus*` 배열 3개 + 게터 3개 |
| `Assets/Scripts/Passive/PassiveEffect.cs` | `Apply` 에 3줄 — 🔴 여기를 빠뜨리면 경고 없이 그 스탯만 죽는다 |
| `Assets/Editor/BalanceImporter.cs` | 읽기·내보내기 양쪽에 열 3개 |
| `Assets/Scripts/Weapon/SummonWeapon.cs` | `_body` → `_bodies` 목록. 공격·따라가기·기차 간격 전부 몸통마다 |
| `Assets/Scripts/Player/PlayerTeleport.cs` | 신규 · `Version=1` |
| `Assets/Scripts/Player/PlayerShield.cs` | 신규 · `Version=1` |
| `Assets/Scripts/Player/PlayerController.cs` | `LastMoveDir` 추가 |
| `Assets/Scripts/Player/PlayerStats.cs` | `TryTakeHit(.., ranged)` + 보호막 소비 |
| `Assets/Scripts/Enemy/EnemyProjectile.cs` | 호출에 `ranged: true` — **이 한 줄이 전부다** |
| `Assets/Game/Balance/Passives.csv` | 열 3개 + 행 3개 + 머리말 |
| `Assets/Game/Balance/Items.csv` · `SceneWiring.csv` | 행 3개 · `allItems` 31→34 |
| `Assets/Game/Sprites/Passives/{SummonCount,Teleport,Shield}.png` | 신규 아이콘 |

### 검증 로그 (플레이 모드 실측 — 전부 대조군을 같이 재었다)

**① 소환수 증가** — 대조군(패시브 없음) 몸통 **1**
...
**② 텔레포트** — 대조군(패시브 없음) 4초 동안 도약 **0회** · 한 프레임 최대 이동 **0.000**.
Lv5(주기 5초)로 12초 → 도약 **2회** · 한 프레임 최대 이동 **5.000**(= `distance`).
걷기로는 한 프레임에 **0.016** 밖에 못 간다.

**③ 보호막** — Lv5 기준 **6.0초** 만에 충전(설계값 6초).

| 종류 | `TryTakeHit` | 막은 횟수 | HP |
|---|---|---|---|
| **원거리** (999 피해) | **False** ✅ | 0 → **1** | 207.3 → **207.3** ✅ |
| **접촉** (대조군, 5 피해) | **True** ✅ | 1 → **1** ✅ | 205.7 → 202.7 |

막은 직후 `IsInvincible` = **False** ✅ (막기는 무적이 아니다) ·
접촉 뒤 `IsCharged` = **True** ✅ (접촉은 막을 소비하지 않는다).

**배선** — `allItems` 31→**34** · `PassiveData` 3개 생성 · 아이콘 3장 물림 ·
`StatBlock.Add` 통과(2/7/11 그대로 나옴) · 콘솔 에러 0.

### 측정 중에 걸렸던 것

측정이 도중에 멈췄고 `timeScale=0` 이었다. 패널을 전부 조회해 보니
🔴 **`StageClearPanel`** 이었다 — 측정 오염을 막으려고 **적을 전부 구석으로 치워놓았더니**
웨이브가 먼저 클리어된 것이다. **버그가 아니라 내 측정이 웨이브보다 길었다.**
시간을 되돌려 나머지를 마저 재고, 로그에 `state`·`timeScale`·`Time.time` 을 같이 찍어
얼어붙은 값을 결함으로 읽는 걸 막았다.

## 2-139. ✅ **무기 2종 — 하나는 코드 0줄, 하나는 관통과 다른 것** (D112, 2026-09-08 133차)

> 사용자: *"무기 추가 — 마이크(일정 시간마다 원뿔 모양 범위 공격) · 적들 사이를 튕겨다니는 쿠나이(표창 진화)"*

### ① 마이크 — **새 코드가 한 줄도 안 들었다**

고치기 전에 `MeleeWeapon` 을 먼저 읽었는데, `Strike()` 가 **이미 부채꼴 판정**을 하고 있었다:

```csharp
if (to.sqrMagnitude > 1e-6f && Vector2.Angle(dir, to) > halfAngle) continue;
```

그래서 마이크는 **검과 같은 클래스**를 쓰고 프리팹 값만 다르다 — `halfAngle` **70° → 32°**.
좋은 거래인 이유는 따로 있다: **좁아진 만큼 멀리 보낸다.** `Range` 를 검(2→2.5)의
두 배 넘게(4→5.6) 줄 수 있었고, 그게 이 무기의 정체다 —
**검은 둘러싸였을 때, 마이크는 한 방향에서 밀려올 때** 값어치가 있다.

### ② 쿠나이 — **관통과 튕김은 다른 문제를 푸는 것이다**

관통(`pierceCount`)은 **직선을 유지**한 채 여럿을 뚫는다 — 일렬로 선 적에게만 값어치가 있다.
튕김은 **방향을 바꿔** 흩어진 적을 잡는다. 그래서 수리검(관통 3)의 진화로 자연스럽다 —
**같은 무기가 반대 상황을 커버하게 된다.**

**중복 방지를 새로 만들지 않았다.** `ProjectileBase._alreadyHit` 이 관통용으로 이미
*"이 발사체가 맞힌 적"*을 들고 있었고, 그게 튕김에도 그대로 맞는다.
🔴 **이게 없으면 두 적 사이를 무한히 오간다.**

🔴 **함정 하나를 설계 단계에서 잡았다** — 부모는 맞힐 때마다 관통을 깎고 0 이면
`Despawn()` 한다. 프리팹의 `pierceCount` 가 1 이면 **첫 명중에 사라져 한 번도 안 튕긴다.**
값을 둘(관통·도약) 두고 사람이 맞추게 하면 **어긋나면 조용히 깨진다** —
`Initialize` 안에서 `SetPierce(_bouncesLeft + 1)` 로 **코드가 계산**한다.

그리고 `Redirect()` 는 방향만 바꾸는 게 아니라 **사거리 기준점(`_startPos`)도 같이 옮긴다** —
안 그러면 이미 사거리를 다 쓴 지점에서 다시 재서 **두 번째 도약이 거의 못 간다.**

### 바꾼 파일

| 파일 | 무엇을 |
|---|---|
| `Assets/Scripts/Weapon/BouncingProjectile.cs` | 신규 · `Version=1`. 맞힌 뒤 반경 6 안의 **안 맞은** 최근접 적으로 꺾인다 |
| `Assets/Scripts/Weapon/ProjectileBase.cs` | `Redirect()` · `AlreadyHit()` · `SetPierce()` 추가 (private 필드를 안 열기 위한 창구) |
| `Assets/Prefabs/Weapon_Mic.prefab` | 신규 · `Weapon_Melee` 복제 + `halfAngle 32` · `hitDelay 0.05` · `comboInterval 0.14` |
| `Assets/Prefabs/Fx_SoundCone.prefab` | 신규 · 🟡 **자리표시** — `Fx_SwingArc`(검격) 복제 + 보라 틴트(`B873FF`) |
| `Assets/Prefabs/Proj_Kunai.prefab` | 신규 · `Proj_Shuriken` 복제 + `BouncingProjectile` 교체 + `spinSpeed 0` + **스프라이트 교체** |
| `Assets/Game/Sprites/Weapons/Mic.png` | 신규 (아이콘) |
| `Assets/Game/Sprites/Weapons/Kunai.png` | 신규 (아이콘 · 45°) |
| `Assets/Game/Sprites/Weapons/KunaiProj.png` | 신규 · 🔴 **수평·+X 향함** (발사체용) |
| `Assets/Game/Balance/Weapons.csv` | `Mic` · `Kunai` 행 + 머리말 2절 |
| `Assets/Game/Balance/Items.csv` | `Mic`(MaxLevel 5) · `Kunai`(MaxLevel 1) |
| `Assets/Game/Balance/Evolutions.csv` | `Shuriken\|AttackSpeed` `5\|3` → `Kunai` · `Perk` 비움 |
| `Assets/Game/Balance/SceneWiring.csv` | `allItems` 30→31 (Mic) · `allEvolutions` 3→4 (Kunai). 🔴 **쿠나이는 `allItems` 에 안 넣는다** |

### 검증 로그 (플레이 모드 실측)

**① 마이크가 정말 좁은가** — 같은 거리에 **각도만 다른** 적을 놓았다.
거리가 대조군이다(*"사거리 밖이라 안 맞은 것"* 을 배제한다).

| 적 | 예상 | 실측 |
|---|---|---|
| A 0° / 2.9 (겨눔) | 맞음 | ✅ 맞음 (hp 70.0 → 51.9) |
| C 20° / 3.0 (원뿔 안) | 맞음 | ✅ 맞음 (hp 45.0 → 23.9) |
| B 50° / 3.0 (원뿔 밖) | 안 맞음 | ✅ 안 맞음 (45.0 → 45.0) |
| D 100° / 3.0 (뒤) | 안 맞음 | ✅ 안 맞음 (70.0 → 70.0) |
| E 0° / 5.5 (사거리 밖) | 안 맞음 | ✅ 안 맞음 (70.0 → 70.0) |

**5/5 예측 일치.** `state=Wave` · `timeScale=1` 을 같이 찍어 얼어붙은 값이 아님을 확인했다.

**② 쿠나이가 정말 꺾이는가** — 일부러 **한 직선에 놓지 않은** 적 4마리(0°/4 · 55°/5 · -50°/6 · 20°/8).
직선이면 관통만으로도 여럿을 맞혀 튕김과 구분이 안 된다.

| | 한 생애가 꺾인 횟수 | 맞은 적 |
|---|---|---|
| **쿠나이** | **2회** | **4/4** |
| 수리검 (대조군 · 관통 3 · 직선) | 0회 | 1/4 |

🔴 **첫 측정은 틀렸다** — 대조군 수리검도 *"6번 꺾였다"* 고 나왔다.
직선 발사체가 꺾을 리가 없다 — **풀 재사용**으로 같은 GameObject 의 **다음 생애 위치 점프**를
꺾임으로 센 것이었다. 한 프레임 이동 한계(`speed x dt x 4`)를 넘으면 새 생애로 보게 고친 뒤
다시 재서 **대조군이 0 으로 떨어졌고** 그제서야 숫자가 뜻을 가졌다.
대조군을 같이 재지 않았으면 **꾸며낸 숫자를 그대로 적을 뻔했다.**

**배선 확인** — `allItems` **31** (Mic 포함) · `allEvolutions` **4** (Kunai 포함) ·
🔴 **`Kunai` 가 `allItems` 에 없음 ✅** (진화 결과가 레벨업 카드로 뜨면 안 된다).
`m_Script` 5개 전부 정상 · 프리팹·아이콘 참조 모두 물림 · 콘솔 에러 0.

### 곁일로 잡은 것

🔴 **쿠나이 그림의 날이 45° 위를 향했다.** 발사체 스프라이트는 `ProjectileBase` 가
진행 방향으로 회전시키므로 **반드시 +X 를 향해야 한다** — 그대로 두면 날아갈 때 45° 틀어진다.
활이 `Bow.png`(아이콘) / `Arrow.png`(발사체)로 **갈라 둔 것과 같은 이유**라,
같은 방식으로 `KunaiProj.png` 를 따로 만들었다.

🟡 **마이크 이펙트는 검격 그림 재탕이다.** `SwingArcFx` 가 `Sprite[] frames` 를 30fps 로
돌리는 구조라 프레임 시트가 통째로 필요하다. 색만 바꿔 구분되게 두고
**CONTENT 요청-63** 으로 넘겼다 — `frames` 만 갈아 끼우면 코드 수정 없이 붙는다.

## 2-138. ✅ **아레나 경계 — 도망을 직선에서 고리로 바꾼다** (D111, 2026-09-08 132차)

> A + ① 로 진행 (넓은 아레나 + 단단한 벽)

### 왜 막는가

🔑 **카이팅의 모양을 바꾸려는 것이다.** 무한 맵에서 도망은 **직선**이다 —
한 방향으로 계속 달리면 다시는 그 자리로 안 돌아오므로 **못 걷는 것(건물)은 3초면 무의미해진다.**
막으면 도망이 **고리**가 된다. 벽에 닿으면 꺾어야 하고, 꺾으면 언젠가 지나온 자리로 돌아온다.

🔴 **제가 드린 숫자가 틀렸었다.** *"반지름 25~30 이면 한 바퀴 15~20초"* 라고 했는데
속도 4 기준으로는 **39~47초**다. 실제로 뜻이 있는 값은 **"직선으로 몇 초나 도망칠 수 있나"** 이고,
`halfExtent 24`(48×48)면 **가로지르는 데 12초**다.

### 설계 — 막는 자리는 셋뿐이다

| 무엇 | 어디 | 왜 |
|---|---|---|
| 플레이어 | `PlayerController.FixedUpdate` | 🔴 **속도가 아니라 위치를 막는다.** 속도를 0 으로 만들면 벽을 따라 미끄러지지 못해 조작이 답답해진다 |
| 적 | `EnemyBase.FixedUpdate` (early return **위**) | 🔴 **Ranged 가 진짜 이유다** — `PreferredRange` 를 지키려고 **뒤로 물러나서 벽 밖에서 쏜다.** 넉백 중에도 걸려야 해서 early return 위에 뒀다 |
| 소환 | `WaveManager.SpawnEnemyAt` | 🔑 **좌표를 만드는 곳은 넷인데 놓는 곳은 하나다.** 네 곳에 흩으면 다섯 번째가 생길 때 하나만 빠진다 |

🔴 **원이 아니라 사각형이다.** `CameraController` 가 이미 `Bounds`(사각형)로 카메라를 가두는
코드를 갖고 있었다 — 원으로 잡으면 **플레이어가 갈 수 있는 곳과 카메라가 갈 수 있는 곳의 모양이 달라진다.**
🔑 **카메라 클램프는 이미 있었다.** `useBounds` 가 꺼져 있었을 뿐이라 값만 넣고 켰다.

### 판정

| 무엇 | 결과 |
|---|---|
| 플레이어 | 중심+(80, −70) 밖으로 던졌는데 **정확히 벽(24.00)** 에 붙음 · `IsOutside=False` |
| 적 | **17마리를 35~90유닛 밖으로 던졌는데 밖에 남은 것 0** |
| 카메라 | `useBounds=True` · `bounds size = (48, 48, 1)` |
| 대조군 | 아레나가 없을 때 `Clamp` 이 좌표를 **그대로** 돌려주고 `IsOutside=False` |
| **화면** | `/screen-check` — **붉은 사각형이 그려진다** (위쪽 변과 좌우 모서리 확대 확인) |
| 콘솔 | 경고 **0** |

### 🔴 벽이 안 보였고, 원인이 둘이었다

**① URP 셰이더.** `LineRenderer` 에 빌트인 `Sprites/Default` 를 물렸는데
`shader.isSupported` 는 **true 를 돌려주면서** URP 2D 렌더러가 **그리지 않는다** —
오류도 분홍색도 없이 그냥 안 보인다.
🔑 **확실히 보이는 것들이 쓰는 셰이더를 읽어서 알았다** — 적·타일맵이 전부
`Universal Render Pipeline/2D/...` 였고 내 라인만 빌트인이었다.
(⇒ `Sprite-Unlit-Default` 로. **Lit 이 아닌 이유**는 경계선이 2D 조명에 어두워지면 안 되기 때문 —
이건 배경이 아니라 **알림**이다.)

**② 🔴 그런데 셰이더를 고친 뒤에도 안 보였고, 이번엔 내가 안 보이는 곳을 찍고 있었다.**
`Camera.orthographicSize` 를 직접 30 으로 넣었는데 `CameraController` 가 `_targetZoom`(10)으로
**되돌린다.** 화면은 `±17.2 × ±10` 만 보여 주는데 벽은 `±24` 라 **네 모서리 전부 화면 밖**이었다.

🔑 **모서리를 화면 좌표로 찍어 보고서야 갈렸다** — *"안 그려진다"* 와 *"화면에 없다"* 는 다르다.
`SetTargetZoom(30)` 으로 바꾸니 바로 보였다.

### 곁일 — 내가 만든 경고 2건을 지웠다

`FreezerBuilding.Version` · `SirenBuilding.Version` 이 `BuildingBase.Version` 을 가려
`CS0108` 경고가 둘 나 있었다 (`D108` 에서 넣은 것). `new` 를 붙여 **경고 0**.

---

## 2-137. ✅ **미끼 — 건물이 전장에 갈 필요 없이 전장이 건물에 온다** (D110, 2026-09-08 131차)

> 건물 추가 — 일정 시간마다 젠이 되는 어그로건물 (부셔진 모습 따로 추가)

### 왜 이게 다른 건물과 다른가

🔑 **이건 이 게임의 구조적 문제를 건드린다.** 뱀서라이크에서 플레이어의 유일한 방어 동사는
**"걸어서 벗어난다"** 인데, 건물은 **못 걷는 유일한 것**이다. 그래서 지어 놓고 3초 뒤에
사거리 밖으로 나가면 그걸로 끝이었다 — 건물이 퇴색된 진짜 이유다.

**미끼는 그 관계를 뒤집는다.** 적이 미끼로 오므로 **건물이 전장에 갈 필요가 없다.**

### 설계 — 새로 만든 것이 거의 없다

| 필요한 것 | 어떻게 |
|---|---|
| 적의 타겟 전환 | 🔑 **`ToPlayer()` 가 단일 진입점이었다** — 이동 6곳(Chaser·Ranged·Charger·Flanker·Swarmer·Blocker)이 전부 이 한 줄을 거친다. **여기만 바꾸니 여섯이 한꺼번에 따라왔다** |
| 부활 타이머 | 🔑 **`BuildingBase.CooldownActive` 가 이미 있었다** (식당이 힐템을 기다릴 때 쓰는 *"false 면 타이머를 얼린다"* 계약). ⇒ **부서져 있는 동안만 true** 로 두니 `AttackCooldown` 이 그대로 부활 시간이 됐다 |
| 적이 때리기 | `OnTriggerStay2D` 에 분기 하나 |

🔴 **`AimPointNow`(Blocker 의 예측 조준)는 일부러 안 바꿨다.** 그건 *"플레이어가 갈 길을 막는다"*
는 행동이라 **움직이는 대상에만 뜻이 있다.** 미끼는 가만히 있으므로 앞을 막을 것이 없다.

🔴 **부서진 동안에는 어그로가 풀린다.** 안 그러면 적들이 잔해 앞에 모여 아무것도 안 하고 서 있는다 —
플레이어에게는 *"적이 사라진"* 것처럼 보인다.

🔴 **미끼 피해는 플레이어와 계산이 다르다.** 플레이어는 **무적 시간**이 연타를 막아 `ContactDamage` 를
"한 방"으로 넣는데, 미끼는 무적이 없어 같은 식이면 **물리 프레임마다 한 방**이 들어간다
(예전에 플레이어 쪽에서 실제로 났던 문제다). ⇒ `fixedDeltaTime` 을 곱해 **초당 피해**로 넣는다.

### 판정

| 무엇 | 결과 |
|---|---|
| 타겟 전환 | 어그로 반경(5) 안 **미끼로 1 · 플레이어로 0** · **반경 밖 대조군 6마리는 플레이어로** |
| 파괴 | HP 120 → **0** · `IsBroken=True` · 어그로 즉시 해제(살아있는 미끼 **1 → 0**) |
| 부서진 모습 | `sprite` = **`Decoy_Broken`** |
| **부활** | 12초 뒤 `IsBroken=False` · `HP 120/120` · `sprite=Decoy` · 어그로 재등록 **4/4** |
| 대조군 | 미끼 0개일 때 `ResolveTarget` 이 **플레이어를 그대로** 돌려준다 |
| 배선 | `allItems` 29 → **30** · 프리팹 동작 `DecoyBuilding` |
| 콘솔 | 에러 **0** · 경고 **0** |

### 🔴 수치를 실측으로 다시 잡았다

첫 값 `HP 120` 은 **3.4초 만에 부서졌다.** 2초 버티는 미끼는 쓸모가 없다.

**초당 피해를 직접 쟀다** — 적 6마리를 미끼 둘레에 배치하니 **접촉 피해 합이 66/초**였고,
`120 / 66 = 1.8초` 라는 계산과 실측(3.4초, 일부가 흩어짐)이 맞았다.

⇒ **Lv1 을 530 으로** 잡았다 = 같은 상황에서 **약 8초**. 곡선은 `530 → 3400`.
🔑 **CSV 머리말에 "초 × 66" 이라는 되돌리는 법을 적어 뒀다** — 숫자만 보면 감이 안 온다.

⚠️ 후반에는 적 수와 `LayerScaling.DamageMult` 가 같이 올라 훨씬 빨리 깎인다.
**Lv5 3400 도 9층에서는 8초가 안 된다** — 그래서 곡선을 가파르게 잡았다.

### 그림 2장

`Decoy.png`(붉은 눈 토템 + 종) · `Decoy_Broken.png`(부러진 기둥 + 무너진 돌 + 넘어진 종).
`I-41` 가짜 투명 검사 통과(알파 0~255) · 유닛 크기 **0.500** 으로 대조군과 동일.

🔑 **`D109` 에서 놓친 자리를 이번엔 먼저 막았다** — 프리팹의 `SpriteRenderer.sprite` 를
갈아 끼우고, **저장된 파일을 다시 읽어** `sprite=Decoy` · `brokenSprite=Decoy_Broken` 을 확인했다.

---

## 2-136. ✅ **그림 3장 — 프리팹이 여전히 `Farm` 을 그리고 있었다** (D109, 2026-09-08 130차)

> 그림 만들고 한번에 푸시해

`D108`·`D109` 가 자리표시로 남긴 그림 3장을 `Unity AI(gpt-image-1-5)` 로 만들었다.

| 파일 | 무엇 |
|---|---|
| `Assets/Game/Sprites/Buildings/Freezer.png` | 석조 기단 + 청록 얼음 결정 |
| `Assets/Game/Sprites/Buildings/Siren.png` | 석조 기단 + 황동 나팔 + 호박색 등 |
| `Assets/Game/Sprites/Weapons/MachineGun.png` | 측면 기관총 + 탄띠 |

**임포트 설정을 지어내지 않았다** — `Turret.png` 의 값을 코드로 **읽어서 복사**했다 (`B7` 의 교훈).
`PPU 2048` · `Point` · `alphaIsTransparency` · 스프라이트 유닛 크기 **0.500 (대조군과 동일)**.

🔴 **`I-41` 가짜 투명부터 쟀다** — 셋 다 `alpha 0~255`, 투명 74~88 %로 **진짜 알파**였다.
🔑 **대조군 `Turret.png` 를 같이 재서** 측정 방법 자체가 맞는지 확인했다.

### 🔴 그런데 화면을 한 번 잘못 통과시켰다

아이콘 6곳(`BuildingData`·`WeaponData`·`ItemData`)이 새 그림을 가리키는 걸 확인하고,
화면에서 좌우로 두 건물이 보이길래 **통과로 적을 뻔했다.**

**마무리 점검에서 걸렸다:**

```
Building_Freezer color=흰색 sprite=Farm      ← 🔴
Building_Siren   color=흰색 sprite=Farm      ← 🔴
```

`Building_Farm.prefab` 을 복제해 만들었는데 **`SpriteRenderer.sprite` 를 안 갈았다.**
색 틴트만 바꿨을 뿐이다. 그리고 **런타임에 스프라이트를 갈아 끼우는 코드가 없다** —
기존 건물 5종은 전부 프리팹에 자기 그림이 박혀 있다(guid 5개가 서로 다른 것을 확인했다).

⇒ **내가 화면에서 "구분된다"고 본 것은 `Farm` 두 개에 색만 다른 것**이었다.
아이콘 배선이 맞아서 **HUD 는 제대로 보였고**, 그게 월드까지 맞다고 착각하게 만들었다.

🔑 **`BuildingData.Icon` 과 `SpriteRenderer.sprite` 는 서로 다른 자리다.**
아이콘만 확인하면 절반만 본 것이다.
...
| 무엇 | 결과 |
|---|---|
| 알파 | 셋 다 `0~255` · 투명 74~88 % · **대조군 `Turret` 도 같은 방식으로 정상** |
| 임포트 | `PPU 2048` · `Point` · 유닛 **0.500** — 대조군과 동일 |
| 아이콘 배선 | `BuildingData`·`WeaponData`·`ItemData` **6/6** 새 그림 |
| 프리팹 | `sprite` `Farm` → **`Freezer`/`Siren`** · 틴트 흰색 복귀 |
| **화면** | `/screen-check` — 왼쪽 **청록 결정 탑** · 오른쪽 **금색 나팔 탑** 으로 **서로 다르다** |
| HUD | 무기 줄에 기관총, 건물 줄에 냉각탑·사이렌 — 셋 다 구분됨 |

---

## 2-135. ✅ **기관총 — "부정확"은 산탄과 다르다** (D109, 2026-09-08 129차)

> 무기 추가 — 기관총 (공격 속도는 빠르지만 명중률은 낮음)

### 기존 무기와 다른 점이 하나뿐이었다

`ProjectileWeapon` 은 발사체가 여럿일 때 **정해진 각도로 부채꼴**을 편다
(`-spread…+spread` 균등 분배). 🔑 **그건 산탄이지 부정확이 아니다** — 같은 상황에서 늘 같은 곳으로 간다.

기관총은 **발사마다 독립 난수**로 흔든다. 두 발이 같은 곳으로 갈 수도, 정반대로 벌어질 수도 있다.
⇒ `ProjectileWeapon` 을 그대로 물려받고 `Fire()` 하나만 갈아끼웠다.

🔴 **"빠름"은 코드가 아니라 데이터다.** 연사는 `Weapons.csv` 의 `Cooldown`(0.16 → 0.10)으로 만든다.
클래스가 하는 일은 **흩뜨리는 것 하나**뿐이다.

🔵 **레벨이 오르면 정확해진다** (14° → 5°). 데미지만 올리면 *"빠른 총"* 이 그냥 *"센 총"* 이 되어
이 무기의 정체성인 **명중률과 화력의 맞바꿈**이 사라진다.

🔴 **기존 `Gun` 설명이 이미 `"Rapid low damage fire."` 였다.** 연사만으로는 두 무기가 안 갈린다 —
그래서 설명을 *"많이 빗나간다"* 쪽으로 잡았다.

### 변경한 파일

| 파일 | 변경 |
|---|---|
| `Assets/Scripts/Weapon/MachineGunWeapon.cs` | 신설 — `ProjectileWeapon` 상속 · `Fire()` 만 재정의 |
| `Assets/Prefabs/Weapon_MachineGun.prefab` | 신설 (🔵 `Weapon_Sword.prefab` 은 이름과 달리 **투사체 무기 공용 껍데기**다 — Bow·Gun·Shuriken 이 같이 쓴다) |
| `Assets/Game/Balance/{Weapons,Items,SceneWiring}.csv` | 행 1개씩 |

### 판정

탄의 각도를 **표적 방향 기준**으로 쟀다 (표본 3발):

```
-11.6°   -10.7°   -6.2°      전부 다르고 전부 ±14 안
```

🔑 **이 판정의 핵심은 "0 이 아니다"가 아니라 "서로 다르다"이다.**
부채꼴(`ProjectileWeapon`)이라면 `ProjectileCount(Lv1) = 1` 이므로 `spread = 0` 이 되어
**정확히 `0.0°` 만 나와야 한다.** 값이 매번 다르다는 것이 무작위의 증거다.

| | 결과 |
|---|---|
| 배선 | `allItems` 28 → **29** · 프리팹 동작 `MachineGunWeapon` |
| DPS (명중 100 % 가정) | Lv1 **18.8** vs Gun 11.1 · Lv5 **180.0** vs Gun 172.7 |
| 컴파일 | `Version` 반영 확인 |
| 콘솔 | 에러 **0** |

DPS 가 Gun 보다 높게 잡힌 건 **명중률 손해를 미리 감안한 것**이다 — 실제로는 빗나가는 만큼 깎인다.

### 🔴 방향을 잘못 재고 있었다

처음엔 `Rigidbody2D.linearVelocity` 로 탄의 방향을 쟀는데 **전부 0** 이었다.
`ProjectileBase` 는 물리를 안 쓰고 `transform.position +=` 로 밀며 `_direction` 은 `private` 이다.

🔑 **단서는 대조군이었다** — *"활성 투사체 2 · 속도 있는 것 0"*.
투사체가 **있는데** 속도가 0 이면 속도가 방향의 근거가 아니라는 뜻이다.
⇒ **발사점(플레이어)에서의 변위**로 바꿔 잡았다.

그리고 측정이 `LevelUp`(`timeScale 0`)에 계속 오염돼서,
**재기 직전에 `Time.timeScale = 0` 을 먼저 걸고 그 순간을 재는** 방식으로 바꿨다.
`D108` 에서 세 번 당한 것과 같은 문제를 구조적으로 막은 것이다.

⚠️ 콘솔 경고 4건은 **내가 적을 `bodyType = Static` 으로 고정한 시험 조작** 때문이다
(`Die`/`ForceDespawn` 이 `linearVelocity` 를 쓴다). 게임 코드 결함이 아니다.

---

## 2-134. ✅ **건물 2종 — 필요한 시스템이 이미 다 있었다** (D108, 2026-09-08 128차)

> 건물 추가 — 일정시간 마다 적들을 멈춤 / 주변 반경 적들 슬로우

사용자가 건물 4종·무기 3종·패시브 4종을 한 번에 요청했고, **싼 것부터** 가기로 해서
이 둘을 먼저 넣었다. 고르는 기준은 *"새 시스템이 필요 없는 것"* 이었다.

### 왜 이 둘이 쌌나

🔑 **`EnemyBase.ApplySlow(mult, duration)` 이 이미 있었다** — 독 장판(`ToxinField`)이 쓴다.
게다가 계약이 *"매 틱 다시 걸어 주면 유지, 안 걸어 주면 저절로 풀림"* 이라
**적이 범위를 벗어났는지 아무도 추적할 필요가 없다.** 오라 건물에 딱 맞는 모양이었다.

⇒ 두 건물 다 **`OnCooldownElapsed` 를 갈아끼운 작은 클래스 하나씩**이면 됐다.

### 🔴 설계 중에 버그를 하나 피했다

스턴은 `ApplySlow(0f, …)` 로 만드는 게 제일 싼 길이다. **그런데 값을 따라가 보니 영구 정지가 된다.**

```
ApplySlow: if (Time.time > _slowUntil || mult < _slowMult) _slowMult = mult;
           _slowUntil = Max(_slowUntil, Time.time + duration);
```

`_slowUntil` 이 **살아 있는 동안에는 더 약한 값으로 못 덮는다.** 그런데 냉각탑은
펄스 사이에 틈이 안 생기게 **지속시간을 주기보다 길게**(1.35 배) 건다 ⇒ `_slowUntil` 이 끊기지 않는다.
⇒ **냉각탑 옆에서 사이렌이 한 번 울리면 `_slowMult` 가 0 에 갇혀 그 적은 다시는 안 움직인다.**

그래서 `ApplyStun`/`IsStunned` 를 **슬로우와 완전히 분리**했다.
`_stunUntil` 은 `ApplyStun` 과 풀 리셋 **두 곳에서만** 쓰이고 `ApplySlow` 는 건드리지 않는다 —
**경로가 구조적으로 없다.**

🔑 의미상으로도 다르다. 슬로우는 *느려지는 것*, 스턴은 *아무것도 못 하는 것*이라
**발사까지 막았다**(`FireProjectile` 한 줄). 안 그러면 *"멈췄는데 총알은 날아온다"* 가 된다.

### 변경한 파일

| 파일 | 변경 |
|---|---|
| `Assets/Scripts/Building/FreezerBuilding.cs` | 신설 — 반경 전체에 `ApplySlow` |
| `Assets/Scripts/Building/SirenBuilding.cs` | 신설 — 반경 전체에 `ApplyStun` · 주기 대비 60 % 상한 |
| `Assets/Scripts/Building/BuildingBase.cs` | `FindEnemiesInRange()` 공용 헬퍼 (`Version` 1→2) |
| `Assets/Scripts/Enemy/EnemyBase.cs` | `ApplyStun`·`IsStunned` 신설 · `CurrentSpeed` 에 반영 · 발사 차단 (`Version` 2→3) |
| `Assets/Prefabs/Building_{Freezer,Siren}.prefab` | 신설 (`Building_Farm` 복제 후 동작 교체 + 색 자리표시) |
| `Assets/Game/Balance/{Buildings,Items,SceneWiring}.csv` | 행 2개씩 · **`Output` 열 규칙을 머리말에 명시** |

🔴 **`Output` 열의 뜻이 건물마다 달라졌다** — 예전엔 *"산출량"* 하나였는데 이제 셋이다
(산출량 · 감속률 · 정지 시간). CSV 머리말에 적어 두지 않으면 다음 사람이 엉뚱한 값을 넣는다.

### 판정

| 무엇 | 결과 |
|---|---|
| **냉각탑 감속** | **같은 개체** 1.600 → 1.200 · 실측 속도비 **정확히 0.750** (기대 0.75) |
| **사이렌 정지** | `v = 0.000` · `IsStunned=True` · **`Time.time` 이 흐르는 중** |
| **스턴 해제** | `IsStunned=False` · `v = 1.600` — 다시 움직인다 |
| 배선 | `allItems` **26 → 28** (2/2 포함) · 프리팹 동작 `FreezerBuilding`/`SirenBuilding` |
| 컴파일 | `Version` 4/4 반영 |
| **화면** | `/screen-check` — 청록(냉각탑)·주황(사이렌) 둘 다 그려짐 · HUD 건물 줄 2칸 참 |

🖼️ **`/screen-check` 의 경로 A(데스크톱 캡처)를 처음으로 실제로 썼다** — `D88` 이 스킬을 만든 뒤
*"실캡처 0회"* 로 남아 있던 항목이다. 권한 tier 는 **`full`**(스킬 A-1 의 "미확인" 칸을 채웠다).
`scaleFactor` 를 런타임에서 읽으니 **0.4458** — 스킬이 적어 둔 `Expand` 실측값과 일치했다.
...
| 무엇 | 증상 | 진짜 원인 |
|---|---|---|
| ① | 냉각탑 안인데 속도가 그대로 | `state=LevelUp` · `timeScale=0` — 건물은 `Wave` 에서만 돈다 |
| ② | `IsStunned=True` 인데 `v=1.200` | `state=GameOver` — **내가 피험체를 플레이어 옆에 붙여 놔 접촉사시켰다.** `Time.time` 이 멈춰 스턴이 안 풀리고 속도도 얼었다 |
| ③ | 피험체가 44 유닛 밖에 있다 | **풀 재사용** — 내가 붙인 `SUBJECT` 이름이 재활용된 새 개체에 남아 있었다 |

⇒ **매 측정에 `state`·`timeScale`·`Time.time` 을 같이 찍고서야 갈렸다.**
`Time.time` 이 흐르는지를 보는 게 결정적이었다 — 얼어붙은 값과 진짜 값이 구분된다.

🔴 **그리고 Import 가 씬을 `isDirty` 로 남겼다.** `git status` 에 `SampleScene.unity` 가 없어
확인했더니 `isDirty=True` 였다 — **저장 안 했으면 다음 실행에 새 건물이 사라졌을 것이다.**
`D101`·`D103` 이 걸린 것과 같은 함정이다.

### 🔴 번호를 한 번 잘못 썼다

작업 내내 `D106` 으로 주석을 달았는데, **병렬 DEV 세션이 그 사이 `D106`·`D107` 을 이미 썼다.**
커밋 직전 `BOARD.md` §3 이 `D108` 을 가리키는 걸 보고 알았다 — 전부 `D108` 로 고쳤다.
⇒ **번호는 착수 시점에 보드에서 선점해야 한다.** 나는 코드를 먼저 쓰고 나중에 보러 갔다.

---

## 2-133. ✅ **직업 초상 10장 + 원형 액자** (D107, 2026-09-07 127차)

**한 줄:** 프로필 자리가 **전신 스프라이트를 40px 에 욱여넣어** 얼굴이 몇 픽셀이었다. 흉상 10장을 만들었다.

### 왜 필요했나

`Portrait` 는 **인게임 전신 그림**(`Sprites/Classes/*.png`)을 그대로 가리키고 있었다.
표시 자리는 **88×88 캔버스px = 화면 40×45px** 인데 거기에 전신을 넣으니
**얼굴이 대여섯 픽셀**이었고, 테두리도 없어 캐릭터가 바닥에 그냥 서 있는 모양이었다.

### 만든 것

| | |
|---|---|
| `Assets/Game/Sprites/Classes/Portraits/*.png` 🆕 | 픽셀아트 흉상 **10종** (Unity AI · `gemini-3.1-flash`) |
| `Classes.csv` `Portrait` 열 | 10줄 → 새 경로 |
| `HUDManager.BuildPortraitFrame()` 🆕 | 뒷판 원 · 원형 마스크 · 금색 고리. **애셋 0 · 씬 변경 0** |

🔑 **참조 이미지를 물려 뽑았다.** 안 그러면 인게임 캐릭터와 딴 사람이 나온다 —
실제로 `Sentinel` 이 콘텐츠 정책에 두 번 막혀 참조 없이 뽑혔고
**올리브빛 군인이 금장 두른 은빛 기사**가 됐다.

🔴 **그리고 글로만 묘사한 두 번째 시도도 부족했다.** 색만 맞췄지
원본의 **초록 발광 눈**과 **놋쇠 탄띠**가 빠져 있었다 — 원본을 **확대해서 제대로 안 봤기 때문**이다.
세 번째에 참조 이미지를 다시 걸어 보니 **그냥 통과했다**(앞의 차단은 일시적이었다).
⇒ **막히면 포기하고 글로 때우지 말고, 한 번 더 시도해 볼 것.**
지금은 **10/10 이 원본과 같은 사람**임을 나란히 놓고 확인했다.

> 🔑 **교체는 파일 내용만 갈아끼웠다.** 지우고 새로 넣으면 `.meta` 의 GUID 가 바뀌어
> **SO 의 `Portrait` 참조가 끊긴다.** `.meta` 를 남기고 PNG 만 덮어썼고,
> `Sentinel SO -> Portraits/Sentinel.png` 로 참조가 살아 있는 것을 확인했다.

🔑 **액자는 코드가 만든다** (`D66` 과 같은 방식). 원과 고리를 런타임에 그려 쓴다 —
씬에 칸을 하나 더 두면 그 칸이 언젠가 비거나(`I-24`) 위치가 어긋난다.

### 🔴 기록에 있던 함정이 셋 다 그대로 났다

| 함정 | 이번에 어떻게 났나 |
|---|---|
| `I-41` 가짜 투명 | 프롬프트에 *"real alpha, not a checkerboard"* 라고 **명시했는데도 10장 전부** 알파 채널 없이(`RGB`) 체커가 RGB 에 그려져 나왔다. `RemoveSpriteBackground`(Photoroom)로 전부 뚫었다 — **10/10 `alpha(0,255)`** |
| `B7` 임포트 설정 | 새 파일이 **Bilinear + 압축 켜짐**으로 들어와 픽셀아트가 뭉개진다. Point · 무압축 · 512 로 맞췄고 **플랫폼 오버라이드까지** 고쳤다(기본값만 고치면 빌드에서 압축이 되살아난다) |
| `D24` 직렬화 확인 | 런타임 `10/10` 만 보지 않고 **`.asset` 10개가 디스크에서 바뀐 것**을 `git status` 로 확인했다 |

### 🔴 액자를 한 번 크게 틀렸다

액자를 **부모에 맞춰 늘렸다.** 그런데 부모 `PortraitGroup` 은 **370×96** 으로
**HP 바까지 아우르는 칸**이라, 원이 HP 바를 덮는 **거대한 타원**이 됐다.
초상 자리는 그 안의 **88×88** 이었다 — 그 앵커·크기를 베끼도록 고쳤고 네 조각 전부 88×88 확인.
⇒ **"부모에 맞춰 늘린다"는 부모가 무엇인지 보고 나서 할 말이다.** 주석으로 박아 뒀다.

### 🟡 그리고 헛짚은 게 둘

- **빈 원으로 보여 결함인 줄 알았다** — `UpdatePortrait` 가 아직 안 돈 프레임에서 찍은 것이었다
  (기록 #122). 마스크만 껐다 켜는 **대조군**을 세우니 둘 다 정상이었다
- **`Warrior` 가 알파 검사에서 실패로 떴다** — *"모서리 4곳이 전부 투명"* 조건을 걸었는데
  이 구도는 **어깨가 아래끝까지 닿는다.** 자홍색 위에 합성해 보고 정상 확인

### 🔴 "어둡다" 는 지적 — **원인이 내 생각과 달랐다**

사용자: *"초상화가 좀 어두운데"*. 처음엔 *"뒷판(0.077)이 게임 바닥(0.234)보다 어두워
구멍처럼 보인다"* 로 보고 **뒷판을 0.20 으로** 올렸다. 재 보니 **1.07배**밖에 안 올랐다.

🔑 **진짜 문제는 뒷판과 초상이 거의 같은 밝기였던 것**이다(0.20 vs 평균 0.23).
원 전체가 한 덩어리로 뭉쳤다. ⇒ **뒷판은 "안 튀게" 가 아니라 "초상이 떠 보이게" 잡는 값이다.**
초상 대부분이 어두운 갑옷·로브라 **밝게 깔아야 실루엣이 뜬다.**

**값은 계산이 아니라 렌더해서 골랐다** — 0.20 / 0.34 / 0.48 / 0.62 를
**가장 어두운 쪽(Warrior)과 가장 밝은 쪽(Summoner) 양쪽에서** 보고 **0.48** 로 정했다
(0.62 는 금색 고리와 경쟁해 씻긴다). 초상은 **1.28배 확대**해 원을 채웠다
(1.0 이면 원 안을 41~71 % 만 채워 나머지가 뒷판이다).

### 🔴 그런데 **그것도 진짜 원인이 아니었다** — 사용자가 *"더 밝게"* 라고 했다

뒷판을 두 번 만지작거렸는데 눈금이 조금밖에 안 움직였다. 원본 PNG 를 게임 바닥 위에
합성해 보니 **투구가 밝은 은색**인데 게임에서는 어두운 남색이었다. 그림이 어두운 게 아니었다.

```
PortraitImage.color = RGBA(0.250, 0.280, 0.340, 1.000)
```

🔴 **`Image.color` 는 곱셈이다** (`D56` 에서 겪은 그것). 어두운 남색 색조가 걸려 있어
**밝은 은색 투구가 25 % 로 깎이고 있었다.** 흰색으로 바꾸니 한 번에 뛰었다.

| 단계 | 원 안 평균 휘도 |
|---|---|
| 액자 최초 (뒷판 0.077 · 색조 걸림) | 0.180 |
| 뒷판 0.20 | 0.193 |
| 뒷판 0.48 | 0.215 |
| **색조 흰색 + 뒷판 0.34** | **0.332** (바닥 0.234) |

**뒷판은 0.10~0.48 전 구간을 굴려도 0.316~0.343 밖에 안 움직인다.**
색조 하나가 나머지 전부보다 컸다.

🔑 **그리고 색조를 고치니 정답이 뒤집혔다.** 초상이 밝아지자 **어두운 뒷판**이 나아졌다 —
`Warrior`(밝은 은색)는 어두운 뒷판에서, `Assassin`(어두운 후드)은 밝은 뒷판에서 산다.
양쪽을 렌더해서 **0.34** 로 정했다(0.48 에서 내렸다).

> 🔴 **`D56` 을 겪고도 못 봤다.** 그때는 *"실루엣을 밝게"* 하려고 `Image.color` 를 올렸다가
> 곱셈이라 안 됐고, 이번엔 반대로 **이미 걸려 있는 어두운 색조를 못 보고** 뒷판만 만졌다.
> ⇒ **"어둡다" 는 지적을 받으면 배경보다 `Image.color` 를 먼저 볼 것.**

> 🔴 **숫자만 봤으면 0.48 을 버렸을 것이다.** 내가 잰 Summoner 휘도 0.448 은
> **스프라이트 전체 평균**이라 "0.48 뒷판이면 대비가 없다"고 나왔다. 그런데 마스크가 자르고
> 1.28배 확대까지 하면 **실제로 보이는 부분은 훨씬 어둡다.** 렌더가 맞고 계산이 틀렸다.

> 🔴 **`B20` 의 함정을 그대로 다시 밟았다.** `portraitBackColor` 코드 기본값을 0.48 로 바꿨는데
> 런타임은 계속 **0.20** 이었다 — **도메인 리로드가 이미 역직렬화된 인스턴스 값을 문다.**
> `topHudBand` 에서 겪고 **문서에 적어 놓고도** 또 밟았다. `SerializedObject` 로 씬에 박아 풀었다.

> 🟡 그리고 **빈 원을 재고 "1.28배 밝아졌다"고 보고할 뻔했다** — `UpdatePortrait` 가 다음 프레임에
> 도는데 같은 호출에서 찍었다(기록 #122). 이제 찍기 전에 **`sprite` 가 들어있는지 확인**한다.

⚠️ **CONTENT 소유 경로를 건드렸다** (`Sprites/` · `Classes.csv`) — 인물은 절차적 생성이 안 돼
DEV 가 만드는 게 `D42` 전례다 → [`REQ/CONTENT.md`](Parallel/REQ/CONTENT.md) **요청-62**

---

## 2-132. ✅ **화살표를 잉크로 쟀다 — 크기는 통과, 대비가 미달** (D106, 2026-09-07 126차)

**한 줄:** `D86` 이 *"96px → 43.5px, 통과"* 라고 적은 건 **사각형 크기**였다. 칠해진 픽셀로 다시 쟀다.

### 왜 다시 쟀나

`D88` 스킬 평가에서 독립 에이전트가 짚었다 — `BossArrowGraphic` 의 정점 4개는
삼각형 둘을 만드는데 그 면적이 `2 × 0.6·hx·hy = 1.2·hx·hy` 이고 사각형은 `4·hx·hy` 다.
⇒ **사각형의 30 % 만 칠한다.** 크기로 통과선을 재면 **실제보다 후하게 나온다.**

### 측정 — 계산과 실측이 0.9 % 안에서 맞았다

상태를 얼리고(`timeScale 0` · 물리 정지 · `ChangeState(Wave)`) 보스를 22유닛 밖으로 보낸 뒤,
**화살표만 끈 그림을 대조군으로** 찍어 차분했다.

| | 값 |
|---|---|
| `scaleFactor` (**런타임에서 읽음**) | **0.4458** ← `Expand` 값. 옛 계산 0.4532 는 틀렸다 |
| 사각형 | 캔버스 96×74.9 → 화면 **42.8×33.4** |
| 칠하는 비율 | **30.0 %** (식과 일치) |
| **잉크 (실측)** | **425 px** · 등가 정사각형 한 변 **20.6px** |
| 예측값 | 429 px · 20.7px — **실측과 0.9 % 차** |
| **대비** | 화살표 L 0.420 / 배경 L 0.234 ⇒ **1.66 : 1** |

### 판정 — **크기는 통과, 대비가 미달**

- 등가 한 변 **20.6px** vs 통과선 20 ⇒ 통과. 다만 **여유가 거의 없다**
- 🔴 **대비 1.66 : 1** — UI 요소 권장선 **3 : 1** 에 크게 못 미친다.
  원인은 색이다: `(1, 0.28, 0.22)` 는 붉은색이라 **휘도 기여가 낮고**(적색 계수 0.2126)
  알파도 **0.853** 이다. 어두운 바닥 위에서 밝기 차가 안 난다
  ⇒ 크기를 더 키우기보다 **색을 밝히는 쪽**이 값이 크다 → [`TUNING.md`](TUNING.md)

> 🔑 **`D104` 가 진짜 원인이었다.** *"안 보인다"* 가 세 번 나온 건 `CanvasRenderer` 가 없어
> **한 픽셀도 안 그려졌기** 때문이고 그건 이미 닫혔다. 이번 측정은 **그 뒤의 상태**를 잰 것이다.

### ✅ `B20` 도 같이 닫았다 — 위쪽만 안으로 민다

`topHudBand` 신설. **위로 갈 때만** 테두리 한계를 다르게 준다.
*"언제 뜨는가"(화면 안 판정)는 안 건드렸다* — 그건 이 버그 밖이다.

🔑 **여유를 `arrowSize` 에서 계산한다** — 상수로 박으면 크기를 바꿀 때 조용히 다시 겹친다
(이미 44 → 96 으로 한 번 바뀌었다). 회전한 사각형의 화면 AABB 는 **대각선을 못 넘으므로**
그 절반이 최악의 반높이다.

**판정: 글리프 겹침 0/12** (수정 전 5).

#### 🔴 값을 정하는 데 두 번 틀렸다

| 시도 | 값 | 왜 틀렸나 |
|---|---|---|
| 1차 | 100 | **보스 없는 상태에서 쟀다.** 화살표는 보스가 있을 때만 뜨고 그때 `BossNameText` 가 붙는다 |
| 2차 | 212 | 글자의 `RectTransform` 으로 쟀다 — **rect 는 넓은데 글자는 왼쪽에만 있다.** 12방향 중 하나가 헛경보 |
| 3차 | **210** ✅ | `ForceMeshUpdate` + `characterInfo[i].isVisible` 로 **실제 글리프** 외곽 (실측 206 + 여유) |

> 🔴 **2차를 고쳤는데 화면이 안 바뀌어 한참 헤맸다.** 코드 기본값을 212 로 올렸는데
> 런타임은 계속 **100** 이었다 — **도메인 리로드가 이미 역직렬화된 인스턴스 값을 물고 있어서**
> 필드 초기값을 바꿔도 안 따라온다. `SerializedObject.FindProperty` 로 씬에 명시적으로 박고
> 저장해서 풀었다(`git diff` 에 `topHudBand: 210` **한 줄**).
> ⇒ **필드 기본값을 바꾼 것은 "값이 바뀐 것"이 아니다.** 런타임에서 읽어 확인할 것.

### ✅ `B21` 도 닫았다 — **피하지 말고 갈라 보이게**

`B20` 을 고치고 화면을 보니 화살표가 **보스 체력 바에 묻혀** 있었다(둘 다 빨강).
`B20` 판정이 **글자만** 보고 `Image` 는 안 봤기 때문이다 — **숫자로는 0/12 통과였다.**

띠를 더 내리는 길은 **버렸다.** 같이 재 보니 왼쪽 아이템 칸 `Frame` 이 **356** 까지
내려와서, 그것까지 피하면 화살표가 **화면 1/3 지점**에 앉아 "테두리 표시"가 아니게 된다.
⇒ **피하는 대신 갈라 보이게 했다 — 어두운 외곽선.**

🔑 **색은 안 바꿨다.** 그건 CONTENT 판단이고(`요청-61`) 붉은색이 지고 있는
*"위험"* 이라는 뜻도 남는다. 외곽선은 팔레트가 아니라 **윤곽**이다.

🔑 **오브젝트를 안 늘렸다** — 같은 메시에서 **나중 삼각형이 위에 그려지므로**
외곽선을 먼저 넣으면 뒤에 깔린다. 정점 8 · 삼각형 4 · 새 `GameObject` 0.

🔴 **`B20` 이 다시 열리지 않게 막았다** — 외곽선은 `rect` 밖으로 나가므로
`OutlineScale()` 을 공개해 `B20` 의 여유 계산에 곱했다.
**`B20` 에서 배운 걸 그대로 적용한 것이다**(상수로 박으면 두께를 바꿀 때 조용히 깨진다).

| 판정 (대조군 = 외곽선만 끈 같은 화면) | 없음 | 외곽선 6 |
|---|---|---|
| **바 위 평균 색차** | **39** /765 | **76** /765 (1.9배) |
| 잉크 | 428px | **576px** (등가 한 변 20.6 → **24.0**) |

`B20` 회귀 **0/12** 유지. 🟢 곁효과로 통과선 20px 여유가 커졌다 —
다만 대비 1.66:1 자체는 그대로라 `요청-61`(색 판단)은 **여전히 열려 있다.**

### 🔴 지적 밖에서 결함을 찾았다 → [`BUGS.md` `B20`](Parallel/BUGS.md)

캡처를 확대하니 화살표가 `0 G` · `0 Kills` 위에 얹혀 있었다.
12방향을 돌려 보니 **5방향(위쪽 절반)에서 HUD 글자와 겹친다** — 최대 566px².
HUD 글자 여섯이 전부 화면 위쪽 띠에 있고 화살표는 테두리를 타기 때문이다.

> 🔴 **겹침을 재는 데 한 번 실패했다.** `GetWorldCorners` 에서 모서리 **2개**만 써서
> 화면 AABB 를 만들었는데 화살표는 **회전한 사각형**이라 폭이 절반으로 나왔다(19×51 / 실제 54×50).
> **잉크 bbox 실측(39×32)과 안 맞아서 잡혔다** — 픽셀 측정이 대조군 노릇을 했다.
> ⇒ **회전하는 것은 모서리 4개를 다 본다.**

---

## 2-131. ✅ **보스 체력 바가 일시정지 화면을 뚫었다 · 묻혀 있던 요청 2건** (D105, 2026-09-06 125차)

> 보스 체력바도 ESC 눌렀을때 맨위에 보이는데 그것도 같이 해결하고

### ① 🔴 보스 체력 바 — **사용자가 본 건 `ESC` 지만 레벨업에서도 났다**

`UI Canvas` 의 형제 순서를 재 봤더니 원인이 한 줄로 나왔다 (아래일수록 위에 그려진다):

```
[7]  LevelUpPanel
[8]  PausePanel
[9]  OptionSubPanel
[11] BossHealthBar     ← 둘 다 뚫는다
```

🔑 **사용자는 `ESC` 에서 봤지만 같은 원인으로 레벨업 화면도 뚫린다.**
보스전 중 레벨업은 자주 나는 상황이라 지적이 늦게 나왔을 뿐이다.

🔑 **형제 순서를 바꾸지 않았다.** `PausePanel` 을 `BossHealthBar` 위로 올리면
**그 위에 있어야 하는 `OptionSubPanel`[9] 이 뒤로 밀려 옵션 창이 가려진다.**
순서는 이미 짝이 맞아 있어서, 하나를 옮기면 다른 짝이 깨진다.
⇒ `BossOffscreenArrowUI`(D104)와 **같은 방식**으로 상태를 본다 —
`GameState.Wave` 가 아니면 내린다.

🔴 **`Hide()` 를 쓰면 안 됐다.** 거기서는 `_boss = null` 을 해서 **영구히** 내린다 —
일시정지를 풀어도 안 돌아온다. **보이기만** 껐다.

### ② `MAX` 도장이 `Lv.2 / 2` 를 덮었다 (`요청-42` ①-3)

🔴 **CONTENT 의 C안을 그대로 못 썼다.** 제안 좌표(141~171)가
`DescText`(152~228)와 **19px 겹친다** — `LevelText` 와 `DescText` 사이 틈이 **11px 뿐**이라
30px 짜리가 안 들어간다. 카드를 다 재고서야 보였다:
...
| DescText | 152~228 |
| PriceText | 228~260 |
| **BuyButton** | **264~312** ← 다 키우면 **눌리지 않는다** |

⇒ **죽어 있는 버튼 자리**로 내렸다 (`anchor` 중앙→위 · 높이 70→48 · `y = -288`).

🔴 **그랬더니 새 문제가 생겼고 재서 잡았다** — `PriceText` 도 `maxed` 면 `"MAX"` 라
**두 `MAX` 가 1.8px 간격**으로 붙어 한 단어가 겹쳐 찍힌 것처럼 보였다.
⇒ 상태는 도장이 말하므로 **가격 칸을 비웠다**(낼 값이 없다).

### ③ 리롤 로그 (`요청-42` ②)

`_currentRerollCost` 를 **올린 뒤** 찍는데 문구가 `비용 누계` 였다.
🔑 **값이 아니라 이름이 틀렸다** — 이미 `D96` 을 낚아 그 판 수입 추정을 **+28 %** 어긋냈다.
이름만 고치면 *"낸 값"* 은 여전히 로그에 없으므로 **둘 다** 찍게 했다.

### 변경한 파일

| 파일 | 변경 |
|---|---|
| `Assets/Scripts/UI/BossHealthBarUI.cs` | `Update` 에서 `CurrentState != Wave` 면 `panel` 만 끈다 · `Version = 1` |
| `Assets/Prefabs/Prefab_UpgradeCard.prefab` | `MaxedText` 앵커 중앙→위 · 240x70 → **240x48** · `y 0 → -288` |
| `Assets/Scripts/UI/UpgradeCardUI.cs` | `maxed` 면 `priceText` 를 **빈 문자열**로 |
| `Assets/Scripts/Shop/ShopManager.cs` | 올리기 전 값을 `paid` 로 잡아 `낸 값 · 다음 리롤` 둘 다 찍는다 |
...
|---|---|
| 전투 중 (`Wave`) | **True** |
| 일시정지 (`Paused`) | **False** ← 대조군 `PausePanel` 은 True |
| 재개 (`Wave`) | **True** · `materialCount 1` (보스 살아 있음 확인) |
| 레벨업 (`LevelUp`) | **False** |

형제 순서는 `BossHealthBar[11]` / `PausePanel[8]` **그대로**다.

**카드 — 보이는 것끼리 겹치는 쌍 0**

```
── MAX 카드 ──                        ── 대조군 (안 키운 카드) ──
LevelText  244.1~255.7  보임=True     LevelText  244.1~255.7  보임=True
DescText   205.3~239.2  보임=True     DescText   205.3~239.2  보임=True
PriceText  191.0~205.3  보임=False    PriceText  191.0~205.3  보임=True  '440 G'
MaxedText  167.9~189.3  보임=True     MaxedText  167.9~189.3  보임=False
```

🔑 **대조군의 세 요소 좌표가 완전히 동일하다** — 안 키운 카드의 배치가 **안 움직였다**.

**리롤** — `리롤 (낸 값: 25G · 다음 리롤: 40G)` → `(낸 값: 40G · 다음 리롤: 55G)` ·
골드 **500 → 475 → 435** (−25, −40). 콘솔 에러 **0** · 경고 **0**.

🔑 **①-3 은 제안을 그대로 따랐으면 틀렸을 것이다** — 좌표가 적혀 있어도
**대상을 다시 재야 한다.** 여기서는 11px 틈에 30px 을 넣으라는 제안이었다.

### ④ 마무리 문서 갱신 — **또 "제목만 낡은 것"이 셋 있었다**

`D104` 가 `BUGS.md`·`BOARD.md` 에서 잡은 것과 **같은 병**이 `ROADMAP.md` 에도 있었다 —
본문은 ✅ 인데 **절 제목만 "없다/미구현"** 으로 남아 있다. 목차만 훑으면 미완으로 읽힌다.

| 어디 | 제목이 말하던 것 | 실제 |
|---|---|---|
| `ROADMAP` §3-3 | *"없는 화면 ★★"* | **다 생겼다.** 남은 둘(키 바인딩·튜토리얼)은 *"안 만든다"* 는 **결정**이다 |
| `ROADMAP` §3-4 | *"게임패드가 사실상 미구현 ★★"* | **안 하기로 한 것**이다 (사용자 결정 2026-09-04) |
| `ROADMAP` §5 | *"자료구조만 있고 게임에 안 붙어 있다 ★★"* | 본문 첫 줄이 *"✅ D30 — 고리가 닫혔다"* 다. 남은 건 **직업 해금 흐름 하나** |
| `TODO` 머리말 | *"남은 가장 큰 항목은 **메타 강화 화면**"* | `D30`(2026-09-03)에 끝났다. `ROADMAP` §3-3 은 그때 고쳤는데 **이 줄만 안 고쳤다** |

🔴 **§5 의 "남은 건 해금 흐름 하나" 는 코드로 확인하고 썼다** —
`ClassSelectUI.IsUnlocked` 가 아직 `cls.UnlockedByDefault` 만 보고
`MetaProgressionManager.UnlockCharacter` 를 **아무도 안 부른다**. 게임패드도 `PlayerInput` 0곳.
⇒ **제목만 고쳤고 내용은 안 지어냈다.**

같이 맞춘 숫자 — 체감 검증 **115개 중 3개 → 121개 중 5개**(가중 합 68 % 는 그대로. 그 축의
가중치가 20 % 라 3→4 로는 안 움직인다) · `TODO` §1 **47 → 45개 미확인**(②③ 을 닫은 뒤).

### 마감 시점 상태

| | |
|---|---:|
| 열린 버그 | **`B8` 하나** (절반 수정 — 겹침은 밀도 문제) |
| CONTENT 열린 요청 | **0** |
| `BOARD` §1 의 `완료` 행 | **0** |
| 문서 링크 깨짐 | **0** |
| `TODO` §1 동작 미검증 | 45 / 18 확인 |
| `TUNING` 체감 | **121개 중 5개** |

🔑 **코드로 더 할 일이 남아서가 아니라 손이 안 탄 채로 끝났다.**

---

## 2-130. ✅ **마감 정리 — 표 세 개가 조용히 낡아 있었다** (D104, 2026-09-06 124차)

> 너도 이제 마무리로 모든 문서 갱신 및 정리 진행해

문서를 훑었더니 **같은 종류의 고장이 세 군데** 있었다 — 전부 *"표에 행을 안 넣은 것"* 이다.
`SETUP_STATUS` 를 채우다 발견한 그 패턴이 다른 문서에도 있었다.

### 찾은 것

| 어디 | 무엇 | 왜 안 보였나 |
|---|---|---|
| `Parallel/BUGS.md` | **빈 줄 3개가 표를 4조각으로 쪼갰다**(23·25·29줄) | 마크다운에서 **빈 줄 뒤는 새 표**다 — `B8` 이후 15행이 헤더 없이 렌더된다 |
| `Parallel/BUGS.md` | `B12`~`B15` 가 **절만 있고 표 행이 없었다** | 목록만 보면 그 넷은 **존재하지 않는 버그**다 |
| `Parallel/BUGS.md` | `B13`~`B19` 절 제목이 **아직 🔴/🟡** 였다 | 표는 `수정됨` 인데 제목은 열린 것처럼 보인다 — **제목만 훑으면 7건이 열려 있다** |
| `Parallel/BOARD.md` §1 | *"지금 돌고 있는 작업"* 에 **`완료` 행 3개**(`D38`·`D39`·`D40`) | §3-1(완료 이력)이 `D91` 에서 갈라져 나오기 **전**에 적힌 것들이라 남았다 |
| `Parallel/REQ/DEV.md` | 🔴 **`요청-42` 안에 DEV 미처리가 둘 묻혀 있었다** | 아래 참조 |
| `Docs/TODO.md` | `B13` 이 *"남은 결정"* 에 있었다 | `D83` 이 **B안으로 이미 결정**했다 (코드로 확인) |
| `Docs/ROADMAP.md` §3-1 | *"TAB 을 누르고 있으면 · `timeScale 0.25`"* | `D92` 가 **TAB 을 아예 없앴다.** `Key.Tab` 을 읽는 코드가 **한 줄도 없다** |
| `REQ/CONTENT.md` | 링크 `../BALANCE.md` 가 한 단계 모자랐다 | — |

### 🔴 `요청-42` — 요청 하나에 항목이 다섯이면 제목이 거짓말을 한다

`요청-42`(C51)에는 **①·①-3·②·③·④** 다섯 항목이 들어 있는데
①·③·④ 는 `조치 불요`고 ①은 CONTENT 가 **스스로 취소**했다.
⇒ **제목만 보면 끝난 요청으로 읽힌다.** 실제로는 DEV 가 할 일이 둘 남아 있었다.

| 무엇 | 왜 사소하지 않나 |
|---|---|
| `Prefab_UpgradeCard` 의 **`MAX` 도장이 `Lv.2 / 2` 를 덮는다** | 증상이 *"지저분하다"* 가 아니라 **"몇 레벨로 끝났는지 못 읽는다"** 다. 게다가 `DimImage` 알파가 **0.902** 라 가린 게 아니라 **비친다** — `D47`·`D92` 와 **같은 병이고 세 번째다** |
| `ShopManager.cs:164` 의 `비용 누계` → `다음 리롤` | **값이 아니라 이름이 틀렸다.** 이미 `D96` 을 낚아 그 판 수입 추정을 **377G → 482G(+28 %)** 로 어긋내 놨다 |

**고치지 않고 `TODO.md` §1 맨 위로 끌어올렸다** (`CLAUDE.md` §1 — 임의로 고치지 않는다).

### 고친 것

| 파일 | 변경 |
|---|---|
| `Docs/Parallel/BUGS.md` | 표의 빈 줄 제거 · `B1`~`B19` 오름차순 · **`B12`~`B15` 4행 신설** · 절 제목 7개를 `✅ 해결됨` 으로 |
| `Docs/Parallel/BOARD.md` | §1 의 `완료` 3행을 §3-1 로 (`D41` 앞 = 연대순 자리) · `C52` 닫음 · `C51` 에 **미처리 2건 명시** |
| `Docs/TODO.md` | 머리말 갱신 · `B13` 을 결정 목록에서 뺌 · **§1 맨 위에 미처리 2건** · 완성률 표 재실측 · 레벨업 카드 복귀 항목 닫음 |
| `Docs/ROADMAP.md` | §3-1 TAB 줄 정정 · §3-2 를 *"비어 있다"* 에서 *"넣으면 좋아진다"* 로 · §0 에 `D85`~`D104` 총평 |
| `Docs/Parallel/REQ/CONTENT.md` | 링크 한 줄 |

### 판정

- 문서 18개 전수 링크 검사 — **깨짐 0** (전 1건)
- `BUGS.md` 표가 `B1`~`B19` **끊김 없이** 이어진다 · 열린 버그는 **`B8` 하나**
- `BOARD.md` §1 에 `완료` 행 **0** (전 3)
- `B13` 이 정말 닫혔는지 **코드로 확인**했다 — `Final = StatBlock.Add(baseStats, meta)` 이고
  `StatBlock.Add` 에 `Luck = a.Luck + b.Luck` 이 있다

🔑 **세 문서가 같은 방식으로 낡았다** — 작업은 했는데 **목록에 안 올렸다.**
목록은 훑는 사람이 *"남은 게 이것뿐"* 이라고 믿는 자리라, 빠지면 **없는 일**이 된다.
`D104` 가 `SETUP_STATUS` 에서 그걸 한 번 겪고 나서야 나머지 둘이 보였다.

---

## 2-129. ✅ **보스 화살표 — 계산은 내내 맞았고 그릴 대상이 없었다** (D104, 2026-09-06 124차)

> 아직도 보스 방향 화살표가 안보여

사용자가 **네 번째로** 같은 말을 했다. 앞의 세 번은 전부 빗나갔다 —
`D83` 은 *"깨진 게 아니라 뜰 일이 없다"*, `D85` 는 조건을 넓혔고(멀면 화면 안이어도 뜬다),
`D86` 은 크기를 44 → 96 으로 올렸다.

### 원인

🔴 **`BossArrowGraphic` 이 얹힌 오브젝트에 `CanvasRenderer` 가 없었다.**
`Graphic` 은 그게 없으면 **한 픽셀도 안 그린다** — 예외도, 경고도 없이.

`Graphic` 이 `[RequireComponent(typeof(CanvasRenderer))]` 를 달고 있는데도
**파생 클래스를 `AddComponent` 할 때 그게 안 따라왔다.** 실측으로 확인했다:

```
components:
  - RectTransform
  - BossArrowGraphic
CanvasRenderer = 없음        <-- 대조군 PortraitImage 는 있음
```

🔑 **이 고장이 세 번의 검증을 통과한 이유**가 여기 있다.
화살표는 오브젝트가 생기고 · 켜지고 · **위치와 각도까지 정확했다**
(실측 `anchoredPos (321, 472)` · `rotZ 55.8°`, 뒤에 잰 `rotZ 35.6°` 는 보스 방향 35.0° 와 일치).
`activeSelf` 를 보는 검증은 **전부 통과한다.** 안 그려지는 것만 빼고 다 맞았다.

### 고친 것

| 파일 | 변경 |
|---|---|
| `Assets/Scripts/UI/BossArrowGraphic.cs` | `[RequireComponent(typeof(CanvasRenderer))]` 를 **클래스에 직접** 달았다 |
| `Assets/Scripts/UI/BossOffscreenArrowUI.cs` | `BuildArrow()` 가 `new GameObject(..., typeof(RectTransform), typeof(CanvasRenderer))` 로 **생성 시점에 못박는다** · 컴파일 확인용 `Version = 1` |
| `CLAUDE.md` §3 | 두 줄 추가 — `Graphic` 상속 시 속성 직접 달기 / **UI 가시성을 `activeSelf` 로 판정 금지** |
| `Docs/TODO.md` | `D85` 의 *"조건을 넓혀 닫았다"* 절을 **진짜 원인으로 정정** |

**2겹으로 막았다.** 속성만으로는 이번처럼 안 따라올 수 있고, 생성 시점 명시만으로는
이 클래스를 다른 데서 `AddComponent` 할 때 또 빈다.

### 판정

| 조건 | activeInHierarchy | `materialCount` |
|---|---|---|
| `Wave` + 보스 **22.6** 유닛 | **True** | **1** ← 대조군 HUD `Fill` 과 같은 값 |
| `Wave` + 보스 **1.4** 유닛 (대조군) | False | 0 |
| 고치기 전 | (조건 무관) | **`CanvasRenderer` 자체가 없음** |

화면 사각형 `x[678..732] y[433..486]` = **54x52px**, 856x498 화면 안.
`cull = False` · `CR alpha 1.00` · 색 알파 0.85(맥동 중).

🔴 **대조군을 한 번 버렸다** — 처음 잰 *"가까우면 꺼진다"* 는 `state = LevelUp` 이 섞여
**원인이 둘**이었다. 레벨업을 소진해 `Wave` 로 되돌린 뒤 다시 쟀다.

🔑 **교훈 — `activeSelf` 는 "그려진다"가 아니다.**
`CanvasRenderer.materialCount` 를 **확실히 보이는 요소와 나란히** 재라.
이번엔 그 한 줄이 세 번 놓친 것을 첫 시도에 잡았다.

---

## 2-128. ✅ **도감 — `EVOLUTION` 은 무기만, `CLASSES` 는 티어별 줄로** (D103, 2026-09-06 123차)

> 모두 확인했어 / 마지막으로 Evolution쪽에는 무기만
> class는 Tier1, 2, 3 이런식으로 분류해서 유저가 더 상위 클래스가 어떤것인지 알수 있게

### 원인

**① `EVOLUTION` 탭에 승급 4종이 섞여 있었다.** `BuildEvolutions()` 가 `ClassEvolutions` 까지
넣어서 `CLASSES` 탭과 **같은 항목이 두 탭에 겹쳐 떴다.** 무엇을 보는 탭인지 흐려진다.

**② `CLASSES` 탭이 10종을 한 덩어리로 흘렸다.** `GridLayoutGroup` 은 칸을 순서대로 채우므로
**티어 2와 3이 같은 줄에 섞였다.** 사용자가 스크린샷에 `tier 2`·`tier 3` 를 **손으로 적어**
보냈다 — 화면만으로는 상위가 무엇인지 알 수 없었다는 뜻이다.

### 변경한 파일

| 파일 | 변경 |
|---|---|
| `Assets/Scripts/UI/CodexPanel.cs` | `BuildEvolutions()` 에서 `ClassEvolutions` 제거 · `Entry.Filler`/`Entry.Label` 신설 · `BuildClasses()` 가 Tier 로 정렬하고 `GridColumns`/`PadRow()` 로 `TIER n` 제목 줄을 끼운다 · `DrawProgress` 는 `Filler` 를 세지 않고 `DrawCells` 는 클릭을 막는다 |

🔑 **`GridLayoutGroup` 에서 줄을 끊는 방법이 이것뿐이다** — 칸을 순서대로 흘리므로
제목이 자기 줄을 쓰려면 **남은 칸을 빈 칸으로 채워야** 한다.
🔴 제목·빈 칸은 **진행률에서 빼고 클릭도 막았다.** 안 그러면 `10/16` 이 되고 눌러도 아무 일이 없다.

### 판정

- `Evolution 3/3`(무기만) · `Classes 10/10` 유지
- 격자가 `[TIER 1] / 6종 / [TIER 2] / 3종 / [TIER 3] / Aegis` 로 끊긴다

🔴 **중괄호를 하나 남겨 파일을 깼다.** `Version` 이 7 에서 안 오르길래 `Editor.log` 를 봤더니
`CS8803`·`CS0106`·`CS1022` 가 **25건**이었다.
🔑 **컴파일 확인이 이걸 잡았다** — 콘솔 에러는 **0** 이고 `isCompilationSuccessful` 도 **true** 였다.
`D27` 의 `Version` 상수 가드가 없었으면 **구버전 코드로 신버전 결과를 적을 뻔했다.**

---

## 2-127. ✅ **`B19` — 도감이 틀린 게 아니라 모자랐다** (D102, 2026-09-06 122차)

> 터렛 5 아머 5 레벨인데 안되는데 잘못 적혀있는거야?

### 원인

🔑 **틀린 게 아니라 모자랐다.** 도감 `CLASSES` 탭이 `Aegis` 조건을
*"Turret Lv.5 + Armor Lv.3"* 로만 적고 **선행 직업 `Sentinel` 을 빼먹었다.**

`EVOLUTION` 탭 본문에는 `"Requires class:"` 줄이 있는데 `CLASSES` 탭에만 없었다 —
**같은 사실을 두 곳에서 따로 만들다** 한쪽이 빠진 것이다
(`Recipe()` 가 재료 배열만 받아 `FromClass` 를 모른다).

### 변경한 파일

| 파일 | 변경 |
|---|---|
| `Assets/Scripts/UI/CodexPanel.cs` | 조건 줄 앞에 선행 직업을 붙인다. 미발견이면 `???` 로 가려 **도감 규칙을 지킨다** |

### 판정

- `Aegis` 조건이 `Sentinel > Turret Lv.5 + Armor Lv.3` 로 나온다
- **대조군 — 선행이 없는 나머지 셋은 그대로다**

⚠️ **`B18` 과 원인이 둘 겹쳐 있었다**(제단 반경 + 도감 표시). 하나만 고쳤으면 사용자는
여전히 못 했을 것이다. 사용자가 *"터렛 5 아머 5"* 라고 **자기 상태를 숫자로** 준 덕에
두 번째가 드러났다.

---

## 2-126. ✅ **`B18` — 레시피는 멀쩡했고 숫자 둘이 안 맞았다** (D101, 2026-09-06 121차)

> 이 직업이 진화가 안되네 (도감 9/10 · `Aegis` 만 미발견)

### 원인

🔑 **레시피는 멀쩡했다.** `Aegis`(Sentinel 선행 · Turret Lv5 + Armor Lv3 · 제단 Turret)로
**끝까지 승급시켜 봤다.**

🔴 **안 맞은 것은 숫자 둘이다.** 건물은 `placeDistance 1.8` 링 위에 놓이는데
실측 거리가 **1.18 ~ 2.51** 로 흩어지고, 제단 반경(`altarRadius`)은 **2.2** 였다.
⇒ **갓 세운 건물이 자리에 따라 `E` 에 닿기도 하고 안 닿기도 한다.**
같은 조작이 될 때도 있고 안 될 때도 있었다.

### 변경한 파일

| 파일 | 변경 |
|---|---|
| `Assets/Game/Balance/Economy.csv` | `EvolutionManager.altarRadius` **2.2 → 3** (링 최대 2.51 을 덮는다) |
| `Assets/Scenes/SampleScene.unity` | Import 결과 반영 |

### 판정

- 거리 2.40 에서 `FindAltarClassEvolution = null` — **재현**
- 0.80 으로 옮기니 `Sentinel > Aegis` 승급 성공
- 반경 3 이 실측 링 거리(1.18~2.51) 전부를 덮는다 · 씬 저장 확인

🔴 **시험이 한 번 오염됐다.** 앞 판정의 잔여 상태(직업 · 터렛 3개)가 남아
`TryEvolveAtAltar = False` 와 `사슬 = Sentinel > Aegis` 가 **동시에** 나왔다.
판을 새로 켜고 한 명령 안에서 다시 쟀다.

---

## 2-125. ✅ **칼 방향 — 코드 버그가 아니라 그림이 엇갈렸다** (D100, 2026-09-06 120차)

> 8종 확인해서 표로 정리한다음 진행해
> warrior, mage, demolitionist, assasin, summoner, doomlord는 아직 반대로 보여
> ranger, sentinal 은 제대로 움직여

### 원인

🔴 **코드 버그가 아니다.** 걷기 시트가 전부 **정면 그림**이라 "향하는 쪽"이 없고,
**무기를 어느 손에 그렸는지**가 곧 기본 방향이다 — 그게 직업마다 다르다.
Warrior 는 칼이 왼쪽, Ranger 는 활이 오른쪽이라
`sr.flipX = input.x < 0` **한 규칙으로는 둘 다 못 맞춘다.**

### 변경한 파일

| 파일 | 변경 |
|---|---|
| `Assets/Scripts/Player/CharacterClassData.cs` | `public bool ArtFacesRight` 신설 |
| `Assets/Game/Balance/Classes.csv` | `ArtFacesRight` 열 (`BonusBuildingSlots` 뒤) |
| `Assets/Editor/BalanceImporter.cs` | 임포트/익스포트 양쪽 |
| `Assets/Scripts/Player/PlayerController.cs` | `flipX = _artFacesRight ? input.x < 0 : input.x > 0` · `SetArtFacesRight` |
| `Assets/Scripts/Player/PlayerStats.cs` | `ApplyClassVisual` 에서 배선 (🔴 **승급할 때도 다시 온다**) |

🔑 **자동 판정을 포기하고 사람 눈으로 표를 만들었다.** 두 번 시도했고 둘 다 틀렸다 —
내 불투명 픽셀 무게중심은 Ranger 를 *"중앙"* 으로 읽었고(몸통 질량이지 무기 위치가 아니다),
CONTENT 의 꼬리 길이 판정은 Warrior 에서 틀렸다(8/10).
**둘 다 무기가 아니라 다른 것을 재고 있었다.**
사용자 관찰 8종 + 내가 그림을 연 4종이 **4/4 일치**했고,
목록에 없던 `Aegis`(총구 오른쪽) · `Warden`(방패 왼쪽)만 내가 채웠다.

### 판정

- 10종 값이 Import 로 애셋에 들어감 — **오른쪽 3 / 왼쪽 7**
- 직업을 갈아 끼우면 규칙이 따라온다 (Warrior → 뒤집는다 / Ranger → 그대로)
- 씬 저장 확인

🔵 `ArtFacesRight` = 무기가 **그림 오른쪽**인가. `Ranger`·`Sentinel`·`Aegis` 만 `1` 이다.

---

## 2-124. ✅ **잔손 3건 — 앵커만 보고 계산이 틀렸다** (D99, 2026-09-06 119차)

> 1. 아직 흰색 아니고, build 좀 더 아래로
> 2. 상점 이제 잘되
> 3. 이제 괜찮아 근데 아직도 warrior 오른쪽으로 이동할때 칼이 왼쪽을 향해

### 고친 것

**① 레벨업 리롤 글자 흰색.** `ShopUI` 는 이미 `Color.white` 인데 `LevelUpManager` 만
금색(`0.96, 0.80, 0.28`)이었다 — 카드 강조색·`LEVEL UP` 제목과 **같은 색**이라
버튼인지 장식인지 안 갈렸다. `RerollOn` 을 `Color.white` 로.

**② `[Z] Build` 를 아래로.** 유효 위치 **30 % → 17 %** (336 → 190px).

🔴 **처음에 앵커만 보고 계산이 틀렸다** — `anchoredPosition y = 90` 이 더해지는 걸 놓쳤다.
⇒ 오프셋을 **0 으로 만들고 앵커 하나로만** 자리를 정했다. 둘이 섞이면 또 틀린다.

**③ 🟡 칼 방향은 못 고쳤다 — 코드 문제가 아니다.** → `요청-60`.
`D100`(2-125)에서 `Classes.csv` 의 데이터로 풀었다.

### 변경한 파일

| 파일 | 변경 |
|---|---|
| `Assets/Scripts/LevelUp/LevelUpManager.cs` | `RerollOn` → `Color.white` |
| `Assets/Scenes/SampleScene.unity` | `HUD/BuildPromptText` 앵커 y **0.17** · 오프셋 **(0, 0)** |

### 판정

리롤 글자 `Color.white` · `[Z] Build` 앵커 y 0.17 · 오프셋 (0,0) · 씬 저장 확인.

---

## 2-123. ✅ **칸이 꽉 차면 레벨업이 빈 패널이었다 — 대체 보상 3종** (D98, 2026-09-06 118차)

> 아이템이 꽉차고 레벨업하면 아무것도 안뜨네
> 골드 일정량 or 체력 회복 or 남은 시간 5초 이렇게 추가해

### 🔴 레벨업을 했는데 아무것도 못 받는 구멍

`PickItems` 가 **0장**을 돌려주면 `RefreshPanel` 이 카드를 전부 껐다:

```csharp
bool hasItem = i < _currentChoices.Count;
cards[i].gameObject.SetActive(hasItem);      // 0장이면 전부 false
```

남는 건 `LEVEL UP` 글자와 리롤 버튼뿐이다(사용자 스크린샷이 정확히 그 모양이다).
**리롤을 눌러도 뽑을 게 없으니 소용없다.** 레벨업이 그냥 증발한다.

### 🔑 상점이 이미 같은 문제를 풀었다

`D71` 이 *"살 게 없으면 휴식·환전을 판다"* 로 상점을 고쳤다. **여기도 같은 발상**이다 —
**고를 게 없으면 다른 걸 준다.** 새 개념을 만들지 않고 그 구조를 따랐다.

| | 상점 (`D71`) | 레벨업 (이번) |
|---|---|---|
| 열거형 | `ShopSlotKind {Item, Heal, Exchange}` | `LevelUpBonusKind {Gold, Heal, Time}` |
| 카드 | `ShopCardUI.SetupService` | `ItemCardUI.SetupBonus` |
| 카드 프리팹 | **같은 것 재사용** (아이콘만 끈다) | **같은 것 재사용** |

### 만든 것

| 무엇 | 값 | 카드 |
|---|---|---|
| 골드 | **+60** 런 골드 | `+60 G` / *Run gold, right now.* |
| 회복 | **+40** HP | `+40 HP` / *Patch up on the spot.* |
| 시간 | **−5초** 남은 웨이브 | `-5s` / *Ends this wave sooner.* |

🔴 **시간 카드는 조건부다.** 킬 목표 웨이브에는 타이머가 아예 없어(`WaveRemainingTime == -1`)
줄일 것이 없다 — 그런 웨이브에서는 **아예 안 내놓는다.**
**못 쓰는 선택지를 보여 주는 건 아무것도 안 주는 것보다 나쁘다.**

🔴 **`WaveManager.TryCutRemainingTime` 을 `DevSetRemainingTime` 과 따로 뒀다.**
그쪽은 `DevPanel` 전용 치트 입구고 이건 게임 규칙이라 릴리즈에도 있어야 한다.
같은 함수를 쓰면 **치트와 규칙이 한 줄에서 얽힌다.**

수치 3개는 `Economy.csv` 가 원본이다(CONTENT 소유 → `요청-59`).
60·40 은 *"일정량"* 이라 감으로 넣었고 **5초는 사용자가 명시했다.**

### 검증 로그 (플레이 모드 · 칸 3/3 · 5/5 · 5/5)

```
칸 — 무기 3/3 · 패시브 5/5 · 건물 5/5
타이머 웨이브인가 = True · 남은 60.0s
  카드: [BONUS] -5s    — Ends this wave sooner.
  카드: [BONUS] +40 HP — Patch up on the spot.
  카드: [BONUS] +60 G  — Run gold, right now.
=> 뜬 카드 3장 (예전엔 0장 = 빈 패널)

① Gold : 23 -> 83   (+60) ok
② Heal : HP 89 -> 129 (+40) ok
③ Time : 35.9s -> 30.9s (-5) ok

실제 경로 Wave -> LevelUp -> 선택 -> Wave · timeScale 1
```

**판정 7/7.** 🔑 **뜨는 것과 먹히는 것은 다르다** — 카드가 3장 보이는 것만 확인하고 끝냈으면
*"눌러도 아무 일 없다"* 를 놓칠 뻔했다. 셋 다 눌러서 값이 실제로 움직이는 것을 봤다.

### 🔴 시험 중 두 가지를 잡았다

**① `B17` 가드 경고가 떴다 — 알고 보니 내 시험 탓이었다.**
`SelectBonus` 를 직접 부르면서 `ChangeState(LevelUp)` 을 건너뛰어, `OnLevelUpCompleted` 가
*"상태가 이미 Wave 다"* 로 물러났다. **실제 경로**(`TriggerLevelUp` 이 상태를 먼저 바꾼다)로
다시 태우니 경고가 안 뜬다. ⇒ **가드는 과하지 않다.**

**② 🔴 CSV Import 뒤 씬이 저장돼 있지 않았다.**
`git status` 에 `SampleScene.unity` 가 없어서 이상해 확인했더니 `isDirty = True` 였다 —
**확인 안 했으면 60·40·5 가 다음 리로드에 사라졌을 것이다.**
⇒ **Import 뒤에는 씬 저장을 눈으로 확인한다.**

---

## 2-122. ✅ **상점 서비스 칸 — 글자를 반으로 줄이고 숫자를 키웠다** (D97, 2026-09-06 117차)

> 개발자 커맨드로 아이템을 꽉채우고 상점을 처음 들어가니 아이템들이 나왔고 살수가 있었어
> 꽉찼을때 heal이랑 exchange는 잘보여 근데 글자가 너무 많아 직관적이게 바꿔줘

### 🔑 긴 것은 서비스 칸 **둘뿐**이었다

아이템 카드의 설명은 `Items.csv` 에서 오고 **원래 한 줄**이다(*"Explodes on the nearest enemy."*).
길었던 건 코드가 들고 있던 `Rest`·`Exchange` 두 개다. 특히 `Exchange` 는 **두 문장이 같은 말을 두 번** 했다:

```
Run gold vanishes when the run ends.      <- 이유
Carry some of it home instead.            <- 이미 "+40 meta G" 가 말하고 있다
```

⇒ 뒤 문장을 버렸다. **남길 건 이유 한 줄**이다 — *"왜 100을 주고 40을 받나"* 에만 답하면 된다.

| | 전 | 후 | |
|---|---|---|---|
| Rest | `Patch yourself up before the next fight.` (40자) | `Heal up before the next fight.` | **30자** |
| Exchange | 2줄 (67자) | `Run gold vanishes at the end.` | **29자** |
| **합** | **107자** | | **59자 (−45 %)** |

그리고 효과값을 `<size=160%>` 로 **카드에서 가장 큰 글자**로 만들었다 —
이 두 칸에서 사람이 알고 싶은 건 *"얼마 주고 뭘 얼마나 받나"* 하나뿐이다.

### 🔴 내가 넘침을 만들었고, 재서 잡았다

글자를 키우자 `LevelText` 가 **32px 필요 / 26px 자리**로 넘쳤다.
🔑 **`D93` 에서 배운 대로 커밋 전에 쟀기 때문에** 화면에 나가기 전에 걸렸다.

`DescText` 가 96 중 **43만 쓰고 있어서** 남는 자리를 넘겼다:

| | 전 | 후 |
|---|---|---|
| `LevelText` | 26 | **38** |
| `CategoryTag` | 26 (y −221..−195) | 26 (y −233..−207) |
| `DescText` | 96 | **84** |

### 🟢 곁일 — `2-E` 의 질문을 닫았다

사용자가 `PLAYTEST_2` 2-E 에 *"아이템 소지 제한이 적용되고 있는지 확인해줘"* 라고 적어 뒀다.
`CanAcquire` 가 막을 때까지 전부 사들여 봤다:
...
**상한은 실제로 걸린다.** 꽉 차면 `Heal · Exchange · Exchange` 로 바뀌는 것도 `D71` 설계대로다.

⚠️ 사용자가 본 *"꽉 찼는데 아이템이 나왔고 살 수 있었다"* 는 **업그레이드**였을 가능성이 크다 —
스크린샷의 카드가 `Fireball Lv.1 → 2` 다. **이미 가진 것의 레벨 올리기는 칸을 더 먹지 않으므로
꽉 차도 살 수 있는 게 맞다.** 콘솔의 `구매 거절 — 'Bow'` 는 반대로 `B12` 가드가 **새 아이템을 막은** 기록이다.

### 검증 로그 (플레이 모드 · 꽉 찬 상태)

```
칸 상한 — 무기 3/3 · 패시브 5/5 · 건물 5/5
── Exchange ──  NameText 29/34 · LevelText 32/38 · CategoryTag 20/26 · DescText 43/84 · PriceText 29/32 · BuyText 29/52
── Heal ──      (같음)
=> 넘치는 글자 0 / 18  전부 들어간다
```

🔑 **프리팹을 고쳤으므로 플레이를 껐다 켜고 다시 쟀다** — 안 그러면 이미 만들어진
옛 인스턴스를 재게 되어 *"고쳤는데 그대로"* 로 보인다.

---

## 2-121. ✅ **`B17` — 상점 레벨업이 상태 기계를 꼰 사슬** (D95, 2026-09-06 116차)

> 상점을 키고 오래 있으니까 레벨업하면서 게임이 고장났어 확인해봐

### 🔴 예외가 하나도 안 났다 — 그래서 콘솔로는 안 잡힌다

`Editor.log` 의 **스택 트레이스**로만 원인을 갈랐다. 재현된 사슬:

```
[ShopManager] 상점 열림 · State → Shop
[ShopManager] 구매: Turret (125G)
State → LevelUp     ExperienceManager.TriggerLevelUp:179 <- CollectXp:171
                    <- VillageBuilding.OnCooldownElapsed:11 <- BuildingBase.Update:65
State → StageMap    ShopManager.CloseShop:140
State → Wave        GameManager.OnStageNodeSelected:261
State → Shop        🔴 GameManager.OnLevelUpCompleted:453 <- LevelUpManager.SelectItem:221
```
...
| | 무엇 |
|---|---|
| ① | `BuildingBase.Update` 가 **무조건** 돌아서 상점에서도 마을이 XP 를 준다 (*"오래 있으니까"*) |
| ② | `ShopRoot`[2] 가 `LevelUpPanel`[1] **위에** 그려져 카드가 숨고 클릭을 상점이 먹는다 |
| ③ | `OnLevelUpCompleted` 가 **열 때 찍은** `_stateBeforeLevelUp`(=Shop)을 뒤늦게 되돌린다 |

`DimBG` 는 `raycastTarget=True` 지만 **자기보다 아래만** 막는다 — 위에 있는 상점은 못 막는다.

### 🔑 내 권고가 틀렸고, 사용자가 바로잡았다

나는 *"①은 고치지 않는 쪽을 권한다 — 막으면 상점에 있는 동안 마을이 논다"* 고 적었다.

> 상점 노드에 있을때는 마을나 농장같은 건물이 작동하면 안되지 / 전투 노드에 있을때만 작동해야해

**맞는 말이다.** 나는 *"기존 동작을 지키는 것"* 을 말없이 기본값으로 놓고 판단했는데,
애초에 **마을·농장이 전투 밖에서 도는 것 자체가 이상하다.** 터렛·곡사포도 마찬가지다 —
맵 화면에 쏠 적이 없다. ⇒ **셋 중 가장 위쪽(원인)을 끊는 답**이었고 내가 그걸 못 봤다.

🔑 **"지금 이렇게 돼 있다"는 그 자체로 근거가 아니다.** 바꿨을 때의 손해만 세고
지금 상태의 이상함은 안 센 것이 이번 실수의 모양이다.

### 변경한 파일

| 파일 | 무엇 |
|---|---|
| `Building/BuildingBase.cs` | ① 쿨다운이 **`GameState.Wave` 에서만** 돈다. `Version = 1` |
| `Core/GameManager.cs` | ③ `OnLevelUpCompleted` 가 **아직 `LevelUp` 일 때만** 되돌린다 |
| `Scenes/SampleScene.unity` | ② `LevelUpPanel` 형제 순서 **1 → 7** (상점 위 · 일시정지 아래) |

🔑 **타이머를 리셋하지 않고 멈춘다** — 전투로 돌아오면 멈춘 자리에서 이어진다.
리셋하면 상점을 들를 때마다 건물이 한 박자씩 손해를 본다.

🔑 **셋 다 했다.** ①이 원인을 없애지만 ②는 *"일어나도 안 보이게"*, ③은 *"일어나도 안 깨지게"* 다.
`B14`·`B15` 의 교훈대로 ①만 하면 다른 XP 경로가 생길 때 같은 사슬이 되살아난다.

### 검증 로그 (플레이 모드 · 실시간 20초씩)

```
[대조군] 전투 아님(StageMap) · timeScale 1 로 20초  ->  Lv 1 · XP 0
[본시험] 전투(Wave) 로 20초                        ->  Lv 1 · XP 2
...
**판정 5/5.** 🔑 **대조군이 절반이다** — *"상점에서 안 오른다"* 만으로는 *"아예 안 돈다"* 와
구별되지 않는다. `timeScale 1` 을 명시적으로 확인해 **시간이 흐르는데도 안 오른다**를 보였고,
전투로 바꾸니 **XP 2** 로 올랐다.

⚠️ `Restaurant`·`Farm` 도 같이 멈춘다 — 의도한 것이다.
🔵 검증 중 뜬 `[Warning]` 은 **내가 새로 넣은 가드가 제 일을 한 것**이다.

---

## 2-120. ✅ **CONTENT 요청 3건을 한 번에 닫았다 — 열린 항목 0** (D94, 2026-09-06 115차)

사용자 지시 *"CONTENT 요청 3건 진행해"*. 열어 보니 **CONTENT 가 이미 답을 다 올려 뒀고**
남은 것은 **DEV 가 배선·확인할 일**이었다.

| 요청 | CONTENT 가 한 것 | DEV 가 할 것 |
|---|---|---|
| `요청-39` (C47) | `HpRegen.png` 아이콘 + `.meta` 를 직접 맞춤 | 임포트 확인 |
| `요청-40` (C48) | `SFX_AmbushStart.wav` 2.40초 스팅어 | **배선** (`SfxId 34`) |
| `요청-41` (C49) | 사용자 스크린샷에서 **도감 공백**을 발견 | **한 줄 추가** |

### ① 🔴 도감의 `NUMBERS` 가 비어 있었다 — `B13` 과 같은 모양, 다른 증상

`CodexPanel.AppendPassive` 가 스탯을 12줄 나열하는데 **`BonusHpRegen` 만 없었다.**
데이터(`PassiveData`)도 효과(`PlayerStats`)도 멀쩡한데 **표시에서만** 빠졌다.

🔑 **`B13`(Luck)과 같은 원인, 다른 증상이다.**

| | `B13` | 이번 |
|---|---|---|
| 빠진 곳 | 최종 스탯 합산 | **도감 표시** |
| 증상 | **효과가 안 난다** | **효과는 나는데 안 보인다** |
| 잡히는 방법 | 수치를 재면 잡힌다 | 🔴 **화면을 봐야만 잡힌다** |

⇒ 새 스탯 열을 넣을 때 고쳐야 하는 **소비처 목록**을 `CodexPanel` 주석에 박았다:
`PassiveData`(배열+Get) · `PassiveEffect` · `StatBlock` · `PlayerStats` · **`CodexPanel`** · `Economy.csv`.

🔑 **컴파일만 보고 넘기지 않았다** — 도감을 열어 18칸을 순회하며 `Regeneration` 을 찾아 눌렀다:

```
Regeneration 칸 = 17 / 18 · 제목 = Regeneration
   <b>NUMBERS</b>   Lv1 -> max
   HP Regen    0.4  ->  2.6
=> 'HP Regen' 줄 = 있다
```

### ② 기습 스팅어 — **판정 자리를 이름이 아니라 경로로 잡았다**

`SfxId.AmbushStart = 34` (CONTENT 권고 · `BossAppear = 33` 옆의 "사건 알림" 블록).

*"`Ambush1` 이 시작되는 프레임"* 을 **웨이브 이름 비교**로 짜면
애셋 이름을 바꾸는 순간 **조용히 죽는다.** 그래서 **"이벤트가 전투를 직접 골랐나"** 로 잡았다:

```csharp
if (overrideWave != null) AudioManager.Play(SfxId.AmbushStart);
```

⚠️ **오늘은 그 경로가 `Ambush1` 하나뿐이라 같은 뜻이다.** 이벤트 전용 웨이브가 하나 더 생기면
**그것도 이 소리를 물려받는다** — 그때는 `WaveData` 에 깃발을 둬야 한다. 주석에 적어 뒀다.

🔑 **소환 코루틴보다 먼저** 울린다. 목적이 *"지금부터 다르다"* 라서
적이 이미 몰려온 뒤에 울리면 **경고가 아니라 설명**이 된다.

### 검증 로그

```
SfxId.AmbushStart = 34
배선 전 빠진 것: Crit, AmbushStart      <- 검증기가 실제로 잡는다
배선 후 빠진 것: Crit                    <- Crit 은 원래 미배선(enum 주석)
GetSfx(AmbushStart) = SFX_AmbushStart · 2.40s · Volume 0.9 · Jitter 0

[대조군] 노말 시작 직후 AmbushStart 재생 = 0
[본시험] 기습 시작 직후 = 1  ->  그 프레임에 울린다
   보이스: vol 0.105 · pitch 1.000 · time 0.000s / 2.40s
보스음과 다른 클립인가 = True
...
**판정 8/9.** 🔑 **대조군이 판정의 절반이다** — *"기습에 울린다"* 만으로는
*"아무 웨이브에나 울린다"* 와 구별되지 않는다. **같은 노드로 노말을 시작해 0건**을 봤다.

### 🟡 못 닫은 하나 — 그런데 원인이 클립이 아닐 수 있다

CONTENT 의 기준 ③ *"100마리가 쏟아지는 중에도 들리나"* 는 사람 귀의 몫이다.
다만 재 보니 **사용자 `SfxVolume` 이 0.117** 이다:

```
0.90 (클립 볼륨) x 0.117 (사용자 SFX) = 0.105
```

🔴 **클립을 1.00 으로 올려도 천장이 0.117 이다.** 안 들린다면 원인이 클립이 아니라
**옵션의 저장 볼륨**일 가능성이 크다 — `D47` 이 이미 `BGM 0.059 · SFX 0.043` 을 봤다.
⇒ *"안 들린다"* 가 나오면 **클립 볼륨보다 옵션 화면을 먼저 본다.**

### ⚠️ 남의 파일을 건드렸다

`Assets/Game/Audio/AudioLibrary.asset` 은 CONTENT 소유다.
인스펙터가 아니라 `SerializedObject` 로 **항목 하나만** 추가했고 다른 항목은 안 건드렸다 → `요청-58`.

🟢 **이걸로 CONTENT 열린 항목이 0 이 됐다.**

---

## 2-119. ✅ **한 상자 · 하단 버튼 4개 — 칸만 키우면 아이템은 안 커진다** (D93, 2026-09-06 114차)

> 두박스 하나로 합치고 다른 게임의 스샷같이 하단에 큰 버튼으로 continue, options, quit, mainmenu 버튼 구성하고
> 스탯 글씨, 아이템 크기 키워

### ① 두 상자를 하나로

`D92` 는 `Body`(스탯)와 `Card`(버튼)를 **나란히** 뒀다. 참고 이미지는 **한 상자**다.

- `Card` 의 자식(`TitleText`→`PausedLabel` · `ControlsText` · 버튼 3개)을 `Body` 로 옮겼다
- `Body` 를 **1440x1000** 한 상자로
- 빈 `Card` 는 **지우지 않고** `Card (unused, D93)` 로 이름을 바꿔 **비활성**으로 남겼다 —
  참조가 없다고 확인했지만, 되돌릴 수 있는 쪽이 싸다

### ② 하단 큰 버튼 4개

`Continue · Options · Quit · Main Menu`, 각 **325x70**, 상자 아래쪽에 가로로.
순서는 사용자가 적은 그대로다.

`Main Menu` 는 새로 만들었다. 🔑 **`RunEndUI` 와 같은 경로를 쓴다** — `GameManager.ReloadScene(false)`.
흉내 내지 않는다: 그 안에 `timeScale` 복구와 `_autoStartRunOnLoad = false` 가 들어 있다.
🔴 **나가기 전에 메타를 저장한다** — 씬을 다시 로드하면 이번 런의 메타 골드가 그냥 사라진다.

### ③ 🔴 칸만 키우면 아이템은 안 커진다 — `B16` 의 나머지 반쪽

격자 칸을 76x96 → **96x104** 로 키웠는데 그것만으로는 **아이콘이 안 커진다.**
`Prefab_ItemChip` 의 `Icon` 이 **고정 64x64**(`B16`)이고, `D91` 은 **HUD 쪽만** 고쳤기 때문이다.
칸만 키우면 **여백만 늘어난다.**

⇒ `SetCompact(false)` 도 아이콘을 칸에 맞추게 했다. 픽셀이 아니라 **비율**로 나눈다:

| | 앵커 |
|---|---|
| `Icon` | 위 **74 %** (`anchorMin.y 0.26` ~ `anchorMax.y 1`) |
| `Label` (`Lv.N`) | 아래 **26 %** |

비율이라 칸을 또 바꿔도 따라간다. 🔵 `RunEndUI` 도 `SetCompact(false)` 를 부르므로
**거기 있던 같은 넘침도 같이 고쳐졌다.**

### ④ 조작 안내는 배치 방식을 바꿔야 했다

한 상자로 합치며 조작 단이 **488 → 390** 으로 좁아졌다.
`WASD · Arrow keys` 만으로 26pt 에서 약 200px 인데 `<pos=52%>` 는 203px —
**키와 값이 부딪힌다.** `HelpPanel` 이 `D85` 에서 같은 이유로 쓴
**키 한 줄 · 설명 들여쓰기** 방식으로 바꿨다. 단 너비와 무관해진다.

### 검증 로그 (플레이 모드 · 화면 856x498 · 손대지 않고 `Open()` 만)

```
② 열림: state=Paused timeScale=0 · Body active=True · 상자 수(활성) = 1
   상자 화면 x[107..749] y[26..472] (856x498) -> 화면 안
   StatsText 30pt · 필요 342 / 자리 400 -> 들어간다
   StatsRightText 30pt · 필요 380 / 자리 400 -> 들어간다
   ControlsText 26pt · 필요 367 / 자리 400 -> 들어간다
   칩 42.8x46.4px · 아이콘 33.9x25.4px · 아이콘이 칸 안 = True     (전 33.9x42.8)
   ResumeButton "Continue" 145x31px / OptionButton "Options" / QuitButton "Quit" / MainMenuButton "Main Menu"
   닫기 후 state=Wave timeScale=1
```
...
값 위치를 `<pos=72%>` → `58%` 로 바꾸며 *"72 % 면 네 자리 체력에서 넘친다"* 고 적었다.
**재 보니 아니었다** — `9999 / 9999` 로 만든 최악 문자열이 72 % 에서 **425/460**, 58 % 에서 **439/460** 이다.
둘 다 들어간다. 58 % 를 쓰는 진짜 이유는 **실제 본문의 여백이 5px(455/460) → 69px(391/460) 로 늘어서**다.

🔑 예측을 근거로 적기 전에 **그 예측도 재야 한다.** 이번엔 커밋 전에 잡았다.

### ✅ 사용자 판정 (2026-09-06)

> 푸시해 적절하네 메인메뉴 버튼도 잘 작동해

**`D87`~`D93` 일곱 건이 여기서 닫혔다.** 스크린샷 다섯 번을 오가며 고친 UI 묶음이
사람 눈으로 통과했다 — 칸 크기 · 간격 · 테두리 · 아이콘 · 한 상자 · 하단 버튼 4개.
🔑 **`Main Menu` 는 배선만 확인한 게 아니라 사용자가 실제로 눌러 봤다** —
`D86` 의 상점 판매 버그가 *"배선 여부만 봤으면 통과였다"* 였던 것과 같은 종류의 위험을 지웠다.

---

## 2-118. ✅ **TAB 을 버리고 ESC 한 화면으로 — 가림막이 아예 없었다** (D92, 2026-09-06 113차)

> 아직도 작아 좀더 크기 키워
> TAB 칸은 완벽한데 뒤 랑 겹쳐보이는게 쎄네
> 그냥 ESC 눌렀을때 3번째 이미지 처럼 게임도 멈추고 한번에 보여주는게 나을거 같아

### ① 칸을 한 번 더 키웠다 — **창 가로 대비로 재야 했다**

`D89` 까지는 *"화면 픽셀"* 로 쟀는데, 사용자는 그 사이 **게임 뷰를 키웠다**(856 → 약 1734).
화면 픽셀은 저절로 커졌는데도 *"아직도 작아"* 였다. ⇒ **절대 픽셀이 아니라 비율**로 재야 한다.
...
간격 22→**24**, 줄 폭 560→**690**(`8x64 + 7x24 = 680`), 줄 간격 76→**88**.
`XPGroup` 아래끝(-86)과 첫 줄 위끝(-116) 사이 **30px 여유**를 확인했다.

### ② 🔴 *"뒤랑 겹쳐 보인다"* — **가림막이 아예 없었다**

`StatsPanel` 의 자식은 `Body` **하나뿐**이었다. `PausePanel` 에는 있는 `DimBG` 가 **없다.**
게다가 `Body` 의 `UI_Panel` 스프라이트는 **채움 알파가 0.92**(`D47` 에서 이미 겪은 것)라
`Image.color` 알파를 1 로 줘도 뒤가 비친다. 그래서 전투 화면과 **HUD 아이템 줄이 글자 위로** 비쳤다.

⇒ 사용자 판단대로 **`ESC` 일시정지 화면으로 옮겼다.** 거기엔 이미 `DimBG` 와 `timeScale = 0` 이 있다.

| 무엇 | 어떻게 |
|---|---|
| `StatsPanel/Body` | **`PausePanel` 의 자식으로 이사** (`DimBG` 다음 · `Card` 앞) · 700x960 · 중앙에서 x −310 |
| `DimBG` 알파 | 0.65 → **0.92** (뒤가 8 % 만 비친다) |
| `Card` | 420x400 → **560x720**, 중앙에서 x +380 |
| `ControlsText` | **신설** — 참고 이미지처럼 조작을 같은 화면에 |
| `StatsPanelUI` | `Update`·`TryOpen`·`Close`·`slowTimeScale` **삭제**. `Refresh()` 만 `public` |
| `PauseMenuUI` | `Open()` 에서 `statsPanel.Refresh()` 호출 |

### 🟢 `timeScale` 3단 계약이 통째로 사라졌다

`D46` 에서 이 창을 만들 때, 저배속을 **스스로** 걸어야 해서 `DoHitstop` 과 같은 3단 계약이 필요했다
— *"웨이브일 때만 연다 · 이미 1이 아니면 안 연다 · 닫을 때 다시 확인한다"*.
멈추는 일을 `PauseMenuUI` 한 곳에 몰자 **그 계약이 통째로 필요 없어졌다.**
🔑 **기능을 옮겼더니 방어 코드가 사라진 것이다** — 옮길 자리가 맞으면 코드가 준다.

### 🔴 검증이 결함을 잡았다 — 값은 다 채워졌는데 화면은 비었다

`Awake` 의 `panelRoot.SetActive(false)` 를 없앴는데, `Body` 는 **씬에 꺼진 채로 저장**돼 있었다.
그걸 켜 주던 유일한 코드가 사라졌으니 **아무도 안 켠다.**

```
Open() 후 state=Paused · timeScale=0
PausePanel active=True
Body active=False          <- 🔴
  ClassText = "Warrior"    <- 값은 전부 정상
  ItemsTitleText = "ITEMS  (1)"
```

**로그만 보면 완벽하다.** `activeInHierarchy` 를 같이 안 쟀으면 통과로 적었을 것이다
(`D86` 의 *"뜬다 ≠ 보인다"* 와 같은 함정이다).
씬 값을 켜고, `Refresh()` 에 **자가복구 한 줄**을 넣었다 — 누가 또 꺼서 저장해도 살아난다.

### 검증 로그 (플레이 모드 · 화면 856x498 · 손대지 않고 `Open()` 만)

```
① 웨이브: state=Wave timeScale=1 · PausePanel active=False
② ESC 열기: state=Paused timeScale=0 · PausePanel=True · Body=True
   ClassText  = Warrior
   ItemsTitle = ITEMS  (1) · 격자 자식 1
   HintText   = [ESC] Resume  ·  The game is fully paused.
   DimBG alpha=0.92 (뒤가 8% 만 비친다)
   Body x[134..446] · Card x[473..722] -> 안 겹친다 (틈 27px)
③ 닫기: state=Wave timeScale=1
StatsText 316/340 · StatsRightText 316/340 · ControlsText 189/334 — 전부 들어간다
```

**판정 8/8.** 🔵 `HelpPanel` 의 `TAB` 안내도 같이 고쳤다 — 조작이 바뀌었는데 안내가 남아 있으면
**틀린 문서가 화면에 떠 있는 것**이다.

---

## 2-117. ✅ **아이콘이 칸보다 컸다 — 프리팹은 TAB 창 치수였다** (D91, 2026-09-06 112차)

> TAB에는 두지 말고
> 아이템이 제대로 테두리 안에 안들어가 / 프레임으로 만들어서 안에 들어가게해

### 🔴 원인 — `Icon` 이 고정 64x64 인데 칸은 50x50 이다

`Prefab_ItemChip` 의 `Icon` RectTransform 은 이렇게 박혀 있다.

```yaml
m_AnchorMin: {x: 0.5, y: 1}     # 위쪽 가운데 정렬
m_AnchorMax: {x: 0.5, y: 1}
m_AnchoredPosition: {x: 0, y: -4}
m_SizeDelta: {x: 64, y: 64}     # 🔴 고정 크기 — 칸을 안 따라간다
```

이 값은 **TAB 스탯 창의 76x96 칸**(아이콘 위 · `Lv.N` 글자 아래)에 맞춘 것이다.
HUD 줄은 칸이 **50x50** 이므로 아이콘이 **칸보다 14px 크고**, 위쪽 정렬이라 아래로도 흘러넘친다.
⇒ 아이콘이 테두리를 **덮어 그린다.** 사용자가 본 게 정확히 그것이다.

🔑 **`D90` 에서 테두리를 자식으로 분리했기 때문에 이 결함이 비로소 드러났다** —
그 전에는 아이콘이 테두리를 아예 대체해 버려서 *"덮는다"* 는 현상 자체가 없었다.

> 🔴 **정정** — 위 문장은 *"테두리를 덮는다"* 는 **현상의 시작점**일 뿐이다.
> **넘침 자체는 훨씬 오래됐다.** 병렬 세션의 [`BUGS.md` B16](Parallel/BUGS.md) 이
> 같은 결함을 `/screen-check` 확대본에서 **따로 잡았고**, 거기 서술이 더 정확하다 —
> 칸이 **36** 이던 시절엔 사방 **14px** 로 **더 심했다.** `D87`(36→50)도 `D90`(테두리 분리)도
> **만든 게 아니라 드러냈을 뿐이다.** 그때는 아무도 화면을 못 봤다.

⇒ 칸에 맞춰 늘리고 안쪽 여백 **8** 을 준다. 칸 50 · 테두리 4 이므로 구멍이 42 고,
아이콘은 **34x34** 로 테두리에서 4px 떨어져 앉는다. `preserveAspect = true` 로
세로로 긴 그림이 눌리지 않게 한다.

### 🔑 `SetCompact` 를 "HUD 줄이냐" 스위치로 승격

*"TAB에는 두지 말고"* 를 처리하려면 **같은 프리팹을 쓰는 두 화면을 갈라야** 한다.
새 플래그를 만들지 않았다 — `SetCompact(bool)` 는 **이미 `HUDManager` 만 부르고 있었다.**

| | `SetCompact` | 테두리 | 아이콘 |
|---|---|---|---|
| HUD 줄 | `true` | **그린다** | 칸에 맞춰 늘림 + 여백 8 |
| TAB 스탯 창 | 안 부름 | **안 그린다** | 프리팹 그대로 (64x64 · 76x96 칸에 맞다) |

### 검증 로그 (플레이 모드 · 화면 856x498)

```
WeaponRow[0]   Chip_Sword | Frame 22.3x22.3 | Icon 15.2x15.2 | 여백 L3.6 R3.6 B3.6 T3.6 -> 안쪽
PassiveRow[0]  Chip_Empty | Frame 22.3x22.3 | Icon 15.2x15.2 | 여백 L3.6 R3.6 B3.6 T3.6 -> 안쪽
BuildingRow[0] Chip_Empty | Frame 22.3x22.3 | Icon 15.2x15.2 | 여백 L3.6 R3.6 B3.6 T3.6 -> 안쪽
[HUD] 아이콘이 테두리 안 : 13/13

[TAB] SetCompact 없이 Bind -> Frame 안 생겼다 (요구대로) · 자식 2개
      자식[0] Icon
      자식[1] Label
[TAB] 임시 오브젝트 제거 · ItemGrid 자식 0개
ItemChipUI.Version = 2
```

**판정 3/3** — ① HUD 13/13 이 사방 여백 3.6px 로 테두리 안쪽
② TAB 경로를 **실제로 태워** 자식이 `Icon`·`Label` 둘뿐임을 확인 ③ 임시 오브젝트 제거.

### 🔴 판정 방법은 `B16` 쪽이 더 낫다

여기서는 `RectTransform` 좌표만 비교했다. 그건 *"의도한 사각형이 안에 있나"* 이지
***"그려진 픽셀이 안에 있나"*** 가 아니다. `D88` 은 **실제 렌더 픽셀**을 세고
**빈 칸 3개를 대조군**(0px)으로 썼으며, 🔴 **첫 측정이 안티에일리어싱 때문에
빈 칸까지 22px 로 나온 것을 그 대조군이 잡았다.** 다음에 UI 넘침을 판정할 때는 그 방식을 쓴다.

### 🔵 같은 프리팹을 쓰는 화면이 **넷**이다

`B16` 은 *"레벨업 카드·TAB 격자"* 라고 적었는데 실제로 guid 를 참조하는 곳은 이렇다
(**레벨업 카드는 이 프리팹을 안 쓴다** — `BUGS.md` 쪽도 같이 정정했다).

| 필드 | 스크립트 | `SetCompact` | 이번 변경 |
|---|---|---|---|
...
⇒ `B16` 이 *"여백을 얼마로 둘지 다른 화면도 같이 보고 정해야 한다"* 고 남긴 걱정은
**범위를 HUD 로 좁혀서 해소됐다.**

🔴 **`[TAB] ItemGrid 자식 0개` 만 봤으면 공허한 판정이었다** — 칩이 아직 안 만들어졌으니
"Frame 가진 것 0" 은 당연하다. **`SetCompact` 를 안 부른 칩을 만들어 `Bind` 를 태워야** 판정이 된다.

---

## 2-116. ✅ **`Image` 한 장은 두 일을 못 한다 — 테두리를 자식으로 뽑았다** (D90, 2026-09-06 111차)

> 아이템을 먹어도 테두리는 있게

### 🔴 아이콘을 그리는 순간 테두리가 사라졌다

`D86` 에서 빈 칸 테두리를 만들 때 **`icon` 한 장에 두 일을 시켰다.**

| 상태 | `icon` 이 그리던 것 |
|---|---|
| 빈 칸 | `FrameSprite` · `Type.Sliced` · 분류색 → **테두리** |
| 찬 칸 | `item.Icon` · `Type.Simple` · 흰색 → **아이콘** |

같은 `Image` 컴포넌트라 **둘을 동시에 못 한다.** 그래서 아이템이 들어오는 순간
테두리가 사라졌고, 줄 안에서 **찬 칸만 테두리 없이 떠 있었다.**

⇒ **겹쳐 그릴 것이 둘이면 `Image` 도 둘이어야 한다.**
`Frame` 이라는 자식 `Image` 를 런타임에 한 번 만들어 재사용한다(**애셋 0개** 유지).

```csharp
rt.anchorMin = Vector2.zero;   // 칸 전체로 늘린다
rt.anchorMax = Vector2.one;
rt.SetAsLastSibling();         // 아이콘 위에 얹는다 — 가운데가 뚫려 있어 그림을 안 가린다
_frame.raycastTarget = false;  // 클릭을 먹지 않는다
```

### 🔑 알파로 "찬 칸 / 빈 칸"을 갈랐다

테두리가 모든 칸에 생기면 *"몇 칸 남았나"* 가 안 보인다. 같은 색을 **알파로만** 가른다.
...
빈 칸의 `icon` 은 이제 테두리 노릇을 안 하므로 **알파 0 으로 지운다** —
🔴 `sprite = null` 로만 두면 Unity 가 **흰 쿼드**를 그린다(D86 에서 겪은 것).

### 검증 로그 (플레이 모드 · 화면 856x498)

```
WeaponRow[0] Chip_Sword | Frame 22.3x22.3px sprite=ItemChipFrame type=Sliced
             col=RGBA(0.450, 0.720, 1.000, 0.950) sib=2/2 | Icon Sword a=1.00
WeaponRow[1] Chip_Empty | Frame ... col=RGBA(0.450, 0.720, 1.000, 0.550) | Icon null a=0.00
PassiveRow[*]           | Frame ... col=RGBA(0.450, 0.920, 0.600, 0.550) | Icon null a=0.00
BuildingRow[*]          | Frame ... col=RGBA(1.000, 0.720, 0.350, 0.550) | Icon null a=0.00
=> 활성 칸 13 (찬 칸 1 · 빈 칸 12) · 테두리 있는 칸 13 -> 전부 있다
ItemChipUI.Version = 1
```

**판정 4/4** — ① 활성 13칸 **전부** `Frame` 보유 ② 찬 칸이 테두리(0.95)**와** 아이콘(a=1.00)을 **같이** 그린다
③ 빈 칸의 `Icon` 이 `a=0.00`(흰 쿼드 없음) ④ `Frame` 형제 순서가 **2/2 = 맨 뒤**라 아이콘 위에 얹힌다.

🔵 같은 프리팹을 쓰는 **TAB 스탯 창의 아이템 격자에도** 같은 테두리가 생긴다 — 의도한 부수효과다.

---

## 2-115. ✅ **틈만 재면 안 됐다 — 테두리를 포함한 덩어리로 봐야 했다** (D89, 2026-09-06 110차)

`D87` 직후 사용자가 게임 화면 스크린샷과 함께:

> 아직도 딱 붙은것처럼 보이잖아 조금 간격을 띄워

### 🔴 4.5px 는 통과선을 넘겼는데 눈에는 안 통했다

`D87` 은 가로 틈을 **1.8px → 4.5px** 로 늘리고 통과라고 적었다. 그런데
`D88` 이 세운 통과선(**20px**)은 *"어떤 것이 보이는가"* 의 기준이지
*"두 것이 떨어져 보이는가"* 의 기준이 아니다. **다른 질문에 같은 자를 댔다.**

칸 하나하나에는 **1.8px 테두리**가 붙어 있다. 그래서 두 칸 사이에 실제로 보이는 것은

```
테두리 1.8px │ 틈 4.5px │ 테두리 1.8px   =  8.1px 안에 선이 두 개
```

이고, 이 폭에서 안티에일리어싱까지 얹히면 **한 줄의 굵은 격자선**으로 뭉친다.
사용자가 본 것은 실제로 **표**였다. ⇒ 붙어 보이는지를 판정할 때는
**틈만 재지 말고 테두리를 포함한 덩어리로 본다.**

### 변경한 값

| 무엇 | 전 | 후 | 화면(856x498) |
|---|---|---|---|
| `HorizontalLayoutGroup.spacing` | 10 | **22** | 4.5 → **9.8px** |
| 줄 간격 (칸 50 고정) | 64 | **76** | 세로틈 6.2 → **11.6px** |
| 줄 폭 | 500 | **560** | `8x50 + 7x22 = 554` |

줄 폭을 같이 늘려야 한다 — 한 줄 최대는 Mage 패시브 **8칸**이라
간격만 벌리면 554 가 500 을 넘어 마지막 칸이 잘린다.

이제 틈(9.8px)이 칸(22.3px)의 **44 %** 라 덩어리로 뭉치지 않는다.

### 검증 로그 (플레이 모드 · 화면 856x498)

```
WeaponRow   칸 3 · 크기 22.3px · 가로틈 9.8px · 줄 전체폭 86.5
PassiveRow  칸 5 · 크기 22.3px · 가로틈 9.8px · 줄 전체폭 150.7px · 위 줄과 11.6px
BuildingRow 칸 5 · 크기 22.3px · 가로틈 9.8px · 줄 전체폭 150.7px · 위 줄과 11.6px
HUDManager.Version = 6
```

**판정 3/3** — 세 줄 모두 가로틈 **9.8px**(전 4.5) · 세로틈 **11.6px**(전 6.2) · 8칸 총폭 554 ≤ 560.

🔑 **이 건은 사용자 스크린샷이 없었으면 또 통과로 적었을 것이다.**
`D86`(뜬다 ≠ 보인다) · `D87`(겹침 ≠ 틈) 에 이어 **세 번 연속으로 판정 대상이 틀렸다.**
셋의 공통점은 **내가 정한 지표가 사용자의 질문과 달랐다**는 것이다.

---

## 2-114. ✅ 참고 스크린샷 2건 — **박스가 붙어 보인 건 간격이 아니라 테두리였다** (D87, 2026-09-06 109차)

사용자가 다른 게임(대전략) 스크린샷 두 장을 가져와 요구했다.

> 아직도 박스끼리 겹쳐 내가 의도한 아이템, 패시브, 건물 보유 보여주는 박스를 다른게임의 스크린샷을 찍어왔어
> TAB 눌렀을때 캐릭터에 대한 정보 뜨는것도 2번째 스크린샷 같은 느낌으로

### 🔴 원인 — 9-슬라이스 테두리가 **12.5배**로 그려지고 있었다

`D86` 에서 빈 칸을 *"흰 박스 말고 테두리만"* 으로 바꿀 때 8x8 텍스처의 가장자리 1px 만
불투명하게 칠하고 `Sprite.Create(..., pixelsPerUnit: 8, border: (1,1,1,1))` 로 만들었다.
**PPU 를 텍스처 크기와 같게 준 것이 실수였다.**

9-슬라이스의 테두리는 텍스처 픽셀 그대로가 아니라 이렇게 그려진다:

```
그려지는 테두리 = border x (Canvas.referencePixelsPerUnit / sprite.pixelsPerUnit)
```

이 프로젝트의 캔버스는 `referencePixelsPerUnit = 100` 이다. 그래서

| | 값 |
|---|---|
| 텍스처의 테두리 | 1 px |
| 배율 | 100 / 8 = **12.5** |
| 실제로 그려진 테두리 | **12.5 캔버스px** |
| 칩 크기 | 36 캔버스px |
| 남은 구멍 | 36 − 12.5 x 2 = **11 px** |

**테두리가 칩의 70 % 를 먹어 사실상 꽉 찬 박스였다.** 사용자가 *"박스끼리 겹쳐"* 라고 한 것은
정확한 묘사였다 — 칸과 칸이 각각 거의 solid 라 사이의 틈이 보이지 않았다.

⇒ `Sprite.Create` 의 PPU 를 **100**(캔버스 기준과 같게), 텍스처를 16x16 · 테두리 4px 로.
이제 배율이 1.0 이라 **텍스처에 칠한 만큼이 그대로 그려진다.**

### 🔴 그리고 `D86` 의 판정이 틀린 것을 잰 것이었다

`D86` 은 *"겹치는 쌍 **0**"* 으로 통과라고 적었다. 겹침은 실제로 0이었다.
**재야 할 것이 겹침이 아니라 틈이었다.**

| | 캔버스 | 화면(856x498) |
|---|---|---|
| 칸 사이 간격 | 4 px | **1.8 px** |
| 줄 사이 여백 | 16 px | 7.2 px |

**1.8px 는 사람 눈에 "붙어 있다" 다.** `D86` 의 화살표 건(*"뜬다"와 "보인다"는 다르다*)과
**같은 종류의 실수를 같은 커밋 안에서 두 번** 한 셈이다.
⇒ 이번 판정은 전부 **화면 픽셀**로 했다.

### 변경한 파일

| 파일 | 무엇 |
|---|---|
| `Assets/Scripts/UI/ItemChipUI.cs` | `FrameSprite` 를 **PPU 100 · 16x16 · 테두리 4px** 로. `BindEmpty()` 가 `ItemCategory` 를 받아 **분류색 테두리**를 칠한다(α 0.30 → 0.55) |
| `Assets/Scripts/UI/HUDManager.cs` | `SlotChipSize` 36 → **50**. `BindEmpty(cat)` 로 분류 전달. `Version` 5 |
| `Assets/Scripts/UI/StatsPanelUI.cs` | TAB 창을 **2단**으로. `statsRightText` 신설 · `<pos=72%>` 값 정렬 · 금색 구역 제목 · `Style()` 로 **코드가 배치를 정한다**(B11). `Version` 3 |
| `Assets/Scenes/SampleScene.unity` | 3줄 높이 50 · 줄 간격 64 · `HLG.spacing` 4 → **10**. `StatsPanel/Body` 560 → **700**, 단 300x340 두 개 + `ColumnDivider` |

### 🔑 분류색 테두리 — 참고 이미지가 가르쳐 준 것

참고 스크린샷의 칸은 **테두리 색으로 종류가 갈린다.** 우리도 줄이 셋인데
빈 칸이 전부 같은 흰색이라 *"이 줄이 무기인지 건물인지"* 를 아이콘 없이는 알 수 없었다.
이미 `ItemChipUI` 에 있던 `WeaponTint`/`PassiveTint`/`BuildingTint` 를 빈 칸에도 쓴다 — **새 상수 0개.**

| 분류 | 색 |
|---|---|
| Weapon | `(0.45, 0.72, 1.00)` 파랑 |
| Passive | `(0.45, 0.92, 0.60)` 초록 |
| Building | `(1.00, 0.72, 0.35)` 주황 |

### 검증 로그 (플레이 모드 · 화면 856x498)

```
[1] WeaponRow   칸 3 · 높이 22.3px · 가로 최소틈 4.5px
[1] PassiveRow  칸 5 · 높이 22.3px · 가로 최소틈 4.5px · 위 줄과의 세로틈 6.2px
[1] BuildingRow 칸 5 · 높이 22.3px · 가로 최소틈 4.5px · 위 줄과의 세로틈 6.2px
[1] 칩 'Chip_Empty' img=ItemChipFrame type=Sliced col=RGBA(0.450, 0.920, 0.600, 0.550)

[1] border now    = 4 canvasPx -> screen 1.8px ; hole = 42 canvasPx
[1] border BEFORE = 12.5 canvasPx -> on a 36px chip hole = 11px (almost solid)
[1] 8 slots width 470 / room 500 -> FITS

[2] 왼쪽   단 @25 필요 285 / 자리 340 -> 들어간다 · 값 위치 216px / 라벨 최대 151px
[2] 오른쪽 단 @25 필요 316 / 자리 340 -> 들어간다 · 값 위치 216px / 라벨 최대 152px
[2] 배선된 statsRightText = StatsRightText
StatsPanelUI.Version = 3 · HUDManager.Version = 5
```

**판정 7/7** — ① 가로틈 1.8 → **4.5px** ② 세로틈 6.2px ③ 칸 16 → **22.3px**
④ 테두리 구멍 11 → **42px** ⑤ 빈 칸이 분류색 ⑥ 두 단 다 안 넘침 ⑦ 라벨(152) < 값 위치(216) 이라 겹치지 않는다.

🟡 **화면 확인은 사용자 몫으로 남는다** — `ScreenSpaceOverlay` 는 여전히 내가 못 찍는다.
`D88` 이 만든 [`/screen-check`](../.claude/skills/screen-check/) 스킬이 그 수단인데
**아직 실캡처 0회**(권한 미승인)라 이번에도 쓰지 않았다.

---

## 2-113. ✅ 화면을 내 눈으로 본다 — **`/screen-check` 스킬** (D88, 2026-09-06 108차)

**한 줄:** 사용자에게 스크린샷을 요청하던 자리를 **데스크톱 캡처**로 대체했다. 게임 코드 변경 **0**.

### 원인 — UI 화면은 원리적으로 못 찍히고 있었다

두 제약이 겹쳐서 **오버레이 UI 를 볼 수단이 없었다**:

| | |
|---|---|
| 기록 #120 | `UI Canvas` 는 `ScreenSpaceOverlay`. `Unity_Camera_Capture` 는 카메라를 렌더 타깃에 그리므로 **오버레이가 한 픽셀도 안 나온다** |
| 기록 #114 | `cam.Render()` / `ScreenCapture` 는 **에디터에 포커스가 없어 프레임이 안 돈다** — 같은 그림만 반복해서 나왔다 (파일 크기가 전부 같아서 알았다) |

그래서 #114 는 *"화면은 사용자 눈에 맡긴다"* 로 끝나 있었다.
**그 대가가 실제로 세 번 나왔다** — 전부 *숫자는 통과한* 건이다:

- `D56` — 대비가 3.1배 좋아졌다. 그런데 `Image.color` 는 **곱셈**이라 실루엣이 되지 않았다.
  **캡처를 안 봤으면 개선이라고 보고했을 것이다**
- `D86` — `activeSelf == true` 라 "화살표 뜬다"고 판정했다. 게임뷰에서 **19px** 이라 안 보였다
- `D85` — 숫자가 통과한 것을 **사용자가 화면으로 잡았다** (세 번째)

### 고친 것 — 세 번째 경로를 열었다

`computer-use` MCP 로 **데스크톱에 합성된 진짜 픽셀**을 찍는다. 두 제약을 동시에 우회한다:
오버레이는 이미 합성돼 있고, 창이 앞으로 오므로 프레임도 돈다.

| 파일 | 내용 |
|---|---|
| `.claude/skills/screen-check/SKILL.md` 🆕 | 캡처 2경로 · 상태 고정 절차 · 판정 규칙 4개 · 함정 표 · 뒷정리 |
| `Docs/SETUP_STATUS.md` | 기록 **#114 · #120 정정** — 옛 결론을 읽고 시도조차 안 하는 걸 막는다 |

🔑 **두 경로를 남겨 뒀고, 보는 것이 다르다.**

| | A · 데스크톱 캡처 | B · `renderMode` 치환 |
|---|---|---|
| 게임뷰 **실해상도** | ✅ 그대로 | ❌ 렌더 타깃 크기 (거짓) |
| 사용자 승인 | 세션당 1회 | 불필요 |

⇒ **`D86` 의 19px 버그는 A 로만 잡힌다.** B 는 렌더 타깃 크기로 찍혀 재현조차 못 한다.
B 는 권한이 거부됐을 때의 차선책으로만 쓴다.

### 🔴 환산식은 **틀렸었다** — 그리고 내 검산이 **순환이었다** (2026-09-07 정정)

처음엔 이렇게 적었다: *"환산식이 과거 기록 두 건과 자릿수까지 맞는다."*

```
scaleFactor = 2 ^ ( 0.5·log2(W/1920) + 0.5·log2(H/1080) )   ← 틀린 식
```

| 대조 | 식의 결과 | 과거 기록 |
|---|---|---|
| 게임뷰 856×498 | `0.4532` | `D56` — 0.453 |
| 본문 25pt | 11.33px | `D56` — 11.3px |

**둘이 맞았으므로 검증됐다고 썼다. 아니었다.**

🔴 이 씬의 `CanvasScaler` 는 `m_ScreenMatchMode: 1` = **`Expand`** 다
(`MatchWidthOrHeight` 는 **0**). 즉 옆에 있는 `m_MatchWidthOrHeight: 0.5` 는
**쓰이지도 않는 값**인데, 나는 **그 값을 보고 모드를 짐작해** 다른 모드의 식을 썼다.

```
Expand → min(W/1920, H/1080) = min(0.4458, 0.4611) = 0.4458   ← 실제
```

🔑 **그런데 과거 기록도 0.453 이었다.** 856×498 을 `Expand` 로 계산하면 0.4458 이 나오므로,
**그 기록 역시 같은 잘못된 식으로 계산된 값**이라고 보는 게 맞다.
⇒ **나는 같은 실수끼리 맞춰 놓고 "두 건이 검증한다"고 적었다.**
서로 독립이 아닌 두 값을 대조하면 **틀린 것도 통과한다.**

**고친 방법 — 계산하지 말고 런타임 값을 읽는다.**

```csharp
float f = Object.FindAnyObjectByType<Canvas>().rootCanvas.scaleFactor;
```

모드가 무엇이든 이 질문 자체가 사라진다. 통과선 **20px** 은 그대로 둔다
(`D86` 이 19px 에서 실패했고 96px 로 고쳐 통과했다 — 1.7 % 차이는 이 판정을 안 뒤집는다).

> 🔑 **이걸 어떻게 잡았나 — 스킬 평가에서 나왔다.** 독립 에이전트 **둘이 각각** 씬 YAML 을
> 읽고 같은 지적을 했다. 한 명은 스킬을 준 쪽, **한 명은 안 준 쪽(대조군)** 이었다.
> ⚠️ 그리고 같은 에이전트가 `BossArrowGraphic` 삼각형이 **자기 사각형의 30 % 만 칠한다**는
> 것도 짚었다 — **rect 크기는 잉크 양이 아니다.** 둘 다 `SKILL.md` 규칙 1 에 반영했다.

### ✅ 첫 실사용 (같은 날 · 사용자 지시로 `D87`·`D89`·`D90` 판정)

**경로 A 는 못 썼다** — 권한이 두 번 다 거부됐다. 스킬대로 되묻지 않고 **B 로 갔다.**

🔴 **그런데 B 를 그대로 쓰면 안 됐다.** `Unity_Camera_Capture` 는 **1920×1080** 을 뱉는데
실제 게임뷰는 **856×498** 이다 — 그 그림으로 *"붙어 보이나"* 를 판정하면 **2.2배 확대판**을
보는 것이라 `D86` 이 저지른 것과 **정확히 같은 실수**가 된다.

🔑 **그래서 B 를 고쳤다** — `Camera.targetTexture` 에 **`Screen.width×Screen.height` 크기**
RenderTexture 를 물리고 `Render()` → `ReadPixels` → PNG. 이제 B 도 **실해상도로 나온다.**
`Camera_Capture` 툴에는 크기 인자가 없어 이 방법밖에 없다. → `SKILL.md` §B 에 반영

| 판정 | 결과 |
|---|---|
| 칸 21px · 가로틈 11px · 세로틈 11~12px · 피치 32px | ✅ 세 줄 전부 일치 (`D89` 주장과 같다) |
| 붙어 보이나 | ✅ **아니다** — 틈이 칸의 **52 %** |
| 분류색 3종 구분 | ✅ 파랑/초록/주황 (`x` 자리 55.5 로 세 줄 동일) |
| 찬 칸에도 테두리 (`D90`) | ✅ 13칸 전부. 밝기 **찬 164~172 / 빈 130~136** 으로 갈린다 |
| **아이콘이 테두리 안에 있나** | 🔴 **아니다 → [`BUGS.md` B16`](Parallel/BUGS.md)** · 🟢 **`D91` 에서 고쳤다**(2-117) — 아이콘을 칸에 맞춰 늘리고 여백 8 |

🔴 **`B16` 은 지적 밖에서 나왔다.** `chip 50x50 | icon 64x64` — 아이콘이 칸보다 크다.
`D87` 이 만든 게 아니라 **드러낸 것**이다(36 이던 시절엔 사방 14px 로 더 심했다).

🔑 **첫 측정은 오진이었다.** 테두리 AA 를 안 걸러 **빈 칸까지 22px 씩** 넘치는 걸로 나왔다.
**빈 칸을 대조군으로 놓고**(아이콘이 없으니 0 이어야 한다) 2px 안쪽을 버리고서야
찬 칸 1~6 · 빈 칸 **0·0·0** 이 나왔다 — 대조군이 없었으면 그대로 적었을 것이다.

### 스킬 평가 (2026-09-07 · 사용자 요청 *"성능 체크해봤었나?"*)

**답은 "안 했다"** 였다. 한 번 써 본 게 전부였다. 그래서 실사용에서 막힌 자리 넷
(`result.Log` 3개 제한 · `Image` 이름 충돌 · `global::` · 대조군 규칙)을 먼저 스킬에 박고 쟀다.

테스트 3건 × (스킬 있음 / 없음) = 6, 그리고 트리거 심사자 3명.
🔴 **Unity 에디터가 하나뿐이라 6개를 동시에 붙일 수 없다** — 플레이 모드·`timeScale`·
`renderMode` 를 두고 싸운다. 그래서 **실행이 아니라 "어떤 절차와 코드를 쓸 것인가"** 로 쟀다
(내가 낸 실패 넷이 전부 *작성* 단계 실수라 이 방식으로 잡힌다).

| 측정 | 값 | 유효한가 |
|---|---|---|
| `with_skill` 절차 완주 | **32/32 (100 %)** | ✅ |
| `without_skill` | 28/32 (88 %) | 🔴 **무효 — 오염** |
| 트리거 정확도 | 20/20 · 심사자 이견 0 | ⚠️ 유효하나 **변별력 0** |

🔴 **with/without 비교는 무효다.** 대조군 셋이 **전부 `SKILL.md` 를 파일로 읽었다**
(언급 9·4·2회). `.claude/skills/` 가 저장소 안에 있는데 *"프로젝트 파일 읽기는 자유"* 라고 했다.
**한 명만 자진 신고했고 나머지 둘은 말도 안 했다** — 부탁으로 막을 수 없다는 뜻이다.
그리고 **시험 도중에 내가 `SKILL.md` 를 고쳤고** 대조군 하나가 그 갱신을 집어갔다.

🔴 **트리거 20/20 도 좋은 소식이 아니다.** 독립 심사자 셋이 20문항에서 **한 번도 안 갈렸다면**
문제가 쉬웠던 것이다. 통과율이 아니라 **변별력이 0** 이라 *"설명문이 좋다"* 가 아니라
*"이 시험으로는 알 수 없다"* 까지만 말할 수 있다.

### 🔑 그래도 평가가 벌어 준 것 — **내 오류 셋** (숫자보다 이쪽이 성과다)

1. **환산식이 틀렸다** → 위 절에서 정정 (검산이 순환이었다)
2. **rect 크기 ≠ 잉크 양** → `SKILL.md` 규칙 1 에 반영.
   ⚠️ `D86` 의 *"96px → 43.5px 통과"* 는 사각형 기준이고 잉크 기준 등가 한 변은 **약 21px** —
   **"화살표가 안 보인다"가 아직 안 닫혔을 수 있다** → [`TODO.md`](TODO.md) §1
3. **채점기가 두 번 틀렸고 둘 다 *올바른 답에 벌점*을 줬다** —
   RT 방식에 `cameraInstanceID` 를 요구했고, 산문 속 `Image` 를 코드로 오인했다.
   고칠 때마다 baseline 이 올랐다 ⇒ **첫 숫자는 스킬을 과대평가하고 있었다.**

> 🔑 **평가의 값어치는 점수가 아니라 여기 있었다.** 100 % 는 아무것도 안 알려 줬고,
> 틀린 것 셋은 전부 **점수 밖에서** 나왔다.

### 🟡 남은 것

- **다시 재려면 대조군을 `.claude/` 없는 사본에서 돌려야 한다.** 부탁으로는 못 막는다.
  그리고 **평가 중에 스킬을 고치지 않는다** (계기를 바꾸면 그 회차는 버린다)
- **경로 A(데스크톱 캡처)는 여전히 0회.** Unity 창 tier 도 모른다
- 🔴 **TAB 정보창(`D87` 2단)은 B 로 못 열었다** — `StatsPanelUI.TryOpen` 이 `private` 이고
  `timeScale == 1` 을 요구하는데, 리플렉션은 금지고 키 주입은 기록 #114 로 막혀 있다.
  **A 였다면 진짜 TAB 을 누를 수 있었다** — 두 경로의 차이가 여기서 처음 실물로 드러났다

> ℹ️ **스킬은 세션 시작 시 로드된다.** 이 세션에서는 `/screen-check` 가 안 뜬다 — 새 세션부터 잡힌다.

---

## 2-112. ✅ 두 번째 스크린샷 묶음 — **"뜬다"와 "보인다"는 다르다** (D86, 2026-09-07 107차)

**한 줄:** 사용자가 다섯 가지를 더 짚었고, 그중 하나는 **진짜 버그**였다.

### 🔴 ③ 상점 판매 — 확인 버튼이 죽어 있었다

사용자: *"상점 보유 아이템 판매 작동 안하는거 같아"*.

```
Prefab_ShopRemoveRow
  confirmOverlay = ConfirmOverlay      (배선됨)
    └ ConfirmBtn                        ← 🔴 아무도 리스너를 안 붙였다
ShopRemoveRowUI 는 removeButton 만 잇는다
```

X 를 누르면 확인 오버레이가 뜨는데 **그 안의 버튼은 눌러도 아무 일이 없었다.**

🔑 **더 나빴던 건 그 다음이다** — 코드상 유일한 판매 경로가 *"X 를 한 번 더 누르는 것"* 이었다.
즉 **취소하려고 X 를 다시 누르면 팔렸다.** 의도와 정반대로 동작하고 있었다.

⇒ 확인 버튼을 잇고, **X 를 다시 누르면 취소**로 바꿨다.

### 🔴 ④ 화살표 — "뜬다"는 맞았고 "보인다"는 틀렸다

`D85` 에서 내가 *"거리 9.0 에서 뜬다 ✅"* 로 판정했는데 사용자는 **또** 안 보인다고 했다.
값을 다시 재니 전부 정상이었다:

```
edgeMargin 56 · arrowSize 44 · 색 (0.90,0.24,0.24,0.92) · 맥동 1.6 · showBeyondDistance 7
```

🔑 **둘 다 맞았다.** 화살표는 뜨고 있었고 — **캔버스 1920 기준 44px 인데
사용자 게임뷰가 약 850px 이라 화면에서 19px 이었다.** 어두운 배경 가장자리의 19px 삼각형은
떠 있어도 안 보인다.

🔴 **내 판정이 `activeSelf` 만 봤지 "사람 눈에 띄는 크기인가" 는 안 봤다.**
*"기능이 도는가"* 와 *"쓸 수 있는가"* 는 다른 질문이다.

⇒ `arrowSize` **44 → 96**(화면 약 42px) · `edgeMargin` 86 · 맥동 2.4 · 더 밝은 빨강.
...
줄 간격 44 (−92 / −136 / −180)
행 높이 40
칩 크기 44          -> 🔴 행보다 4 크고, 줄 간격과 같다 (여백 0)
```

칩이 **행을 위아래로 넘치는데 줄 간격이 칩과 같아서** 붙어 보였다.
⇒ 칩 **36** · 줄 간격 **52** (여백 16). 판정 — 켜진 칩 13개, **겹치는 쌍 0**.

### ② 흰 박스 → 테두리만

사용자: *"아직 안먹은 박스 흰박스로 두지 말고 테두리만 해둬"*.

🔑 **원인은 Unity 의 기본 동작이다** — `Image.sprite` 가 `null` 이면 **흰 쿼드를 통째로 그린다.**
내가 알파만 0.10 으로 낮췄으니 *"옅은 흰 박스"* 가 됐다.

⇒ **8×8 텍스처의 가장자리 1px 만 불투명**하게 칠하고 9-슬라이스 `border (1,1,1,1)` 로 만든다.
칸이 얼마로 늘어나도 **테두리는 항상 1px** 이고 늘어나는 건 투명한 가운데뿐이다.
🔑 **새 애셋 0개** — `BossSlam` 이 흰 사각형을 만든 것과 같은 수법인데, 여기는 속이 비어야 해서 직접 칠했다.

### ⑤ HELP — 넓히고 키웠다

*"글자 크기 늘리고 좌우로 박스 더 늘려서 더 읽기 쉽게"*.

🔑 **순서가 중요했다** — 폭이 340 인 채로 글자만 키우면 줄이 자주 접혀 **오히려 나빠진다.**
카드를 먼저 넓히고(920 → **1240**, 단 폭 340 → **520**) 글자를 21 → **27** 로 올렸다.

🔴 그랬더니 오른쪽 단이 **21px 넘쳤다**(711 / 690). 카드를 900 → **1000** 으로 키워 자리를 790 으로 늘렸다.

### 판정

```
① 켜진 칩 13개 · 겹치는 쌍 0
② 빈 칸 sprite = ItemChipFrame · type = Sliced · a 0.30   (흰 사각형 아님)
③ X 한 번 -> 오버레이 뜸 · 보유 True (아직 안 팔림)
   확인 버튼 -> 보유 False · 런 골드 4 -> 32              팔렸다
⑤ 왼쪽 668 / 오른쪽 711  vs  자리 790                    둘 다 들어간다
```

🔑 **③ 은 "프리팹에 버튼이 있다"와 "그 버튼이 동작한다"가 다르다는 사례다.**
배선 여부만 봤으면 통과였다 — 행을 직접 만들어 **버튼을 눌러 봤기 때문에** 잡혔다.

---

## 2-111. ✅ 스크린샷 5장의 지적 — **내가 만든 겹침을 하나 찾았다** (D85, 2026-09-06 106차)

**한 줄:** 사용자가 실제 화면 5장을 보내 다섯 가지를 짚었다.

### 🔴 ① 메인 메뉴 — `D83` 이 만든 겹침

사용자 지적은 *"버튼이 너무 아래로 많아졌다"* 였는데, 스크린샷을 재 보니 **그것만이 아니었다**:

```
CurrencyText  y[115..165]      "1848 G"
StartButton   y[ 66..144]      -> 29px 겹쳤다 🔴
```

`D83` 이 HELP 버튼을 넣으면서 **버튼 7개만 45px 올리고 `CurrencyText` 는 안 올렸다.**
🔑 *"기둥을 다시 가운데로"* 만 생각했고 **그 위에 있는 것을 안 봤다.**

⇒ Credits 를 **좌하단 구석**으로 빼고(사용자 요구) 기둥을 6개로 줄여 다시 짰다.

| | 전 | 후 |
|---|---|---|
| 기둥 | 7개 · y[-474..144] | 6개 · y[**-438..90**] |
| Currency 와 겹침 | **29** | **0** |
| 아래 여유 | 66 | **102** |
| Credits | 기둥 안 | **좌하단 (28,24) · 160x50** |

### ② HUD 아이템 — 한 줄 13칸 → 분류별 3줄

사용자가 원하는 모양을 그림으로 적어 줬다:
...
`D82` 는 한 줄에 전부 늘어놓아서 **무엇이 무기고 무엇이 건물인지 안 읽혔다.**
⇒ `itemSlotRows[3]`(무기/패시브/건물)로 갈랐다. 판정 — Warrior **3 / 5 / 5**, 전부 상한과 일치.

### ③ 상점 골드 — 카드에 깔려 있었다

`D82` 가 만든 `GoldText` 가 y[-132..-84] 인데 카드가 y −96 부터 시작한다 ⇒ **12px 깔렸다.**
사용자가 *"아래 부분에 공간이 좀 남는데 그 부분을 활용해"* 라고 짚어 준 대로
카드 아래 빈 자리(**y −810..−750**)로 내리고 크기를 34 로 키웠다.

### 🔴 ④ HELP — 첫 시도가 실패했다

*"너무 글씨가 몰려있어"* 라기에 줄간격을 키웠더니 **본문이 카드를 296px 넘쳤다**(필요 1004 / 자리 708).

🔑 **글자를 줄이면 요구와 정반대**가 된다(*"가독성 위주"*). ⇒ **카드의 남는 가로를 썼다** —
카드를 620x720 → **920x900** 으로 넓히고 **2단**으로 나눴다(왼쪽 CONTROLS · 오른쪽 SCREEN + GOOD TO KNOW).

```
1단   필요 1004 / 자리 708   🔴 넘친다
2단   왼쪽 513 · 오른쪽 545 / 자리 708   ✅
```

그리고 표 배치를 버렸다 — 예전에는 **공백으로 칸을 맞춰서** 글자가 조금만 길어져도 붙어 보였다.
지금은 키를 한 줄, 설명을 들여쓰기로 아래에 둔다. 🔴 **자동 축소도 껐다** —
켜져 있으면 본문이 길어질수록 글자가 스스로 작아져 *"몰려 보인다"* 가 다시 생긴다.

### ⑤ 보스 화살표 — 세 번째 지적, 조건을 넓혔다

`D83` 에서 *"깨진 게 아니라 뜰 일이 없다"* 로 결론 내고 `TODO` 에 결정거리로 남겼는데,
사용자가 **또** *"아직도 안보여"* 라고 했다. 스크린샷을 보니 **보스는 화면 안에 있었다** — 작은 점이었다.

⇒ 조건을 하나 더 준다: **멀면(7유닛 초과) 화면 안이어도 띄운다.**
🔑 가까우면 여전히 안 뜬다 — 눈앞의 것을 화살표로 가리키면 그건 방해다.

### 판정

```
① Currency y[115..165] · Start y[12..90]        겹침 0 ✅  (전 29)
  기둥 아래 끝 -438 · 여유 102 · Credits 앵커 (0,0)
② WeaponRow 3/3 · PassiveRow 5/5 · BuildingRow 5/5   전부 일치 ✅
③ HELP  왼쪽 513 · 오른쪽 545 / 자리 708            들어간다 ✅
⑤ 거리 9.0 · 화면 안 True  -> 화살표 뜬다 ✅
  거리 3.0 · 화면 안 True  -> 화살표 안 뜬다 ✅ (대조군)
```

### 🔴 ⑤ 를 재는 데 네 번 실패했다 — 전부 시험 오염이었다

| 시도 | 왜 실패했나 |
|---|---|
| 1 | 보스가 **0.9유닛까지 다가왔다** — 화살표가 꺼진 건 맞는 동작이었다 |
| 2 | `AiSuspended` 를 걸었는데 **물리로 밀려** 1.1유닛까지 붙었다 |
| 3 | `GameState` 가 **`LevelUp`** 이었다 — 화살표는 `Wave` 에서만 뜬다 |
| 4 | 잡몹을 껐는데도 **쌓인 레벨업 큐**가 계속 상태를 바꿨다 |

⇒ `Rigidbody2D.simulated = false` + `timeScale 0` + `ChangeState(Wave)` 로
**세 가지를 동시에 고정**하고 나서야 쟀다.
🔑 **네 번 다 "코드가 틀렸다"가 아니라 "조건이 안 만들어졌다"였다** — 한 번이라도
그 자리에서 *"안 뜬다"* 로 결론 냈으면 멀쩡한 코드를 고칠 뻔했다.

### 🔴 같은 실수를 두 번 했다 — 파이썬으로 C# 문자열 쓰기

`HelpPanel` 의 본문을 파이썬 heredoc 으로 두 번 썼고 **두 번 다 `
` 이 한 겹 벗겨져
실제 줄바꿈이 되어 파일이 깨졌다.** 두 번 다 `Write` 로 다시 썼다.
⇒ **C# 문자열 리터럴이 들어가는 파일은 `Write` 로만 만진다.**

---

## 2-110. ✅ `B14`·`B15` — **"같이 사라진다"는 가정이 틀렸다** (D84, 2026-09-06 105차)

**한 줄:** 사용자 플레이 로그에 있던 런타임 예외 49건. 원인이 하나였고, 세 번째를 더 찾았다.

### 🔴 `B14` 를 등재할 때 나는 "설명이 안 된다"고 적었다

```
_sr 도 _mpb 도 둘 다 직렬화 안 되는 필드이고 Setup() 에서 나란히 채워진다.
그러므로 "Setup 이 안 돌았다" 로는 설명이 안 된다.
```

**빠뜨린 조건은 "플레이 중 재컴파일" 이었다.** 에디터 기본값이 *Recompile And Continue Playing*
이라 플레이 도중 C# 을 고치면 **도메인 리로드**가 돈다. 그때 둘은 **같이 사라지지 않는다**:
...
가드가 `if (_sr == null) return;` **하나뿐**이라 그 상태를 통과했다.
🔑 **"두 필드가 같이 사라진다"가 내 잘못된 가정이었다.**

### 🟡 그래서 성격이 다르다 — 출시를 막는 버그가 아니다

**빌드된 게임에서는 안 난다**(리로드가 없다). 개발 중에만 나지만
**콘솔을 뒤덮어 진짜 에러를 가린다.** 사용자 플레이에서 47건이 난 것도
**그 옆에서 내가 C# 을 고치고 있었기 때문**이다 — 사용자가 뭘 잘못한 게 아니다.

### 고침 — 가드가 아니라 지연 생성

가드를 하나 더 다는 대신 **쓸 때 만드는 프로퍼티**로 바꿨다. **리로드 뒤에도 스스로 낫는다.**
`EnemyBase` 는 이미 이 방식이었다 — **같은 프로젝트 안에 답이 있었다.**
`PlayerVisual` 도 같은 모양이라 **함께** 고쳤다(아직 안 터졌을 뿐이다).

### `B15` — 고침의 핵심은 예외가 아니라 **순서**

```csharp
_collected = true;
ExperienceManager.Instance.CollectXp(_amount);   // ← 여기서 터지면
if (SharedPool != null) SharedPool.Return(...);  // ← 이게 안 돈다
```

`_collected` 가 이미 서 있으므로 다음 프레임엔 바로 돌아간다 ⇒ **구슬이 화면에 영원히 남는다.**
XP 지급을 가드로 감싸 **반납이 예외와 무관하게** 돌게 했다.

### 🟢 재현하다가 세 번째를 찾았다

`PlayerStats.Die()` 가 `GameManager.Instance.OnPlayerDied()` 를 **가드 없이** 부르고 있었다.
`GameManager.Instance` 도 **정적 필드**라 리로드 뒤 null 이 된다.
죽음은 핵심 경로라 여기서 예외가 나면 **정산·클리어 화면이 통째로 안 뜬다**(`I-24` 와 같은 모양).

### 판정 — 원인을 그대로 재현했다

리플렉션이 금지돼 `_mpb` 를 직접 비울 수 없었다. ⇒ **진짜 원인을 만들었다** —
적 9마리가 살아 있는 상태에서 **플레이 중 재컴파일**을 일으켜 도메인 리로드를 통과시켰다.

```
리로드 전   적 9 · EnemyVisual 9
리로드 후   플레이 모드 유지 · ArgumentNullException 0건   ← 고쳐졌다
            (대신 PlayerStats.Die 의 NRE 가 새로 잡혔다 — 위 세 번째)
```

🔑 **가짜 조건으로 재려다 진짜 조건을 만들게 됐고, 그게 더 나은 판정이었다** —
필드를 억지로 비우는 것보다 *"실제로 일어나는 일"* 을 일으키는 쪽이 세 번째 결함까지 잡아 줬다.

---

## 2-109. ✅ 연출·조작 7건 — **세 번 고친 것을 껐고, 원 일곱 개를 사각형 하나로 바꿨다** (D83, 2026-09-06 104차)

**한 줄:** `PLAYTEST_2` 답의 나머지를 전부 처리했다.

### ① 기울기 — **세 번 고치고 껐다**

| 시도 | 무엇을 고쳤나 | 사용자 판정 |
|---|---|---|
| `D65` | `flipX` 보정을 지웠다 (**부호**) | *"아직 반대다"* |
| `D79` | 전단 **기준선**을 밑변으로 (다리가 반대로 밀렸다) | *"너무 세다"* + 6개 직업이 반대 |
| `D83` | **껐다** (`leanAmount 0`) | — |

🔑 마지막 답이 결론이었다: **"좌우 이동에 대해서 애니메이션을 만들어놨는데 굳이 필요할까 싶어"**.
걷기 시트가 이미 방향을 말해 주므로 전단은 **얹는 게 아니라 겹치는 것**이었다.
🔴 셰이더와 기준선은 **그대로 뒀다** — 되살리려면 `leanAmount` 한 값만 올리면 된다.
🔴 코드 기본값과 **씬 값을 같이** 0 으로 했다 (`B11`/`D68` — 씬이 이긴다).

### ② 보스 일직선 — **원 7개 → 직사각형 1개**

사용자 판정: *"일직선으로 안 보인다 (그냥 원 여러 개)"* → *"직사각형의 범위로 새로 만들어"*.

`D73` 은 `BossSlam` 원을 줄 세워 일직선을 만들었다. **줄 지어 있어도 하나의 띠로 안 읽힌다.**

🔑 **보이는 것만 고친 게 아니다 — 판정도 사각형이 됐다.**
예전에는 원 7개가 **각각** 판정해서 **원과 원 사이 오목한 자리가 안전지대**였다.
이제 보이는 모양과 맞는 곳에서 맞는다.

🔑 **애셋을 안 만들었다** — `Texture2D.whiteTexture` 로 1×1 사각형 스프라이트를 만든다.
🔴 **풀 재사용이 함정이다** — 사각형으로 쓴 오브젝트가 다음에 원형 내려찍기로 나오면
사각형인 채로 뜬다. 원본 스프라이트를 캐시해 두고 **양쪽 진입점에서** 되돌린다.

### ③ 승급 파동 — 3연 → **1개**

사용자 판정: *"3개의 파동이 동시에 나와 난잡해 큰 파동 하나면 될거 같아"*.

🔑 **`D75` 가 *"약하다"* 를 "수"로 푼 게 틀렸다 — "크기"로 풀어야 했다.**
`promotePulseCount 3 → 1`, 그리고 `promotePulseGrowth` 의 **뜻을 바꿨다**:
예전에는 *두 번째부터* 곱해지는 값이라 1개면 아무 효과가 없다. 이제 **첫 파동부터** 곱한다(1.25 → **1.6**).
🟢 `promotePulseCount` 를 2 이상으로 올리면 예전 동작이 그대로 돌아온다 — 되돌릴 손잡이를 남겼다.

### ④ F7 / F8 화면 배율 + 도움말

사용자 요구: *"적과 플레이어 크기는 적절한데 화면에 너무 끼는 느낌이라 F7,F8로 화면 배율을 조절할 수 있게"*.

🔑 **적을 줄이는 대신 화면을 넓힌다.** `D79` 가 적을 1.5배로 키운 건 사용자 요구였고
판정도 *"크기는 적절"* 이었다. 좁게 느껴지는 건 크기가 아니라 **보이는 범위**의 문제다.

```
F7 좁게 / F8 넓게 · 한 단계 0.5유닛 · 범위 4 ~ 11 (기본 6)
PlayerPrefs 로 런을 넘어 유지 — 매 판 다시 맞춰야 하면 손잡이가 아니라 성가신 일이 된다
```

🔴 **키 입력은 `Update` 에서 받는다.** `LateUpdate` 는 `target == null` 이거나
`deltaTime == 0`(일시정지)이면 바로 돌아간다 — 거기 넣으면 **메뉴나 일시정지 중에 안 먹는다.**

그리고 요구대로 **도움말 페이지**를 만들었다(`HelpPanel`). 🔑 **새 조작을 만들었으면
그걸 알 방법도 같이 만들어야 한다** — 지금까지 `Z`·`TAB`·`ESC` 도 어디에도 안 적혀 있었다.
본문은 `CreditsPanel` 과 같이 **코드가 정본**이다(`B11`).

### ⑤ 클리어 화면 — 글자에서 아이콘으로

사용자 판정: *"너무 난잡해 — 아이작의 번제 같이 아이콘과 Lv 로 간단하게.
너무 글자를 많이 쓰지 마. 글자 크기도 키우고 거리도 띄워서"*.

🔑 **난잡함의 대부분이 아이템 줄이었다** — 16종이 글자로 늘어서면 이 상자에서 가장 긴 줄이 된다.
아이템을 아이콘 격자로 빼니 글자 상자가 **230 → 150** 으로 줄었고,
그만큼 **글자를 키울 수 있게 됐다**(자동 축소 하한 15 → **22** · 줄 간격 28).

### ⑥ `B13` — 사용자가 고른 **B안**

`RecalculateStats` 가 합산을 **손으로 나열**하고 있었고 그 나열이 **두 번** 사람을 속였다 —
`Luck`(줄곧 빠져 있었다)과 `HpRegen`(`D74` 에서 실측으로 잡았다).

🔴 **고친 자리가 `PlayerStats` 가 아니라 `StatBlock` 이다.** `Luck` 한 줄만 더하는 A안은
5분이면 되지만 **다음 필드에서 또 밟는다.** `StatBlock.Add(a, b)` 로 합산을 한 곳에 모으고,
`Zero()` 바로 옆에 뒀다 — 필드를 더할 때 **하나만 고치면 눈에 띈다.**

### 판정

```
[F7/F8]   시작 6.00 -> 넓게x2 7.00 · 상한 11.00 · 하한 4.00 · PlayerPrefs 6.00 (일치)
          Keyboard.current 있음 · 예외 0
[아이템 줄] 직업 없음 -> 0칸 (멈춤의 원인이던 자리)
          Warrior(3+5+5) -> 켜진 칩 13 / 만든 칩 13 / 기대 13
[직사각형] scale (12.00 x 3.00) · 회전 30.0도 · 스프라이트 = 원형(BossSlamRing) 아님
          정중앙 맞음 · 5유닛 앞 맞음 · 7유닛 앞 빗나감(길이 12 → 반 6)
          옆 1유닛 맞음 · 옆 2유닛 빗나감(폭 3 → 반 1.5)
[클리어]   아이템 칩 켜짐 1 / 보유 1 · 요약 6줄 · 상자 680x150 · 크기 22~34 · 줄간격 28
          본문에 아이템 이름 남았나 = False   ← 글자에서 빠지고 아이콘으로 갔다
[보스 화살표] active=True · 알파 0.84 · 각도 15도 (보스를 25.7유닛 밖으로 옮겼을 때)
```

🔑 **판정 하나가 어긋났는데 틀린 건 코드가 아니라 내 기대값이었다.**
*"회전 무시하고 월드 +x 로 5유닛"* 을 *"맞음"* 으로 적었는데 실제로는 빗나갔다 —
30도 기운 띠에서 그 점은 축에서 `5·sin30° = 2.5` 유닛 벗어나고 반폭은 1.5 다. **안 맞는 게 맞다.**
🟢 오히려 **회전이 판정에 실제로 걸린다는 증거**다 — 회전을 무시하는 사각형이었다면 맞았을 것이다.

### 🟢 보스 화살표는 안 깨져 있었다

사용자 판정은 *"화살표가 안보여"* 였다. 보스를 **25.7유닛 밖으로 옮기니 바로 떴다**
(`active=True` · 알파 0.84 · 각도 15도 — 오프셋 (40,12) 의 16.7도와 맞는다).

🔑 **원인은 코드가 아니라 상황이다** — 보스는 플레이어를 쫓고 카메라는 플레이어를 따라가므로
**보스가 화면 밖으로 나갈 일이 거의 없다.** 화살표는 그때만 뜨게 돼 있다.
⇒ 고치지 않았다. 이건 *"안 뜨는 버그"* 가 아니라 *"뜰 일이 없는 기능"* 이다 → `TUNING` 판단거리.

### 🟡 시험이 두 번 오염됐다

① 레벨업 패널이 `timeScale` 을 0 으로 잡아 보스 코루틴이 멈췄다 →
정상 경로(`OnLevelUpCompleted`)로 닫았더니 그 사이 플레이어가 죽어 `GameOver` 가 됐다.
② 그래서 보스 일직선을 쿨다운으로 기다리는 방식을 버리고 **`InitializeBox` 를 직접 불러** 격리해 쟀다.
🔑 **그게 더 나은 판정이었다** — 쿨다운·페이즈·AI 를 다 통과시키지 않고 *내가 바꾼 부분만* 본다.

### 🔴 내가 에디터를 멈췄다 — **확인할 수 있었는데 단정했다**

F7/F8 을 넣으면서 `UnityEngine.Input.GetKeyDown` 을 썼고, 주석에
*"이 프로젝트는 새 Input System 을 안 쓴다"* 라고 **적어 두기까지 했다.** 틀렸다.

```
ProjectSettings   activeInputHandler: 1      ← Input System 패키지 "전용"
```

이 모드에서 옛 `Input` 은 **호출될 때마다 `InvalidOperationException` 을 던진다.**
그걸 `Update()` 에 넣었으니 **플레이 중 매 프레임** 터졌다 —
에디터가 응답을 멈추고 메모리가 4.2 GB → 4.9 GB 로 계속 늘었다(콘솔에 로그가 쌓인다).

🔑 **한 번만 `grep` 했으면 나왔다** — `DevPanel` 과 `LevelUpManager` 가
**이미 `Keyboard.current` 를 쓰고 있었다.** 새 코드를 쓰기 전에 *"이 프로젝트는 이걸 어떻게 하나"*
를 먼저 보는 것이 규칙인데, 그걸 건너뛰고 **일반적인 방식을 가정**했다.

⇒ `CLAUDE.md` §3 에 규칙으로 올렸다. 코드는 프로젝트 방식(`Keyboard.current`)으로 고쳤다.

🔴 **판정 절차 자체의 교훈이기도 하다** — 지금까지 이 세션은 *"숫자가 통과한 것을 그림이 잡는다"*
를 여러 번 겪었는데, 이번엔 **코드가 컴파일을 통과했고 계산도 다 맞았는데 런타임이 잡았다.**
컴파일 성공은 *"돈다"* 의 증거가 아니다.

### 🔴 계산이 결함을 둘 잡았다

```
① 메뉴 버튼   7번째(HELP)를 그냥 붙이면 y -519 → 화면 아래 끝(-540)에서 21px
              기둥이 90 길어졌으므로 전체를 45 올려 다시 대칭 (위 396 / 아래 66)
② 클리어 격자  한 줄 9칸인데 소지 상한 합은 최대 14 (Mage)
              2줄 x 9 = 18칸으로 바꾸고 버튼을 -196..-124 -> -240..-188 로 내렸다
```

🔑 둘 다 **"최댓값이 몇인가"를 먼저 물어봐서** 나왔다. `D82` 의 아이템 줄과 같은 종류의 실수다.

---

## 2-108. ✅ 상점·HUD 정보 6건 — **비어 있던 것들의 이유가 다 달랐다** (D82, 2026-09-06 103차)

**한 줄:** 사용자가 `PLAYTEST_2` 를 채우고 추가 요구 4건을 적었다. 그중 화면 정보 6건.

### 고친 것

| # | 요구 | 🔴 원인 | 고친 것 |
|---|---|---|---|
| ① | 상점에서 **보유 골드가 안 보인다** | `ShopUI.currencyText` 가 **`HUD/CurrencyText`** 를 가리켰다. 상점을 열면 **HUD 가 통째로 꺼진다** — 글자는 갱신되는데 화면에 없었다 | `ShopRoot/CenterPanel/GoldText` 신설 |
| ② | 상점에 **`-` 가 남아 있다** | `KILLS/TIME/LEVEL` 셋은 껐는데 **`WaveProgressText` 만 켜진 채** `"-"` 를 그렸다 | 껐다 |
| ③ | **상인이 너무 작다** | 270x270 이 900 높이 패널 바닥에 박혀 있었다 | 새 일러스트 + **380x507 (면적 2.6배)** |
| ④ | **리롤 글자 흰색으로** | 🔴 씬을 고쳐도 소용없다 — `ShopUI` 가 **코드로 매번 덮어쓴다** | `RerollOn` 상수를 흰색으로 |
| ⑤ | **초상화 박스가 비어 있다** | 🔴 **그림이 없었다** — HP 4단계 얼굴 `faceHealthy`~`faceCritical` 이 **넷 다 미할당**이라 매번 `sprite = null` 을 넣고 있었다 | 지금 직업의 `Portrait` 로 |
| ⑥ | **보유 아이템을 아이콘으로** | 없던 기능 | HP 바 아래 `ItemSlotRow` 신설 |

### 🔑 ⑤·⑥ 둘 다 **새 애셋이 0개다**

- **초상화** — 직업 10종이 이미 `Portrait` 를 갖고 있다(직업 선택 화면이 쓰는 그 그림).
  얼굴 그림이 나중에 생기면 **그쪽이 우선**이라 코드를 되돌릴 필요도 없다.
  🔴 **캐시하지 않는다** — 승급으로 직업이 바뀌면 옛 얼굴이 남는다.
- **아이템 줄** — 칩은 TAB 스탯 창이 쓰는 `Prefab_ItemChip` 그대로,
  상한은 `PlayerStats.SlotLimit`, 보유 수는 `LevelUpManager.CountOwned`.

🔑 **요구의 핵심은 "가진 것"이 아니라 "남은 자리"였다** — *"상한까지 몇 개 남았는지 확인하기 쉬워"*.
그래서 가진 것만이 아니라 **상한만큼** 칸을 그린다(무기 → 패시브 → 건물 · 빈 칸은 흐린 사각형).
🟢 그 덕에 이 줄이 **`2-E` 의 질문(*"소지 상한이 실제로 도는지 확인해줘"*)에 화면으로 답한다.**

🔴 **매 프레임 다시 만들지 않는다** — (종류 · 레벨 · 상한)으로 만든 **서명**이 바뀔 때만 재구축한다.
이벤트를 안 쓴 이유는 인벤토리를 바꾸는 경로가 **상점 구매 · 레벨업 · 승급 셋**이라
하나만 놓쳐도 줄이 조용히 낡기 때문이다.

### `[Z] Build` 안내를 가운데 아래로

요구: *"중앙 아래로 (맨 아래 말고 중앙에서 아래 2:1 느낌으로)"*.
위:아래 = 2:1 로 나누는 자리가 곧 **바닥에서 1/3** 이다.

🔑 **`D65` 와 안 부딪힌다** — `D65` 가 피하려던 건 *"하단 중앙"* 이 아니라 **플레이어 바로 아래**였다.
바닥 1/3 이면 플레이어(화면 한가운데)보다 충분히 아래라 난전에 안 묻힌다.

🔴 **앵커 자체를 1/3 에 뒀다.** 바닥 앵커에 `y = 높이/3` 을 주면 비율이 바뀔 때 못 따라간다 —
`D81` 에서 겹침을 만든 방식이 정확히 그것이었다.

🟢 그리고 `D81` 이 4px 겹침 때문에 66 → 78 로 올렸던 승급 안내를 **26 으로 되돌렸다** —
건물 안내가 가운데로 갔으니 좌하단 구석을 혼자 쓴다. **그 이유가 사라졌다.**

### 상인 일러스트 임포트

`PPU 1024 · Single · 무압축`으로 기존 상인과 맞췄고 **필터만 `Bilinear`** 로 다르다 —
1086px 그림을 화면 380px 로 줄여 그리므로 `Point` 면 계단이 심하게 진다. 픽셀아트가 아니라 페인팅이다.

### 🔴 계산이 결함을 하나 잡았다 — 아이템 줄이 화면 밖으로 넘칠 뻔했다

칩 프리팹은 **TAB 스탯 창용이라 76x96** 이다. 그런데 상한 합은 직업마다 다르고 **최대 14칸**(Mage)이다.

```
칩 76px 그대로   ->  14 x 76 + 13 x 4 = 1116px 필요   vs  자리 460px   🔴
칩 44px + 아이콘만 ->  14 x 44 + 13 x 3 =  655px 필요   vs  자리 660px   ✅
캔버스 좌표 124..784 · TimerText 는 810 부터  ->  여유 26
```

🔑 **"몇 칸이 최대인가"를 안 재고 만들었으면 Mage 로 플레이할 때만 깨졌을 것이다** —
Warrior(13칸)·Ranger(11칸)로는 안 나타난다. 상한 합의 **최댓값**을 먼저 뽑은 게 그걸 막았다.

44px 에서는 `Lv.N` 이 안 읽히므로 **글자를 껐다**(`ItemChipUI.SetCompact`). 레벨은 TAB 창에서 본다.

### ⚠️ 이 화면은 내가 캡처할 수 없다

`ScreenSpaceOverlay` 라 렌더 텍스처로 안 잡힌다(`D72`·`D81`). 숫자로만 확인했다 —
상인 그림 위 끝(-373)과 대사문 아래 끝(-380) 사이 7px · 좌우 여백 30px.
**실제로 잘 앉았는지는 사람이 봐야 한다.**

---

## 2-107. ✅ 화면을 줄이면 UI 가 겹쳤다 — **값 하나가 원인이었다** (D81, 2026-09-06 102차)

**한 줄:** 사용자가 작은 창의 직업 선택 화면을 스크린샷으로 보냈다.

> 사용자: *"화면 줄이면 UI 끼리 겹치는거 해결해"*

### 🔑 개별 앵커가 아니라 `ScreenMatchMode` 였다

`CanvasScaler` 가 `MatchWidthOrHeight 0.5` 였다. 이 모드는 가로·세로 배율을 **로그 평균**하므로
**화면 비율이 바뀌면 논리 캔버스의 가로가 줄어든다.**

```
16:9   -> 1920 x 1080
1.47   -> 1745 x 1188      <- 사용자 스크린샷
4:3    -> 1663 x 1247
1.25   -> 1610 x 1288
```

그런데 화면들은 **왼쪽 고정 + 오른쪽 고정**으로 짜여 있다. 가로가 줄면 두 기둥이 서로 다가온다:

```
DescBar        왼쪽 앵커    x 70 .. 1250   (가로 고정)
PortraitPanel  오른쪽 앵커  x W-590 .. W-70
   W=1920 -> 1330   여유 80
   W=1745 -> 1155   겹침 95
   W=1663 -> 1073   겹침 177
   W=1610 -> 1020   겹침 230
...
⇒ **`Expand` 로 바꿨다.** 이 모드는 `min(가로배율, 세로배율)` 을 써서
**논리 크기가 1920x1080 아래로 절대 안 내려간다.** 레이아웃이 짜인 공간을 항상 확보하므로
**비율 때문에 생기는 겹침이 원리적으로 0** 이 된다. 남는 공간은 위아래로만 늘어난다(겹침을 만들지 않는다).

⚠️ **대가는 좁은 화면에서 UI 가 더 작아지는 것**이다 — 4:3 에서 배율 0.616 -> 0.533.
겹치는 것보다는 작은 게 낫다.

### 🔴 프리팹 원본은 `ConstantPixelSize · 800x600` 이었다

씬 인스턴스만 `ScaleWithScreenSize · 1920x1080` 으로 오버라이드돼 있었다.
**`B11`·`D43` 과 같은 함정이 그대로 남아 있었다** — 누가 캔버스를 프리팹에서 새로 만들거나
오버라이드를 되돌리는 순간 **UI 스케일링이 통째로 꺼진다.** 프리팹도 같이 맞췄다.

### 곁가지 둘

| 무엇 | 값 | 왜 |
|---|---|---|
| `StageMapPanel/HeaderText` | ±700 -> **±620** | 비율과 무관하게 **16:9 에서도** 재화 표시와 40px 겹쳤다 |
| `EvolutionPromptUI` 안내문 y | 66 -> **78** | 아래 칸이 y26 에서 높이 44 라 **70 까지** 차지한다. 4px 겹쳤다 |

🔑 안내문 겹침은 **둘이 동시에 뜰 때만**(승급 가능 + 건물 설치 가능) 보인다. 그래서 아무도 못 봤다.

### 판정 — 화면을 못 보니 **앵커로 사각형을 다시 풀어** 쟀다

`ScreenSpaceOverlay` 캔버스는 렌더 텍스처로 안 잡힌다(`D72`). 대신 각 요소의
`anchorMin/Max · offsetMin/Max` 로 **주어진 논리 크기에서의 사각형을 계산**해 겹침 쌍을 뽑았다.

```
고치기 전 (MatchWidthOrHeight)
   1920x1080 -> 1920x1080   StageMap 40
    999x680  -> 1745x1188   🔴 ClassSelect 95  + StageMap 127
   1024x768  -> 1663x1247   🔴 ClassSelect 177 + StageMap 169
   1280x1024 -> 1610x1288   🔴 ClassSelect 230 + StageMap 195

고친 뒤 (Expand)
   1920x1080 -> 1920x1080 · 배율 1.000
   2560x1080 -> 2560x1080 · 배율 1.000     (와이드도 정상)
    999x680  -> 1920x1307 · 배율 0.520
   1024x768  -> 1920x1440 · 배율 0.533
   1280x1024 -> 1920x1536 · 배율 0.667
    640x480  -> 1920x1440 · 배율 0.333
   ⇒ 비율에 따라 커지던 겹침 전부 0
```

### 🔴 측정을 한 번 못 믿었고, 검산으로 갈랐다

같은 오브젝트를 두 번 읽었는데 앵커가 달랐다(`aMin(0.5,0)` vs `aMin(0,0)`).
인덱스 순회·`Find`·씬 전수 조사를 **한 번에** 돌려 대조한 결과,
`EvolutionPromptUI` 가 **런타임에 자기 `RectTransform` 을 코드로 다시 잡는다**는 걸 찾았다
(`PromptCorner.Place` · `D65` 가 안내를 좌하단으로 옮기며 넣은 것).
⇒ **그 항목은 씬 값이 정본이 아니다.** 플레이 중에 읽은 값과 편집 모드 값이 다른 게 정상이다.

### ⚠️ 이 작업 중에 발견한 것 — 고치지 않았다

사용자 플레이 로그에서 **런타임 예외 49건**을 찾았다. UI 와 무관한 별개 문제 둘이다.
→ [`BUGS.md`](Parallel/BUGS.md) **`B14`**(`EnemyVisual` 이 매 프레임 예외 · 8초에 47건) ·
**`B15`**(`ExpDrop.Collect` NRE · 구슬이 풀에 안 돌아간다)

---

## 2-106. ✅ Swift 픽업 배선 — **한 줄이었지만, 한 줄이 아닐 뻔했다** (D80, 2026-09-05 101차)

**한 줄:** CONTENT `요청-38`. `D78` 때는 `.meta` 가 없어 막혔고, `C46` 이 풀어 줬다.

### 한 줄

```
Assets/Prefabs/Pickup_Swift.prefab
  m_Sprite  1ebfcf14d8e640d18f513ba367bcceea (Haste)  ->  f2ee2f6f8f865d74e82c6015348d5c07 (Swift)
  m_Color   (0.55, 1.00, 0.45)  그대로 둔다        <- 줍고 나면 뜨는 오라 색과 짝이다
```

🔴 **`m_Color` 를 건드리면 안 된다.** 그림이 **중립(흰~회색)으로 그려져 있어서**
여기서 초록으로 칠하면 **두 번 곱해져 형광**이 된다. 색 보존을 판정에 넣었다.

### 🔑 CONTENT 가 미리 고친 것이 이 작업의 절반이었다

`C46` 이 요청을 쓰다가 `Swift.png.meta` 가 **Unity 기본값**인 걸 찾아 네 항목을 고쳤다:

| | 기본값 | 고친 값 | 그대로 꽂았으면 |
|---|---|---|---|
| `spritePixelsToUnits` | 100 | **2048** | 🔴 **10.24유닛 — 20배 크기** |
| `filterMode` | Bilinear | **Point** | 픽셀아트가 뭉개진다 |
| `spriteMode` | Multiple | **Single** | 슬라이스 없는 멀티 |
| `textureCompression` | 1 | **0** | 5색 아이콘에 압축 아티팩트 |

**내가 그대로 배선했으면 화면 절반만 한 흐릿한 꺾쇠가 떴을 것이고, 나는 "배선했다"고 보고했을 것이다.**
CONTENT 가 판정 ②(*"공속 픽업과 같은 크기일 것. 크면 임포트가 안 먹은 것이다"*)를 넣은 이유가 그것인데,
**넣은 사람이 미리 고쳐서 통과했다.**

### 판정 5/5

```
① 프리팹   m_Sprite = Swift (guid f2ee2f6f…)                 ✅
...
🔴 **첫 캡처가 새까맣게 나왔다.** 픽업 머티리얼이 `Sprite-Lit-Default`(2D 라이트)라
조명 없는 임시 카메라에서는 색이 안 나온다. **대조군이 그걸 증명했다** —
Haste 도 똑같이 검게 나왔다. 게임 카메라로 다시 찍어 판정했다.

🟡 **픽업이 즉시 회수돼 한 번 헛돌았다** — 플레이어 1.6유닛 위에 놓았더니 자석 반경에 들어갔다.
40유닛 밖으로 옮겨 다시 쟀다.

### 🟡 재다가 찾은 것 — **가로 폭만 이속이 튄다** (고치지 않음)

칸 크기는 같은데 **그림이 채운 넓이**가 다르다. 픽업 6종의 알파 경계:
...
🔑 **세로 480px 은 여섯 중 다섯이 지키는 규격**이고 가로도 352~544 에 모여 있는데 Swift 만 864 다.
판정 ②는 *칸 크기*로 재라 했고 그건 통과지만 **눈에는 이속만 유독 넓다.**
다만 *"주축을 90도 틀었다"* 는 의도의 결과일 수 있어 **CONTENT 판단으로 넘겼다** → 요청-56.

---

## 2-105. ✅ 픽셀·몹 크기·바닥 색·기울기 — **사용자가 화면으로 넷을 짚었다** (D79, 2026-09-05 100차)

**한 줄:** `D78` 이 바닥을 고쳤더니 **다음 층위의 문제가 드러났다.**

> 사용자: *"화면 확인해 아직도 너무 픽셀이 커 / 그리고 몹크기도 좀 키워줘"*
> 사용자: *"스샷같이 특정색이 너무 튀어 좀더 자연스러운 배경이 되게 /
> 아직도 좌우 이동할때 반대로 캐릭터의 몸이 움직이는것처럼 보여 다시 고쳐"*

### ① 숫자 하나가 앞 둘을 한꺼번에 설명했다

```
px/유닛     71.7        타일 한 칸 0.500u = 36px
플레이어    1.000u = 72px
Goblin      0.500u = 36px      ← 타일 한 칸과 정확히 같다
```

🔑 **적이 바닥 타일과 같은 크기였다.** 그래서 바닥이 배경으로 물러나지 않고
**적과 같은 층위의 격자**로 읽혔다 — *"픽셀이 크다"* 와 *"몹이 작다"* 는 **한 문제의 두 얼굴**이다.

| | 전 | 후 |
|---|---|---|
| 타일 | 0.500u (36px) | **0.250u (18px)** — PPU 2048→4096 · `Grid.cellSize` 0.5→0.25 |
| 적 | 플레이어의 45~80 % | **65~120 %** (`SizeScale` 전부 ×1.5) |

### 🔴 타일만 줄이면 채우기 비용이 4배가 된다

`Fill()` 은 카메라가 **한 칸**을 넘을 때마다 창 전체를 다시 쓴다.
칸이 절반이면 창의 칸 수가 4배(78×44 → 156×88)가 되고 넘는 횟수도 2배다 — **8배**.

⇒ 기준점을 `refillStep`(8)칸 단위로 **양자화**했다. 다시 채우는 간격이
*"칸 몇 개"* 가 아니라 **"2.0유닛마다"** 로 고정되어 **칸 크기와 무관**해진다.
대가는 창을 `step` 만큼 넓게 잡는 것뿐이다(카메라가 창 중심에서 최대 `step` 칸 벗어나므로).

🔴 **`v / step * step` 으로 쓰면 안 된다** — C# 정수 나눗셈은 0 쪽으로 자르므로
음수에서 위로 튄다(`-1/8*8 = 0`). 그러면 원점 왼쪽·아래에서 기준점이 **매 프레임 흔들려 계속 다시 채운다.**
`Mathf.FloorToInt` 로 막았고, 판정에서 **음수 사분면을 일부러 골랐다.**

### ② 바닥 색 — 층 구분을 타일에서 빼내 타일맵으로 옮겼다

`C37` 은 타일 10종을 **얕은색 → 깊은색 램프**로 칠하고 층마다 가중치를 뒤집었다.
그 구조의 대가가 사용자가 본 것이다 — **층0 에서도 깊은 타일이 7 % 는 깔리는데
그게 가장 먼 색(차가운 청회색)이라 올리브 바닥에 남색 구멍처럼 박힌다.**
`C37` 도 알고 있었다(GAMMA 1.6 으로 36 % 줄였다). **램프가 있는 한 없앨 수 없다.**

🔑 **두 목표가 서로를 깎고 있었다.** 층 안의 균일함과 층 사이의 차이는
*같은 손잡이*로 조절되고 있었다. 손잡이를 둘로 갈랐다:

| 무엇 | 어디 |
|---|---|
| 층 **안**의 균일함 | 타일 10종을 **전부 같은 한 점**으로 (`retint_tiles.py`) ⇒ 색 편차 **원리적으로 0** |
| 층 **사이**의 차이 | `Tilemap.color` 하나 (`GroundTiler.ApplyDepthTint`) ⇒ 모든 타일에 **같이** 곱해진다 |

⚠️ 정점 색은 곱셈이라 **1 을 넘겨 밝힐 수 없다.** 그래서 공통 목표는
**채널별 원본 최솟값 이하**여야 한다 — 실측 `(0.1560, 0.1586, 0.1130)` 안에 드는
`(0.150, 0.158, 0.108)` 을 골랐다. 밝기 L 0.150 으로 `C37` 의 층0(0.1542)과 사실상 같다.

🔴 **`DeepTint` 를 한 번 되돌렸다.** 처음에 (0.45, 0.50, 1.00) 을 넣었더니 층9 가
**L 0.054 로 예전의 절반**이 됐다. 요구는 *"튀는 색을 없애라"* 지 *"후반을 어둡게 하라"* 가 아니다.
(0.62, 0.68, 1.00) 으로 낮춰 예전 층9 밝기 근처에 맞췄다 — **아무것도 뒷걸음치지 않는다.**

### ③ 기울기 — `D65` 는 부호만 고쳤고 그걸로는 부족했다

사용자가 **두 번** 지적했다. `D65` 는 `flipX` 보정을 지워 **부호**를 맞췄는데,
전단이 **피벗(= 스프라이트 한가운데) 기준**이라는 것이 남아 있었다:

```
pos.x += pos.y * _LeanAmt;      // pos.y 는 피벗 기준 → 위가 오른쪽이면 아래는 왼쪽
```

**화면에서 가장 크게 움직이는 건 다리다.** 부호가 맞아도 다리가 반대로 밀리면
**몸이 가는 쪽의 반대로 가 보인다.** 셰이더 주석은 이미 알고 있었다 —
*"피벗이 Center 면 발도 반대로 밀린다 … 그 정도 반동은 오히려 자연스럽다"*.
**자연스럽지 않았다.**

⇒ `_LeanPivotY` 를 신설하고 `pos.x += (pos.y - _LeanPivotY) * _LeanAmt` 로 바꿨다.
코드가 매 프레임 `sprite.bounds.min.y`(밑변)를 넣는다 — 프레임마다 스프라이트가 바뀌므로.

### 판정

```
[기울기] 셰이더만 격리해 전용 카메라로 찍고 머리띠·발띠의 x 중심을 쟀다
         기준선 0 (예전)   lean +0.40 -> 머리 +38.5 / 발 -37.5    ← 정확히 반대
                           lean -0.40 -> 머리 -40.0 / 발 +37.5
         기준선 밑변 (D79) lean +0.40 -> 머리 +88.5 / 발 +12.5    ← 같은 방향
                           lean -0.40 -> 머리 -90.0 / 발 -12.5      발은 거의 고정
[바닥]   칸 평균밝기 표준편차 0.0138(C37 실측) -> 0.0075   · 층0 L 0.1411 / 층9 L 0.0820 (dL 0.059 > 통과선 0.04)
[타일]   재임포트 확인 0.250u · Grid.cellSize (0.25, 0.25)
[끊김]   음수 사분면 (-137.3, -91.7)  검은 픽셀 0.00 % · 네 모서리 전부 바닥색
         최악 위치 (21.99, 21.99)     검은 픽셀 0.005 % · 테두리 2/4096
                                      ← 칸 index 를 8로 나눈 나머지 (7,7) = 기준점에서 최대로 벗어난 자리
[적]     Wolf 47 · Slime 48 · Goblin 54 · Zombie 57 · Demon 66 · Ogre 86px (플레이어 72px)
```

🔑 **최악 위치를 계산해서 만들었다.** 양자화는 *"어디서든 되나"* 가 아니라
*"가장 벗어난 자리에서도 되나"* 로 판정해야 한다 — 아무 데나 찍으면 대부분 통과한다.

### ⚠️ 사람이 봐야 하는 것

- **적 크기는 `transform.localScale`** 이라 **히트박스·접촉 피해 범위·분리 반경이 같이 1.5배**다.
  맞기 쉬워지고 서로 밀치는 거리도 커진다 — 로그로는 판정할 수 없다 → `PLAYTEST_2.md` §2-L
- 바닥이 이제 **너무 밋밋할 수** 있다. 튐을 없앤 대가다 → `PLAYTEST_2.md` §2-J

---

## 2-104. ✅ 바닥이 깨져 있었다 — **한 줄이 두 곳에서 어긋났다** (D78, 2026-09-05 99차)

**한 줄:** CONTENT 가 타일을 0.5유닛으로 줄였는데 **DEV 쪽 두 자리가 1.0 을 그대로 쓰고 있었다.**

> CONTENT(`C44` 커밋): *"`C43` 에서 타일 PPU 를 1024 -> 2048(1.0 -> 0.5유닛)로 바꾸고
> 씬 `Grid.Cell Size` 를 0.5 로 같이 바꿔 달라고 넘겼는데 아직 1,1 이다.
> 임포트가 한 번이라도 돌았으면 칸마다 절반이 비어 있다."*

`요청-35` 가 **이틀째 미처리**였다. 화면을 찍어 보니 실제로 깨져 있었다 —
검은 바탕 위에 작은 타일이 **드문드문** 깔린 격자.

### 깨진 곳은 하나가 아니라 둘이었다

| # | 어디 | 무엇이 틀렸나 | 증상 |
|---|---|---|---|
| ① | 씬 `Grid.Cell Size` | 타일은 0.5유닛인데 칸은 **1.0** | 칸마다 **75 % 가 빈다** |
| ② | `GroundTiler.EnsureBuffer` | 버퍼 크기를 **월드 유닛 = 칸 수**로 계산 | 칸이 0.5 가 되자 **화면의 절반만** 덮는다 |

🔑 **②는 ①을 고쳐야만 드러나는 결함이다.** 칸이 1.0 인 동안은 두 값이 우연히 같아서
`CeilToInt(halfW * 2f)` 가 맞는 답을 냈다. ①만 고치고 끝냈으면 **가장자리가 검게 남았을 것**이다.
...
### 🔴 CONTENT 를 지적하려다 내 계산이 틀린 것을 찾았다

*"256px 텍스처에 PPU 2048 이면 0.125유닛이다. PPU 를 512 로 내려라"* 라는 `요청` 을
쓰려던 참이었다. **보내지 않았다** — 근거를 다시 재니 전제가 틀렸다.

| 값 | 실제 |
|---|---|
| 원본 텍스처 | **1024×1024** (내가 256 이라고 봤다) |
| 임포터 PPU | 2048 |
...
Unity 는 `maxTextureSize` 로 텍스처를 줄일 때 `sprite.pixelsPerUnit` 을 같은 비율로 낮춰
**월드 크기를 보존한다.** CONTENT 의 수치는 처음부터 맞았고, 틀린 자리는 **DEV 몫 두 곳**뿐이었다.

🔴 하마터면 **맞는 설정을 고치라고 시킬 뻔했다.** `.meta` 한 줄(`spritePixelsToUnits`)만 읽고
플랫폼 오버라이드 블록을 안 봤다. 병렬 세션에서 남의 파일을 지적할 때는
**그 파일 전체를 읽고 나서** 말해야 한다.

### 판정

```
Grid.cellSize        (0.50, 0.50)                    ← 스프라이트 0.500u 와 일치
sprite               256x256px · PPU 512 · 0.500x0.500u
칸 채움률            cell 1.0 -> 25.0%  |  cell 0.50 -> 100.0%
buffer  OLD          41x24 = 20.5x12.0u              ← 화면 36.8x20.0u 의 절반
        NEW          78x44 = 39.0x22.0u              ← 화면을 덮는다
```

🔑 **그림이 최종 판정이다.** `SubmitRenderRequest` 로 게임 카메라를 찍어
고치기 전(검은 바탕 + 드문드문한 타일)과 고친 뒤(이음새 없이 꽉 참)를 나란히 봤다.
`D56`·`D67`·`D72` 에 이어 **숫자만으로는 못 잡는 결함**이 또 나왔다 —
여기서는 숫자도 잡아 줬지만, *"얼마나 이상한가"* 는 화면에서만 보였다.

---

## 2-103. 📋 세션 요약 — **플레이테스트 한 번이 14개 작업이 됐다** (D63~D76, 2026-09-05)

**한 줄:** 사용자가 `PLAYTEST.md` 를 채운 것에서 시작해 **24건을 전부 닫았다.**
그리고 그 과정에서 **버그 하나를 닫고 하나를 새로 찾았다.**

> 다음에 할 일은 코드가 아니라 **한 판 플레이**다 → [`PLAYTEST_2.md`](PLAYTEST_2.md)

### 무엇을 했나

| 이슈 | 무엇 | 절 |
|---|---|---|
| `D63` | Dev 패널 `Clear Node` | 2-89 |
| `D64` | Dev 패널 `Spawn Boss` · `Wave timer` | 2-90 |
| `D65` | 피드백 1차 — 기울기 버그 · XP 곡선 · 1/2/3 키 · 리롤 색 · 안내 위치 | 2-91 |
| `D66` | 보스 오프스크린 화살표 · 클리어 화면 | 2-92 |
| `D67` | 버프 오라 (무적/공속) | 2-93 |
| `D68` | 진화 특전 3종 · 이속 픽업 | 2-94 |
| `D69` | 상점 가격 스케일 | 2-95 |
| `D70` | 노드 배치 규칙 | 2-96 |
| `D71` | 상점 휴식 · 골드 전환 (**`B12` 닫힘**) | 2-97 |
| `D72` | 맵 화면 정정 3건 (사용자가 잡았다) | 2-98 |
| `D73` | 보스 패턴 2종 | 2-99 |
| `D74` | 체력 재생 (**`B13` 발견**) | 2-100 |
| `D75` | 승급 연출 강화 | 2-101 |
| `D76` | 기습 이벤트 | 2-102 |
...
**① 로그는 *무엇을 했는지* 만 말한다.**
`D62`·`D63`·`D64` 에서 **세 번 연속** 플레이 로그로 사용자의 의도를 추측했다가 틀렸다.
상점 *"구매 0"* 은 설계 문제가 아니라 **스킵**이었고, 보스를 못 본 것도 **사용자가 버튼을 눌러서**였다.
🔑 공통 원인 하나 — **사람이 무엇을 눌렀는지가 로그에 안 남는다.**

**② 숫자가 통과한 것을 그림이 잡는다 — 세 번.**
`D56`(도감 실루엣) · `D67`(오라 굵기) · `D72`(맵 화면).
앞의 둘은 내가 캡처해 잡았고 **`D72` 는 못 잡아서 사용자가 잡았다.**
🔴 이유가 분명하다 — **`ScreenSpaceOverlay` 캔버스는 렌더 텍스처로 못 찍는다.**
월드는 `SubmitRenderRequest` 로 보는데 **UI 는 내가 볼 수단이 없다.**

**③ *"이미 도착한 적을 쟀다"* 를 또 밟았다.**
`D51` 에서 한 번, `D76` 에서 또 한 번. `D75` 에서는 `SyncTransforms` 를 안 불러
**충격파가 0 으로 나와 기능을 의심할 뻔했다.**
🔑 셋 다 *"어디서 나왔나"* 를 재려는데 *"어디까지 왔나"* 를 재고 있었다.

**④ 필드별 나열은 반드시 사람을 속인다.**
`PlayerStats.RecalculateStats` 의 `Final = new StatBlock { ... }` 이
`Luck`(오래전) 과 `HpRegen`(`D74`)을 **둘 다 빠뜨렸다.**
`D74` 는 **런타임 판정이 아니었으면 0 인 채로 넘어갔다** — CSV·임포터·`PassiveEffect` 를
전부 고치고 Import 까지 마친 뒤였다.

### 🟢 버그 장부

| | |
|---|---|
| **`B12` 닫힘** (`D71`) | 칸이 꽉 차면 골드만 사라졌다. `Purchase` 가 **줄 수 있는지 먼저 묻는다** |
| **`B13` 신규** (`D74`) | `Luck` 이 최종 스탯에 안 들어간다. **아직 안 터진다** · 사용자 결정 대기 |

### 🔑 이번 세션에서 만든 새 애셋 — **2개뿐이다**

`Pickup_Swift.prefab`(기존 프리팹 복제) 와 `Ambush1.asset`(CSV 행).
보스 화살표·승급 연출·버프 오라·일직선 공격은 **전부 있던 부품을 겹치거나 줄 세운 것**이다.

| 만든 것 | 빌려 온 것 |
|---|---|
| 보스 화살표 | `Graphic.OnPopulateMesh` (폰트 `▲` 는 문자표에 없으면 빈칸이다) |
| 버프 오라 | `SpriteOutline` 셰이더의 외곽선 (엘리트/보스 표시용) |
| 예고 일직선 | `BossSlam` 을 **줄 세웠다** (`D37` 지뢰밭과 같은 부품) |
| 승급 연출 | 히트스톱 + 흔들림 + 넉백 + `PulseFx` 를 **같은 순간에** |

---

## 2-102. ✅ 기습 이벤트 — **"기습만" 이라는 말이 코드를 하나 더 만들게 했다** (D76, 2026-09-05 98차)

**한 줄:** 사용자 요구 `9`. 그리고 **플레이테스트 피드백 24건이 전부 닫혔다.**

> 사용자: *"기습 이벤트 (ambush)는 전투 시간은 짧되 시작시 전맵 테두리에서 몹들 빽빽히 스폰되게 (원거리 Demon 빼고)"*

### 🔴 기습 이벤트는 지금까지 **그냥 노말 웨이브**였다

`SceneWiring.csv` 에 `WaveManager,eventWaves` 행이 **아예 없었다.**
`StartWave` 의 폴백(`eventWaves` 가 비면 노말)이 조용히 받아 주고 있었다 — `D37` 이 그 폴백을 넣어 둔 덕에
*"이벤트 전투가 안 돈다"* 는 안 됐지만, **이벤트 전투라는 게 사실상 없었다.**

### 만든 것 — `Ambush1`

```
Ambush1  타이머 35s (노말 60~90 의 절반 이하) · MaxAlive 110 · 소환 반경 20
         Slime*30@0.18:0-6~Ring | Goblin*30@0.18:0-6~Ring
         Zombie*24@0.25:0-7~Ring | Wolf*16@0.35:1-8~Ring       = 100마리를 8초에
```

🔑 **`~Ring` 이 요구를 그대로 만족한다** — `D50` 의 *"원으로 조여오기"* 는
소환 반경 위에 한꺼번에 놓는다. **그게 곧 "전맵 테두리에서" 다.** 새 패턴이 필요 없었다.

🔴 **Demon 을 뺐다.** 원거리라 테두리에 서서 쏘기만 하면 *"빽빽히 몰려온다"* 가 안 된다.

### 🔴 "기습만" 이라는 말이 `WaveId` 를 만들게 했다

`eventWaves` 에 넣고 무작위로 뽑으면 **지뢰밭에도 100마리가 쏟아진다.**
지뢰밭은 *"설 자리가 주는"* 웨이브라 성격이 정반대다.

⇒ `GameEvent.WaveId` 를 신설하고 `WaveManager.StartWave(node, overrideWave)` 를 열었다.
**`Ambush` 행만** `WaveId=Ambush1` 이고 나머지는 비어 있다.

🔴 **못 찾으면 `null` 이라 예전처럼 노드 종류로 간다** — CSV 에 오타가 나도
전투가 통째로 안 도는 일은 없다. 대신 경고를 찍는다.

### 판정 6/6

```
[데이터]   35s · 킬클리어 False · 반경 20 · MaxAlive 110 · 4항목 100마리 전부 Ring
[Demon]    0마리
[WaveId]   Ambush → 'Ambush1' · Minefield → ''          ← 기습만 특정
[FindWave] "Ambush1" 찾음 · "없는이름" null              ← 대조군
[런타임]   108마리 소환 · Demon 0
[소환 위치] 거리 최소 11.8 · 평균 17.2 · 최대 20.0 (반경 20)
           4분면 분포 18 / 20 / 20 / 18                 ← 사방에 고르다
```

🔑 **4분면 분포가 *"전맵 테두리"* 의 증거다.** 평균 거리만 보면 한쪽에 몰려 있어도 통과한다.

### 🔴 또 "이미 도착한 적을 쟀다"

첫 측정에서 거리가 **0.0 ~ 18.2** 로 나왔다. 25초가 지나 적들이 이미 플레이어에게 붙은 뒤였다.
`timeScale 0.25` 로 낮춰 **소환은 진행되고 이동은 거의 없는** 상태에서 다시 쟀다.

🔑 `D51` 에서 똑같이 밟았던 함정이다 — *"어디서 나왔나"* 를 재려는데 *"어디까지 왔나"* 를 재고 있었다.

---

## 2-101. ✅ 승급 연출 강화 — **네 가지가 같은 순간에** (D75, 2026-09-05 97차)

**한 줄:** 사용자 요구 `B-5`. `D41` 이 파동 하나를 넣었지만 판정이 **"약하다"** 였다.

> 사용자(`B-5`): *"승급 이펙트 좀 더 추가해야 할거 같아"*

### 🔑 새 애셋 0개 — 이미 있던 부품을 겹친다

하나씩은 이미 게임 어딘가에 있던 것들이고, **겹치는 것 자체가**
*"이건 다른 사건이다"* 라는 신호다. 파동 한 번짜리로는 **레벨업 파동과 구분이 안 됐다.**

| 순서 | 무엇 | 어디서 왔나 |
|---|---|---|
| ① | **히트스톱** 0.12s — 시간이 멎는다 | `GameManager.DoHitstop` (엘리트/보스 처치) |
| ② | **충격파** 반경 5.5 · 세기 16 — **피해 0** | `EnemyBase` 넉백 |
| ③ | **화면 흔들림** 0.5 / 0.45s | `CameraController.Shake` (보스 등장) |
| ④ | **파동 3연** 간격 0.13s · 1.25배씩 커진다 | `PulseFx` (`D41`) |

🔴 **`TakeDamage(0, from)` 으로는 안 된다** — 그 안의 `Mathf.Max(1, raw - Armor)` 가
**최소 1 피해를 넣는다.** 승급 연출이 잡몹을 조금씩 죽이면 그건 연출이 아니라 기술이다.
피해 없이 미는 `EnemyBase.PushAway` 를 따로 냈다.

🔴 **`WaitForSecondsRealtime` 을 쓴다.** 히트스톱이 `timeScale` 을 0 으로 만들므로
`WaitForSeconds` 로 하면 **두 번째 파동이 영원히 안 온다.**
승급 직후 레벨업 패널이 뜨는 경우도 많다(무기 만렙 = 레벨업 직후) — 같은 이유다.

### 판정 5/7 — 🟡 **두 줄은 못 쟀다**

```
[승급]     True · 직업 Warden
[히트스톱] timeScale = 0  → 그 뒤 1 로 복구 (state=Wave)
[충격파]   반경 5.5 안 r3: 16.0 (= 세기 16 × 잡몹 저항 1)
           반경 5.5 밖 r9:  0.0   ← 대조군
[파동]     첫 발 즉시 생성 · 크기 1.00
🟡 2·3번째 파동과 1.25배 성장 — 못 봤다
🟡 추가 화면 흔들림 — 못 봤다
```

🔑 **충격파는 안/밖 대조군이 전부다.** `16.0` 하나만 보면 반경이 무한대여도 통과한다.

🟡 **왜 못 쟀나** — `RunCommand` 왕복이 **1초 이상**인데 파동 수명은 **0.55초**다.
세 발이 동시에 살아 있는 구간은 `0.26 ~ 0.55초` 뿐이라 **왕복 한 번보다 짧다.**
흔들림도 같은 이유(카메라 오프셋이 0.45초 안에 사라진다).
⇒ 코드로는 자명하지만 **화면 확인은 사용자 몫**이다.

### 🔴 시험 설계에서 한 번 헛짚었다 — 물리 위치가 안 따라온다

처음 측정에서 **안쪽 적도 속도 0.0** 이 나왔다. 원인은 충격파가 아니라 시험이었다.

**`transform.position` 을 옮긴 직후에는 물리 브로드페이즈가 옛 위치를 쓴다** —
`Physics2D.OverlapCircleAll` 이 아무도 못 잡는다.
`Physics2D.SyncTransforms()` 를 넣고 다시 재니 `16.0 / 0.0` 이 나왔다.

🔑 `D51` 의 *"이미 도착한 적을 쟀다"* 와 같은 종류다 — **시험이 틀렸는데 기능을 의심할 뻔했다.**
...
| **D87** | 사용자가 **다른 게임 스크린샷 2장**을 가져와: *"아직도 박스끼리 겹쳐"* · *"TAB 정보창도 2번째 스크린샷 같은 느낌으로"* | ✅ 해결 (2026-09-06 109차 → 2-114) — 🔴 **원인은 간격이 아니라 테두리였다.** 9-슬라이스 테두리는 `border x (Canvas.referencePixelsPerUnit / sprite.pixelsPerUnit)` 로 그려지는데, `D86` 에서 8x8 텍스처에 **PPU 를 8** 로 줘서 배율이 **12.5** 가 됐다 ⇒ 1px 테두리가 **12.5 캔버스px**, 36px 칩에서 양쪽 25px 를 먹고 **구멍이 11px** — 칸 하나하나가 거의 solid 라 사이가 안 보였다. PPU **100** · 16x16 · 테두리 4px 로 고쳐 **구멍 42px**. 🔴 **`D86` 의 판정은 틀린 것을 쟀다** — *"겹치는 쌍 0"* 은 맞았지만 재야 할 것은 **틈**이었고 그 값이 화면에서 **1.8px** 였다(같은 커밋의 화살표 건과 **같은 종류의 실수**). ② 칸 36→**50** · 간격 4→**10** · 빈 칸에 **분류색 테두리**(무기 파랑 · 패시브 초록 · 건물 주황, 이미 있던 상수 재사용 — **새 상수 0**). ③ TAB 창을 **2단 + `<pos=72%>` 값 정렬 + 금색 구역 제목 + 가운데 구분선**으로, 배치는 `Style()` 로 **코드가 정한다**(B11) | 판정 **7/7**, 전부 **화면 픽셀**로 — 가로틈 1.8→**4.5px** · 세로틈 **6.2px** · 칸 16→**22.3px** · 테두리 구멍 11→**42px** · 빈 칸 `col=(0.45,0.92,0.60,0.55)` · 두 단 285/340 · 316/340 · 라벨 152 < 값 위치 216. 🟡 화면 확인은 사용자 몫 — `/screen-check`(D88)는 아직 실캡처 0회 |
| **D123** | 사용자: *"균열 + 도전 석상으로 가자"* | ✅ 해결 (2026-09-09 144차 → 2-150) — 🔑 **균열과 정반대여서 짝이 된다.** 균열은 *"안 가면 나빠진다"*(강제), 석상은 *"가면 걸 수 있다"*(선택) — 둘 다 한 지점에 발을 묶는데 **이유가 반대**라 같이 놓이면 *"균열부터 닫을까, 지금 눌러도 될까"* 가 생긴다. 🔑 **이 게임에 없던 것은 "내가 고른 위험"** 이다 — 난이도는 늘 층과 시간이 정했고 플레이어는 받기만 했다. `D121` 이 **패시브로** 연 문을 석상은 **자리로** 연다. 🔵 **엘리트를 새로 안 만들었다** — `SpawnMinion(..., isElite:true)` 하나로 체력 ×2.5 · 보라 외곽선 · 사망 시 상자가 통째로 따라온다. 무엇이 깨어나는지는 `ChallengeEnemy()` 가 정하는데 노말 웨이브는 `EliteOverride` 가 비어 있어 **그 층 잡몹의 엘리트판**이 나온다(층마다 상대가 달라진다). 🔴 **안내와 실제가 같은 함수**(`NearestUsable`)를 본다 — 갈라 두면 *"[E] 가 떴는데 눌러도 안 된다"* 가 반드시 생긴다. 🔴 반경 2.8 (`B18`: 제단 2.2 가 설치 링 2.51 보다 좁아 **E 가 안 닿았다**). 🔴 **석상은 적이 아니라 `ClearWave` 가 안 걷는다** — 디렉터가 직접 지운다(균열은 적이라 공짜) | 판정 — ① 개입 없이 **12초** 만에 섬(거리 10.6) ✅ · ② 반경 **2.0 잡힘 vs 3.5 안 잡힘**(대조군) ✅ · ③ Wolf 엘리트 `HpMax` **100 vs 같은 종류 일반 40 = ×2.50**(`EliteHpMult` 와 일치) ✅ · ④ 두 번째 `TryChallenge=False` ✅ · ⑤ 죽이기 전 **+0** → 죽인 뒤 **+42**(현상금 40 + 늑대 드랍 2) + 상자 1 ✅ · ⑥ 웨이브 종료 뒤 석상 **1→0** ✅ · 안내 실제 렌더 확인(`materialCount=1`) · 콘솔 0. 🟡 어두운 바닥에서 눈에 확 띄지는 않는다(룬이 2px) → TUNING |
| **D122** | 사용자: *"균열 + 도전 석상으로 가자"* | ✅ 해결 (2026-09-09 143차 → 2-149) — 🔑 **이 게임에는 지리가 없었다.** `WaveManager.GetSpawnPosition` 이 적을 **100 % 플레이어 중심 링**에 놓는다 — 어디에 서 있든 세상이 똑같아서, 바닥에 점을 찍는 것(오벨리스크·석상)으로는 "저기"가 안 생긴다. **적이 태어나는 자리를 옮겨야** 생긴다. 🎁 그래서 **건물 9종에게 처음으로 지킬 대상이 생긴다** — 터렛·포대·벽·지뢰는 전부 *"한 지점을 붙든다"* 인데 이 게임엔 붙들 지점이 없었다(D110·D117·D120 이 세 번 우회한 빈칸). 🔵 **적으로 태어나게 한 것이 전부다** — 무기 13종이 `GetComponent<EnemyBase>()` 로 때리므로 새 타입이면 **아무 무기도 균열을 못 본다.** 대신 "적"이라 따라오는 둘을 껐다: **재배치**(`IsAnchored`)와 **넉백**(`FixedUpdate` 앵커 복귀). 🔴 방치하면 소환 간격이 줄어든다(3.5→1.0) — 무시할 수 있는 위협은 위협이 아니다 | 판정(대조군 = **같은 자리·같은 순간에 태어난 잡몹**) — ② 8초 이동 **0.00 vs 12.31** ✅ · ④ 5회 피격 후 밀림 **0.00 vs 0.28** ✅ · ③ 뱉는 순간 반경2 안 **2 vs 반대편 0** ✅ · 감쇠 `3.5→3.02`(설계 −0.48) ✅ · ⑤ 닫으니 `1→0` + **상자 1** ✅ · 디렉터가 스스로 하나 열었다 ✅ · 콘솔 0. 🔴 **측정이 두 번 틀렸고 둘 다 진짜 결함이었다** — ① 8초 뒤에 세어 **0 vs 0**(잡몹이 이미 걸어가 버린다) ⇒ 뱉는 프레임에 잰다. ② 🔴 **`base.OnInitialized()` 를 안 불러 `SizeScale 3.2` 가 통째로 무시**돼 균열이 플레이어보다 작았다(0.46 vs 1.00) — 화면을 안 찍었으면 못 봤다 |
| **D121** | 사용자: *"적이 강해지지만 (체력 50%증가) 정산 골드 증가 패시브 하나 추가"* | ✅ 해결 (2026-09-09 142차 → 2-148) — 🔑 **패시브 15종이 전부 이득만 준다.** 무엇을 골라도 손해가 없어 레벨업 3택이 *"무엇을 살까"* 가 아니라 *"지금 뭐가 모자라나"* 였다 — 선택이라기보다 정리다. **현상금은 처음으로 값을 치르고 산다.** 🎁 **보상 쪽은 코드 0줄** — `GameManager.GrantMetaGold` 가 이미 `Final.GoldGain` 을 곱하고 있어 `BonusGoldGain` 열에 값만 넣으면 **런 골드와 정산 골드가 똑같이** 먹는다. 그래서 **대가 쪽만**(`StatBlock.EnemyHpBonus`) 새로 만들었다. 🔴 `AddPendingMetaGold`(상점 전환)에는 일부러 안 곱한다 — 벌 때 이미 먹었으므로 **두 번 먹는다**(D71). 🔴 **적 체력은 태어날 때 한 번만 곱한다** — 소급하면 고른 직후 화면의 적이 갑자기 안 죽어 *"내가 뭘 잘못 눌렀나"* 가 된다. `LayerScaling.HpMult` 바로 옆·같은 규칙이다. 🔵 `PlayerStats.Current` 는 정적이고 웨이브 중에 읽으므로 `Awake` 순서 함정(I-8·I-38)과 무관하다 | 판정(대조군 = **먹기 전에 태어난 같은 적**) — 고블린 **30 → 45** · 좀비 **70 → 105**(정확히 ×1.5) ✅ · **먼저 나와 있던 고블린은 6초 뒤에도 30 그대로**(소급 없음) ✅ · `GrantMetaGold(100)` 이 먹기 전 **`+100`** → 먹은 뒤 **`+150`** ✅ · `GoldGain 1 → 1.5` · `EnemyHpBonus 0 → 0.5` ✅ · 아이콘 알파 `0~255` + 네 귀 `0`(I-41 통과) · 콘솔 0 |
| **D120** | 사용자: *"화톳불 (근처에 있으면 데미지 증가)"* | ✅ 해결 (2026-09-09 141차 → 2-147) — 🔑 **건물 9종이 전부 "설치하면 알아서 일한다"** 였다. 그래서 **플레이어가 그 자리에 있을 이유가 없었다.** `D110` 의 문제를 **세 번째 각도**로 푼다 — 미끼는 적을 **부르고**, 벽은 공간을 **자르고**, 화톳불은 플레이어를 **붙든다.** 🔑 이게 이 게임에 없던 긴장이다 — *"안전하지만 약하게 도망칠까"* vs *"강하지만 서 있을까"*. 서 있으려면 적을 막아야 하니 **미끼·벽·보호막을 쓸 이유**가 생긴다. 🔑 **새 갱신 루프를 안 만들었다** — 쿨다운을 0.25 로 두고 **매 틱 다시 걸어 준다**(냉각탑 방식). 🔴 **"나갔다"를 알아채는 코드가 없다** — 그런 코드는 늘 한쪽을 빠뜨린다. 갱신이 끊기면 끝나는 쪽이 안전하다. 🔴 지속시간은 주기보다 길어야 한다(1.8배) — 같으면 **한 프레임 꺼져 피해가 깜빡인다.** 🔴 초당 4번인 갱신에서 **값이 바뀔 때만** `RecalculateStats` 를 돈다 | 판정(대조군은 **같은 플레이어가 밖에 있을 때** — 자리만 옮겼다) — `Final.Damage` 밖 **1.150**/`IsWarmed=False` vs 안 **1.350**/`True`, 차이 **정확히 0.200**(설계 0.2) ✅ · 나가면 **1.150 복귀** ✅ · 경계 반경 2.6 에서 **2.3 안/2.9 밖** ✅ · 실제 피해 3.2초간 **21.0 → 33.8** ✅ · 🖼️ 고리 48점·따뜻한 색 확인. 🔵 ④의 비 1.61 은 설계 1.20 보다 큰데 **검 쿨다운 1.5초 vs 측정 창 3.2초라 벤 횟수가 2/3 회로 갈린 것** — 배율은 이 값으로 읽으면 안 되고 정확한 값은 ①의 0.200 이다 |
| **D119** | 사용자: *"지뢰"* | ✅ 해결 (2026-09-09 140차 → 2-146) — 🔑 **폭탄은 던지고 지뢰는 두고 간다.** `AoeWeapon` 은 가장 가까운 적을 노려 날리고, 지뢰는 **지금 선 자리**에 놓는다 — 조준이 아니라 **내가 지나온 길**이 무기가 된다. 🔑 그래서 **이 게임의 핵심 동사(도망)와 짝이 맞는다** — 도망친 거리만큼 피해가 된다. 🔵 **폭발을 새로 안 만들었다**(`AoeProjectile` 을 그대로 빌렸다). 🔴 **밟힘을 콜라이더로 안 잡는다**(D117 과 같은 이유 — 적 콜라이더가 `isTrigger` 라 트리거를 달면 플레이어·발사체까지 들어온다) → `OverlapCircle` 로 적 레이어만. 🔴 안전장치 셋: `armDelay 0.45`(0 이면 **즉발 폭탄과 똑같아진다**) · `maxLive 6`(화면이 덮인다) · `OnDisable` 회수(**없는 무기의 지뢰가 남는다**) | 판정(대조군은 폭탄) — ① **지뢰 플레이어까지 0.00** vs **폭탄 6.00** ✅ · ② 태어날 때 무장 **False** · **0.45초** 뒤 무장(설계 0.45) ✅ · ③ 밟으니 사라지고 적 hp **30.0→4.7** ✅ · ④ 20초 관찰 **최대 4개**(상한 6에 안 참) ⇒ 수명이 걷어 간다 ✅. 🔴 **측정이 세 군데 스스로 무너져 다 고쳤다** — ① `subs[0]` 하나만 기준으로 삼았고(폭탄은 아무나 고른다), 가장 가까운 적으로 바꿔도 10.39 가 나왔는데 **폭발 자리의 적이 죽어** 120° 옆(6√3)이 잡힌 것이었다. ② 적을 붙여 놓고 무장 시각을 쟀더니 **"무장→즉폭→재설치"** 가 돌아 표본이 매번 새 지뢰였다(적 없이 재야 한다). ③ 수명을 재려 무기를 뗐더니 `OnDisable` 이 **잴 대상을 전부 걷었다** → **평형 개수**로 바꿔 증명 |
| **D118** | 사용자: *"공전체"* | ✅ 해결 (2026-09-09 139차 → 2-145) — 🔑 **무기 11종이 전부 겨눈다.** 조준 없이 늘 켜져 있는 게 하나도 없어 **등 뒤가 늘 열려 있었다** — 이 무기가 파는 건 피해가 아니라 *"뒤를 안 봐도 된다"* 다. 🔑 **새 열을 하나도 안 만들고 여섯 개를 다시 읽었다**(`ProjectileCount`=구체 수 · `Range`=공전 반지름 · `ProjectileSpeed`=회전 속도(도/초) · `Cooldown`=같은 적 재타격 간격 …). 🎁 그 덕에 **`DoubleProjectiles` 특전이 구체를 2배로** 만든다 — 배선한 게 아니라 이름을 맞춘 결과다. 🔴 **`Update` 를 선언하면 무기가 죽는다**(`WeaponBase` 가 그걸로 쿨다운을 돌린다) → `LateUpdate` 에서 자리를 잡는다. `SummonWeapon` 이 남긴 주석 덕에 처음부터 피했다 | 판정 — 🔑 **사방 8방향**으로 짰다(겨누는 무기와의 차이는 한 방향에서는 안 드러난다): **대조군 수리검 8 중 4** `[....OOOO]` vs **공전체 8 중 8** `[OOOOOOOO]` ✅ · 반지름 **1.41~1.60**(타원이라 정상) · 2초에 **217°**(설계 220°) ✅ · 개수 **5/5 CSV 일치** ✅ · `timeScale=1` 확인 · 콘솔 0 · 🖼️ `RenderTexture` 직접 렌더로 확인. 🟡 구체 지름 1.4 유닛이라 커 보인다(`ProjectileSize` 로 조절 · TODO). 🔵 측정 시 `bounds` 가 1.87 로 나왔는데 **자전 중이라 축정렬 상자가 √2 배로 부푼 것** — 회전체의 `bounds` 를 크기로 읽으면 안 된다 |
| **D117** | 사용자 원 요청의 건물 4종 중 **마지막 하나** — *"적들이 통과 못하고 도트데미지 주는 벽"* | ✅ 해결 (2026-09-09 138차 → 2-144) — 🔴 **물리로는 막을 수가 없었다.** 적 콜라이더가 `isTrigger` 라(접촉 피해를 `OnTriggerStay2D` 로 받는다) **지금 적은 모든 건물을 그냥 통과한다.** 실체로 바꾸면 적이 플레이어·건물·서로와 전부 부딪혀 이동이 통째로 달라진다 ⇒ **이동 코드에서 막는다** — `ArenaBounds` 와 **같은 자리·같은 방식**(early return 위). 🔵 **플레이어는 공짜로 통과한다**(막는 코드가 `EnemyBase` 에만 있다). 막았으면 제 벽에 갇히는 사고가 났다. 🔑 **미끼와 동사가 다르다** — 미끼는 끌어당기고 벽은 막는다. 미끼는 어디든 일하고 **벽은 놓는 자리가 전부**다. 방향은 `D113` 이 만든 `LastMoveDir` 을 재사용해 **가던 쪽을 가로막게** 눕힌다. 🔴 **그림이 막는 범위를 속일 뻔했다** — `sprite.bounds` 는 텍스처 전체지 잉크가 아니다(세로 26 %만 참) ⇒ PNG 를 잉크 상자로 잘랐다(`1024²`→`992x277`). **다시 만들면 다시 자를 것** | 판정(전부 대조군 동반) — ① **벽 없음 1.54 접근 vs 벽 있음 3.07 에서 멈춤** ✅ · ② 미는 적 **3/3 아픔** vs 멀리 둔 대조군 **3/3 변화 0** ✅ · ③ 표면에 붙들어 9.87초 → 내구도 7.46→**-0.02 · 부서짐=True** ✅ · ③ 부서진 뒤 두께방향 **-0.90→+0.88**(부호 반전)·길이방향 -0.78 ⇒ **가로질렀다** ✅ · ④ 플레이어 **안 밀려남** ✅ · 🖼️ `RenderTexture` 직접 렌더로 확인, `bounds 0.76x2.80` = 막는 크기와 동일. 🔴 **측정이 두 번 틀렸다, 둘 다 축을 하나만 봐서** — ① 월드 x 만 보고 "뚫었다"로 읽었는데 **로컬 좌표로 보니 끝을 돌아간 것**(길이방향 -2.35)이었다(=원하는 동작) · ② 내구도가 2.9초에 0.32 만 닳았다 — `PushOut` 이 경계에 정확히 놓아 `Contains` 가 격프레임으로 false 가 됐고(접촉 여유 0.06 추가), **그러고도 0.30** 이라 진짜 이유는 **적이 곧바로 우회**하는 것이었다. 🔴 **봉인 방지 논리를 실측이 바꿨다** — 설계는 "많이 밀수록 빨리 부서진다" 였는데, 실제로는 **우회가 봉인을 막는다**. 길이를 크게 올리면 이 균형이 깨진다 |
| **D116** | 사용자: *"SFX도"* | ✅ 해결 (2026-09-08 137차 → 2-143) — **빌려 쓰던 소리 4종을 전부 걷어냈다** (마이크=칼바람 · 순간이동=빨려드는 소리 · 보호막 충전=먹는 소리 · 막음=UI 소리 — **넷 다 뜻이 어긋났다**). 🔑 **소리를 짓기 전에 자리를 먼저 쟀다** — `compare_sfx.py` 가 길이·어택·대역으로 짝 거리를 내므로 **비어 있는 자리**를 골라 들어갔다. 🔴 **그 자부터 고쳐야 했다** — `Bomb/Buff/GoldPickup` 셋이 `_Incoming/` 경로로 남아 **표에서 사라져 있었다**(그 파일 머리말이 경고한 바로 그 고장이 재발). 🔴 **세 번 부딪히고 세 번 옮겼다**: `MicPulse` ↔ `EnemyHit` **0.79**(대역 거리 0.12) → 200~800Hz 로; `ShieldUp` ↔ `WeaponCast` **0.65** → 내렸더니 `XpPickup` **0.64** → **대역이 아니라 길이**가 빈 자리(0.78s)로; `ShieldBlock` ↔ `GoldPickup` **0.70** → 0.13s + 저역(금화는 0~800 이 0%). 🔴 **같은 실수를 세 번** — 사인 층 이득을 눈대중으로 넣어 저역이 81%/2%/0.3% 로 튀었다. 사인은 결맞음이라 잡음보다 에너지가 수십 배다 ⇒ **층마다 rms 를 재서** `g = sqrt(목표비율)/rms`. 🔴 **마이크와 검은 같은 `MeleeWeapon`** 이라 `Play(WeaponSwing)` 이 박혀 있었다 → `swingSfx` 필드로 **프리팹이 고르게** 했다 | 판정 — 🔴 *"소리가 났나"* 가 아니라 **"그 클립이 났나"** 를 쟀다(`AudioManager` 는 못 찾은 id 를 조용히 무시한다). 울린 직후 **재생 중인 `AudioSource.clip` 이름**을 읽어 **5/5 일치** — 넷 다 ✅ + **대조군 검 `SFX_WeaponSwing` 도 그대로** ✅. 프리팹도 저장 파일에서 재확인 (`Weapon_Mic MicPulse/32` vs `Weapon_Melee WeaponSwing/70`). 최종 자리 — `MicPulse` 0.320s/4.5ms/6-53-30-10(최근접 **1.08**) · `ShieldBlock` 0.130s/4.0ms/57-4-17-20 · `Blink` 0.260s/39ms/4-20-39-34 · `ShieldUp` 0.780s/100ms/0-0-66-23(🟡 `PlayerHit` **0.85** — 기존 `LevelUp`↔`Magnet` 과 같은 수준이라 받아들이고 귀 확인으로 남겼다). 임포트 설정은 `SFX_GoldPickup.wav` 것을 읽어 복사(넷 다 `ch=1` 확인) · 콘솔 0 |
| **D115** | 사용자: *"이펙트 재사용하지 말고 새로 만들어"* | ✅ 해결 (2026-09-08 136차 → 2-142) — 🔑 **검격과 반대로 그렸다** — 검격은 초승달이 **호를 따라 옆으로** 쓸고, 음파는 띠가 **중심에서 앞으로** 밀려 나간다. 프레임이 넘어갈 때 시선이 도는 방향이 달라 **색을 빼도 구분된다.** 🔑 **코드는 한 줄도 안 고쳤다** — `SwingArcFx` 의 약속(6프레임·30fps·PPU 100·피벗 중앙·오른쪽 반쪽·흰색)을 그대로 지키는 시트를 만들면 끝이었다. 🔴 **각도는 `Weapon_Mic` 의 `halfAngle`(32) 과 같아야 한다** — 다르면 **그림이 사거리를 속인다.** 🔴 바깥 반지름을 124 로 잡았더니 띠 두께만큼 더 나가 **마지막 프레임이 칸에 걸렸다** → 118. `r=1.0` 에서 알파를 칼같이 끊었더니 **"잘린 그림"** 으로 보여 바깥 경계도 부드럽게 했다. 🔵 필터만 `Bilinear`(검격은 `Point`) — 픽셀아트가 아니라 **빛**이다 | 판정 — 시트 6칸 전부 **칸 안**(x 최대 245 < 255) · x 최소 131 이라 **중앙보다 오른쪽**만 ✅. 🔴 `activeSelf` 로 판정 안 했다(D104) — 호가 뜬 프레임에 **`timeScale`=0 으로 얼리고** 그 프레임을 찍었다: `sprite=SoundCone_0 · renderer.enabled=True · scale=3.39`(= `Range 4 / 1.18` ✅). 게임 카메라를 `RenderTexture` 에 직접 그려 PNG 로 확인 — 앞으로 퍼지는 보라 음파가 사거리 끝에 닿는다. 대조군 `Fx_SwingArc` 는 `SwingArc_0`·`1.074` 로 **그대로**. 🔴 `Unity_Camera_Capture` 는 **인스턴스 ID 를 넘겨도 씬 뷰를 찍었다** — 직접 렌더로 우회했다 |
| **D114** | 사용자: *"쿠나이 진화 특전 추가해"* | ✅ 해결 (2026-09-08 135차 → 2-141) — 🔑 **쿠나이가 파는 것을 런 전체로 옮겼다** — *"첫 적에서 멈추지 않는다"* ⇒ `ExtraPierce`(모든 발사체가 **적을 하나 더 맞힌다**). 🔑 **값은 하나인데 무기마다 뜻이 다르다** — 직선은 **하나 더 뚫고**, 튕김은 **한 번 더 튕긴다.** 둘 다 *"한 발이 적을 하나 더 맞힌다"* 로 같은 문장이다. 🔴 **한쪽 언어로만 구현하면 어긋난다** — 튕기는 발사체에게 관통 +1 은 쿠나이 자신에게 거의 쓸모가 없다. 🔴 폭발·장판·근접은 `ProjectileBase` 를 안 써서 아무 일도 안 일어난다(뚫을 게 없다). 🔵 **무기가 아니라 발사체가 읽는다** — 특전은 런 도중에 켜지므로 무기 생성 시점에 읽으면 그 무기만 영영 못 받는다. 🔴 enum 은 정수 직렬화라 **뒤에만** 붙였다(`ExtraPierce = 4`) | 판정 — **4/4 예상 일치**. 🔴 **무기로 안 쏘고 발사체를 하나만 직접 쏘았다**(쿠나이는 `ProjectileCount 2` 라 두 발이 섞인다) · 🔴 **피해 1**(크게 주면 적이 죽어 사슬이 끊긴다) · 🔴 **특전은 못 끄므로 대조군 둘을 먼저** 쟀다. ① 직선(일렬 5) **3/5 `[OOO..]` → 4/5 `[OOOO.]`** · ② 튕김(지그재그 6) **4/6 `[OOOO..]` → 5/6 `[OOOOO.]`**. 🔑 배치를 가른 것이 판정의 핵심이다 — 일렬이라야 관통이, 흩어져야 튕김이 뜻을 갖는다. `BonusPierce` 0→**1** · `state=Wave`·`timeScale=1` · 콘솔 0 |
| **D113** | 사용자: *"패시브 추가 — 소환수 증가 · 일정 시간마다 텔레포트 · 보호막(원거리 공격 막아주는 쉴드)"* | ✅ 해결 (2026-09-08 134차 → 2-140) — 🔵 **요구 4종 중 3종만 만들었다** — *"골드 수급량 증가"* 는 `GoldGain` 으로 **이미 있었다**(하나 더 만들면 레벨업 카드에 같은 게 둘 뜬다). 🔑 **이 셋은 기존 패시브와 성격이 다르다** — 앞엣것들은 *이미 있는 수치를 키우고*, 이 셋은 **없던 동작을 켠다.** 새 범주를 만들지 않고 `StatBlock` 에 필드를 더해 **`0` = 기능 없음**으로 읽어 CSV→SO→Final 파이프라인을 통째로 재사용했다. 🔴 대신 함정이 하나 생겼다 — 두 "주기" 필드는 **작을수록 좋은데 `StatBlock.Add` 는 더한다.** 지금은 출처가 하나뿐이라 덮어쓰기처럼 동작하지만 **둘이 되면 합쳐서 오히려 느려진다**(CSV 머리말·필드 주석에 적었다). 🔴 **소환수는 `SummonWeapon` 이 몸통을 하나만 낳고 있었다** — `_body` → `_bodies` 목록으로 바꾸고 **공격도 몸통마다** 돌린다(대표 하나만 쏘면 화면에만 늘고 세기는 그대로라 패시브가 거짓말이 된다). 마릿수는 **매 프레임 다시 읽는다**(패시브는 판 도중에 얻는다). 기차 간격도 마릿수만큼 곱했다(안 하면 겹친다). 🔑 **텔레포트는 방어 기술이다** — 유일한 방어 동사가 *"걸어서 벗어난다"* 인데 둘러싸이면 걸을 곳이 없다. 🔴 바라보는 쪽이 없어서 `PlayerController.LastMoveDir` 을 새로 만들었다(지금 누른 키가 아니라 **마지막으로 움직인 쪽**). 위치는 `Rigidbody2D.position` 으로 옮기고 `ArenaBounds.Clamp` 를 거친다(안 거치면 순간이동만 벽 밖으로 나간다). 🔴 **보호막은 원거리만 막는다** — `TryTakeHit` 에 `ranged` 인자를 더했는데 **기본값 `false` 가 핵심**이라 접촉 호출부는 한 줄도 안 고쳤다(전부 true 로 채우면 무적 픽업과 구분이 사라진다). 고친 건 `EnemyProjectile` **한 줄**. 🔵 막았을 때 **무적을 주지 않는다**(주면 뒤따르는 공격까지 지워 "한 번 막는다"가 아니게 된다) | 판정(전부 대조군 동반) — **소환수 5/5 예상 일치**(대조군 1 → Lv1 2 · Lv3 3 · Lv5 4) · **텔레포트** 대조군 4초 도약 **0회**·최대 이동 **0.000** vs Lv5 12초 도약 **2회**·최대 이동 **5.000**(걷기는 0.016) · **보호막** 원거리 `TryTakeHit=False`·막은 횟수 0→**1**·hp **207.3→207.3** vs **대조군 접촉** `True`·막은 횟수 1→**1**·hp 205.7→202.7 · 막은 직후 `IsInvincible=False` ✅ · 충전 **6.0초**(설계 6초) · `allItems` 31→**34** · `StatBlock.Add` 통과 · 콘솔 0. 🔴 측정이 도중에 멈췄고 `timeScale=0` 이었다 — 패널을 전부 조회하니 **`StageClearPanel`** 이었다. **측정 오염을 막으려 적을 전부 치웠더니 웨이브가 먼저 클리어된 것**이라 버그가 아니었다 |
| **D112** | 사용자: *"무기 추가 — 마이크(원뿔 범위 공격) · 적들 사이를 튕겨다니는 쿠나이(표창 진화)"* | ✅ 해결 (2026-09-08 133차 → 2-139) — 🔑 **마이크는 새 코드가 한 줄도 안 들었다** — `MeleeWeapon.Strike()` 가 이미 부채꼴 판정을 하고 있었고 `halfAngle` 을 70°→32° 로 좁혔다. 좋은 거래인 이유는 따로 있다 — **좁아진 만큼 멀리 보낸다**(Range 2.5→5.6). 🔑 **관통과 튕김은 다른 문제를 푸는 것** — 관통은 일렬로 선 적, 튕김은 흩어진 적. 그래서 수리검의 진화로 자연스럽다. 🔑 **중복 방지를 새로 안 만들었다** — 관통용 `_alreadyHit` 이 그대로 맞는다(없으면 두 적 사이를 무한히 오간다). 🔴 **함정을 설계 단계에서 잡았다** — 부모가 명중마다 관통을 깎아 `pierceCount 1` 이면 **첫 명중에 사라져 한 번도 안 튕긴다**. 값 둘을 사람이 맞추게 하지 않고 `SetPierce(_bouncesLeft+1)` 로 코드가 계산한다. 🔴 `Redirect()` 는 **사거리 기준점도 같이 옮긴다**(안 그러면 두 번째 도약이 거의 못 간다) | 판정 — **마이크 원뿔 5/5 예측 일치**(같은 거리에 각도만 다른 적 5마리 · 0°·20° 맞음 / 50°·100° 안 맞음 / 사거리 밖 안 맞음 — 거리가 대조군) · **쿠나이 한 생애 꺾임 2회·4/4 적중** vs **대조군 수리검 0회·1/4** (일부러 한 직선에 안 놓았다) · `allItems` 30→**31** · `allEvolutions` 3→**4** · 🔴 **`Kunai` 가 `allItems` 에 없음 ✅** · 콘솔 에러 0. 🔴 **첫 측정은 틀렸다** — 대조군 수리검도 "6번 꺾였다"고 나왔고, 풀 재사용의 **다음 생애 위치 점프**를 꺾임으로 센 것이었다. 대조군을 같이 재지 않았으면 꾸며낸 숫자를 적을 뻔했다. 🔴 곁일로 **쿠나이 그림의 날이 45° 위를 향해** 발사체용 `KunaiProj.png`(수평·+X)를 따로 만들었다 (활의 `Bow.png`/`Arrow.png` 와 같은 이유). 🟡 마이크 이펙트는 검격 그림 재탕(보라 틴트) — CONTENT 요청-63 |
| **D111** | 사용자 결정: *"A + ① 로 진행"* (넓은 아레나 + 단단한 벽) | ✅ 해결 (2026-09-08 132차 → 2-138) — 🔑 **카이팅의 모양을 바꾸는 작업이다**: 무한 맵에서 도망은 **직선**이라 다시는 그 자리로 안 돌아오고, 그래서 **못 걷는 건물이 3초면 무의미해진다.** 막으면 도망이 **고리**가 된다. 🔴 **내가 드린 숫자가 틀렸다** — *"반지름 25~30 = 한 바퀴 15~20초"* 는 실제로 **39~47초**다. 뜻이 있는 값은 **"직선으로 몇 초 도망칠 수 있나"** 이고 `halfExtent 24`(48x48)면 **12초**다. 🔑 **막는 자리는 셋뿐**: 플레이어(위치를 막는다 — 속도를 0 으로 하면 벽을 못 타고 미끄러져 답답해진다) · 적(🔴 **Ranged 가 진짜 이유** — `PreferredRange` 때문에 **벽 밖으로 물러나 쏜다**) · 소환(🔑 좌표를 만드는 곳은 넷인데 **놓는 곳은 `SpawnEnemyAt` 하나**다). 🔴 **원이 아니라 사각형** — `CameraController` 가 이미 `Bounds` 로 가두는 코드를 갖고 있어 원이면 모양이 어긋난다. 🔑 카메라 클램프는 **이미 있었고 `useBounds` 만 꺼져 있었다** | 판정 — 플레이어를 중심+(80,-70) 밖으로 던졌는데 **정확히 24.00 에 붙음** · **적 17마리를 35~90유닛 밖으로 던졌는데 밖에 남은 것 0** · 카메라 `bounds (48,48,1)` · 대조군(아레나 없음)에서 `Clamp` 이 좌표를 그대로 돌려줌 · 🖼️ **화면에 붉은 사각형 확인** · 경고 **0**. 🔴 **벽이 안 보였고 원인이 둘이었다**: ① 빌트인 `Sprites/Default` 는 `isSupported=true` 인데 **URP 2D 가 안 그린다** (확실히 보이는 것들의 셰이더를 읽어서 알았다) ② 고친 뒤에도 안 보였는데 **내가 안 보이는 곳을 찍고 있었다** — `orthographicSize` 직접 대입은 `CameraController` 가 되돌린다. **모서리를 화면 좌표로 찍어 보고서야 갈렸다**. 🔵 곁일로 내가 만든 `CS0108` 경고 2건(`Freezer`/`Siren` 의 `Version`)을 `new` 로 지웠다 |
| **D110** | 사용자: *"건물 추가 — 일정 시간마다 젠이 되는 어그로건물 (부셔진 모습 따로 추가)"* | ✅ 해결 (2026-09-08 131차 → 2-137) — 🔑 **이 건물은 이 게임의 구조적 문제를 건드린다**: 플레이어의 유일한 방어 동사가 *"걸어서 벗어난다"* 인데 건물은 **못 걷는 유일한 것**이라 지어 놓고 3초 뒤면 끝이었다. 미끼는 그 관계를 뒤집는다 — **건물이 전장에 갈 필요 없이 전장이 건물에 온다.** 🔑 **새로 만든 것이 거의 없다**: `ToPlayer()` 가 **단일 진입점**이라 여기만 바꾸니 이동 6종이 한꺼번에 따라왔고, 부활 타이머는 `BuildingBase.CooldownActive`(*"false 면 타이머를 얼린다"*)를 **부서져 있는 동안만 true** 로 두어 공짜로 얻었다. 🔴 `AimPointNow`(Blocker 예측 조준)는 **일부러 안 바꿨다** — 앞을 막는 행동은 **움직이는 대상에만** 뜻이 있다. 🔴 부서지면 **어그로를 즉시 푼다**(안 그러면 잔해 앞에 적이 서 있는다). 🔴 미끼 피해는 플레이어와 계산이 다르다 — 무적이 없어 같은 식이면 **물리 프레임마다 한 방**이 된다 ⇒ `fixedDeltaTime` 으로 **초당 피해** | 판정 — 어그로 반경 안 **미끼 1 · 플레이어 0**, **반경 밖 대조군 6마리는 플레이어로** · 파괴 시 `sprite=Decoy_Broken` + 어그로 1→0 · **부활 4/4**(12초 뒤 `IsBroken=False` · HP 만복 · `sprite=Decoy` · 재등록) · 미끼 0개면 `ResolveTarget` 이 플레이어 그대로 · `allItems` 29→**30** · 콘솔 에러 0. 🔴 **수치를 실측으로 다시 잡았다** — 첫 값 HP 120 은 **3.4초** 만에 부서졌다. 적 6마리 접촉 피해 합이 **66/초** 임을 재서 Lv1 을 **530(약 8초)** 으로 올리고, CSV 머리말에 **"초 x 66"** 이라는 되돌리는 법을 적었다 |
| **D109** | 사용자: *"무기 추가 — 기관총 (공격 속도는 빠르지만 명중률은 낮음)"* | ✅ 해결 (2026-09-08 129차 → 2-135) — 🔑 **기존 `ProjectileWeapon` 과 다른 점이 하나뿐이라 그걸 물려받았다** — 저쪽은 발사체를 **정해진 각도로 부채꼴**로 펴고(같은 상황에서 늘 같은 곳), 이쪽은 **발사마다 독립 난수**다. 그게 “산탄”과 “부정확”의 차이다. 🔴 **“빠름”은 코드가 아니라 데이터다** — 연사는 `Cooldown`(0.16→0.10)으로 만든다. 🔵 레벨이 오르면 **정확해진다**(14°→5°) — 데미지만 올리면 “빠른 총”이 그냥 “센 총”이 된다. 🔴 **기존 `Gun` 설명이 이미 “Rapid low damage fire.” 였다** — 연사로는 구분이 안 되어 설명을 *“많이 빗나간다”* 쪽으로 잡았다 | 판정 — 탄 각도 실측 **-11.6° · -10.7° · -6.2°**(표본 3발). 🔑 **부채꼴이면 `count=1` 이라 정확히 0.0° 만 나와야 한다** — “0 이 아니다”가 아니라 **“서로 다르다”**가 근거다. 배선 `allItems` 28→**29** · DPS(명중 100 % 가정) Lv1 18.8 vs Gun 11.1 · Lv5 180 vs 172.7 · 콘솔 에러 **0**. 🔴 **처음엔 `linearVelocity` 로 방향을 재서 전부 0 이었다** — `ProjectileBase` 는 `transform.position +=` 로 밀고 `_direction` 은 private 이다. 단서는 대조군이었다(*“활성 투사체 2 · 속도 있는 것 0”*) ⇒ **발사점에서의 변위**로 바꿈. 🔴 측정이 `LevelUp`(`timeScale 0`)에 계속 오염돼 **재기 직전에 시간을 얼리는** 방식으로 바꿨다(`D108` 에서 세 번 당한 것을 구조적으로 막았다) |
| **D108** | 사용자: *"건물 추가 — 일정시간 마다 적들을 멈춤 / 주변 반경 적들 슬로우"* (4+3+4 요청 중 **싼 것부터**) | ✅ 해결 (2026-09-08 128차 → 2-134) — 🔑 **필요한 시스템이 이미 다 있었다**: `EnemyBase.ApplySlow` 를 독 장판이 쓰고 있었고 *"매 틱 다시 걸어 주면 유지"* 계약이 오라 건물에 그대로 맞았다 ⇒ 두 건물 다 **작은 클래스 하나씩**. 🔴 **설계 중에 버그를 하나 피했다** — 스턴을 `ApplySlow(0f, …)` 로 만들면 **영구 정지**가 된다(`_slowUntil` 이 살아 있는 동안 약한 값으로 못 덮는데 냉각탑이 그걸 계속 민다). ⇒ `ApplyStun` 을 **슬로우와 분리**했고 `_stunUntil` 은 `ApplyStun`·풀 리셋 **두 곳에서만** 쓰인다. 의미가 다르므로 **발사까지 막았다**. 🔵 `Output` 열의 뜻이 건물마다 셋이 되어 CSV 머리말에 규칙을 명시 | 판정 — 냉각탑 **같은 개체 1.600 → 1.200 · 속도비 정확히 0.750** · 사이렌 `v=0.000`(+`Time.time` 이 흐름을 같이 확인) · 해제 뒤 `v=1.600` 복귀 · `allItems` **26→28** · 🖼️ **`/screen-check` 경로 A 를 처음 실제로 썼다**(tier `full` · `scaleFactor` 0.4458). 🔴 **시험이 세 번 오염됐다**(레벨업 · 내가 피험체로 플레이어를 죽임 · 풀 재사용 이름) — `state`·`timeScale`·`Time.time` 을 같이 찍고서야 갈렸다. 🔴 **Import 가 씬을 `isDirty` 로 남겼다**(저장 안 했으면 새 건물이 사라졌다). 🔴 **번호를 `D106` 으로 썼다가 고쳤다** — 병렬 세션이 이미 쓴 번호였다 |
| **D105** | 사용자: *"보스 체력바도 ESC 눌렀을때 맨위에 보이는데 그것도 같이 해결하고"* · 그리고 `요청-42` 에 묻혀 있던 DEV 미처리 2건 | ✅ 해결 (2026-09-06 125차 → 2-131) · 🟢 **CONTENT 열린 항목 0** — ① 형제 순서가 `BossHealthBar`[11] > `PausePanel`[8] > `LevelUpPanel`[7] 이라 체력 바가 **두 화면을 뚫었다.** 🔑 **사용자는 `ESC` 에서 봤지만 레벨업에서도 같은 일이 났다** — 원인이 하나다. 🔑 **순서를 바꾸지 않았다** — `PausePanel` 을 올리면 그 위에 있어야 하는 `OptionSubPanel`[9] 이 뒤로 밀려 옵션 창이 가려진다. `BossOffscreenArrowUI`(D104)와 **같은 방식**으로 상태를 본다. 🔴 `Hide()` 를 쓰면 안 됐다 — 거긴 `_boss` 를 지워 **영구히** 내리므로 일시정지를 풀어도 안 돌아온다. ② **`MAX` 도장이 `Lv.2 / 2` 를 덮었다** — 🔴 **CONTENT 의 C안을 그대로 못 썼다**: 제안 좌표(141~171)가 `DescText`(152~228)와 **19px 겹친다**(틈이 11px 뿐). ⇒ **다 키우면 죽어 있는 `BuyButton` 자리**(264~312)로 내렸다. 🔴 그랬더니 `PriceText` 의 `"MAX"` 와 **1.8px 간격**으로 붙어 한 단어가 겹쳐 찍힌 것처럼 보여, 가격 칸을 **비웠다**(낼 값이 없다). ③ 리롤 로그가 **값이 아니라 이름**이 틀렸다(`비용 누계` = 실은 다음 가격) — `낸 값 · 다음 리롤` **둘 다** 찍게 했다 | 판정 — 체력 바 **3/3**(전투 True → 일시정지 **False**(대조군 `PausePanel` True) → 재개 **True · `materialCount 1`**) · 레벨업도 False · 형제 순서 **[11]/[8] 그대로**. 카드 **겹치는 쌍 0**(전 `LevelText` 62 % · `DescText` 57 % 피격) · 🔑 **대조군(안 키운 카드) 세 요소 좌표가 완전히 동일** — 배치가 안 움직였다. 로그 `낸 값: 25G · 다음 리롤: 40G` → `40G · 55G`, 골드 **500→475→435**. 콘솔 에러 0 · 경고 0. 🔑 **제안 좌표를 그대로 따랐으면 틀렸다 — 대상을 다시 재야 한다** |
| **D104** | 사용자: *"아직도 보스 방향 화살표가 안보여"* (**네 번째** 지적) | ✅ 해결 (2026-09-06 124차 → 2-129) — 🔴 **`BossArrowGraphic` 이 얹힌 오브젝트에 `CanvasRenderer` 가 없었다.** `Graphic` 은 그게 없으면 **한 픽셀도 안 그린다** — 예외도 경고도 없다. `Graphic` 이 `[RequireComponent(typeof(CanvasRenderer))]` 를 달고 있는데도 **파생 클래스를 `AddComponent` 할 때 안 따라왔다.** 🔑 **이래서 세 번을 놓쳤다** — 오브젝트는 생기고 · 켜지고 · **위치와 각도까지 정확했다**(`rotZ 35.6°` vs 보스 방향 35.0°). `activeSelf` 를 보는 검증은 **전부 통과한다.** 앞선 셋(`D83` 조건 · `D85` 조건 확대 · `D86` 크기 44→96)은 증상이 아니라 **내 가설**을 고친 것이었다 ⇒ 속성을 클래스에 직접 달고 `BuildArrow()` 도 생성 시점에 못박았다(**2겹** — 속성만으로는 이번처럼 안 따라올 수 있고, 생성 시점만으로는 다른 데서 `AddComponent` 할 때 또 빈다) | 판정 — `Wave`+보스 **22.6유닛** → `materialCount 1`(**대조군 HUD `Fill` 과 같은 값**) · 화면 사각형 **54x52px** 이 856x498 안 · 대조군 `Wave`+**1.4유닛** → 꺼짐(0) · 고치기 전엔 `CanvasRenderer` 자체가 없었다. 🔴 **대조군을 한 번 버렸다** — 첫 측정에 `state=LevelUp` 이 섞여 **원인이 둘**이었다. 🔑 `CLAUDE.md` §3 에 두 줄 박았다 — **UI 가시성을 `activeSelf` 로 판정 금지**, `materialCount` 를 대조군과 나란히 잰다 |
| **D103** | 사용자 요구 2건: *"Evolution쪽에는 무기만 / class는 Tier1, 2, 3 이런식으로 분류해서 유저가 더 상위 클래스가 어떤것인지 알수 있게"* | ✅ 해결 (2026-09-06 123차 → 2-128) — ① **`EVOLUTION` 은 무기만.** 승급 4종이 같이 들어가 있어 `CLASSES` 탭과 **겹쳐 뜨고** 무엇을 보는 탭인지 흐렸다 ⇒ 3/3(무기)으로 깔끔해졌다. ② **`CLASSES` 를 티어별 줄로.** 사용자가 스크린샷에 `tier 2`·`tier 3` 를 **손으로 적어** 보냈다 — 화면만으로는 상위를 알 수 없었다는 뜻이다. 예전엔 10종이 한 덩어리로 흘러 **티어 2와 3이 같은 줄에 섞였다.** `Entry.Filler`/`Label` 신설 + `GridColumns`·`PadRow` 로 제목이 자기 줄을 쓰게 했다 — `GridLayoutGroup` 은 칸을 순서대로 흘리므로 **줄을 끊는 방법이 이것뿐**이다. 제목·빈 칸은 **진행률에서 빼고 클릭도 막았다**(안 그러면 10/16 이 되고 눌러도 아무 일이 없다) | 판정 — `Evolution 3/3` · `Classes 10/10` 유지 · 격자가 `[TIER 1] / 6종 / [TIER 2] / 3종 / [TIER 3] / Aegis` 로 끊긴다. 🔴 **중괄호를 하나 남겨 파일을 깼다** — `Version` 이 7 에서 안 오르길래 `Editor.log` 를 봤더니 `CS8803` 25건이었다. 🔑 **컴파일 확인이 이걸 잡았다** — 콘솔 에러 **0** 이고 `isCompilationSuccessful` 도 **true** 였다(D27) |
| **D102** / **B19** | 사용자: *"터렛 5 아머 5 레벨인데 안되는데 잘못 적혀있는거야?"* | ✅ 해결 (2026-09-06 122차 → 2-127) · 🟢 **`B19` 닫음** — 🔑 **틀린 게 아니라 모자랐다.** 도감 `CLASSES` 탭이 `Aegis` 조건을 *"Turret Lv.5 + Armor Lv.3"* 로만 적고 **선행 직업 `Sentinel` 을 빼먹었다.** `EVOLUTION` 탭 본문에는 `"Requires class:"` 줄이 있는데 `CLASSES` 탭에만 없었다 — **같은 사실을 두 곳에서 따로 만들다** 한쪽이 빠졌다(`Recipe()` 가 재료 배열만 받아 `FromClass` 를 모른다). ⇒ 조건 줄 앞에 선행 직업을 붙이고, 미발견이면 `???` 로 가려 도감 규칙을 지켰다 | 판정 — `Aegis` 조건이 `Sentinel > Turret Lv.5 + Armor Lv.3` 로 · **대조군: 선행이 없는 나머지 셋은 그대로**. ⚠️ `B18` 과 **원인이 둘 겹쳐 있었다**(반경 + 표시) — 하나만 고쳤으면 사용자는 여전히 못 했다. 사용자가 *"터렛 5 아머 5"* 라고 **자기 상태를 숫자로** 준 덕에 두 번째가 드러났다 |
| **D101** / **B18** | 사용자: *"이 직업이 진화가 안되네"* (도감 9/10 · `Aegis` 만 미발견) | ✅ 해결 (2026-09-06 121차 → 2-126) · 🟢 **`B18` 닫음** — 🔑 **레시피는 멀쩡했다.** `Aegis`(Sentinel 선행 · Turret Lv5 + Armor Lv3 · 제단 Turret)로 **끝까지 승급시켜 봤다**. 🔴 **숫자 둘이 안 맞았다**: 건물은 `placeDistance 1.8` 링 위에 놓이는데 실측 거리가 **1.18~2.51** 로 흩어지고, 제단 반경은 **2.2** 였다 ⇒ **갓 세운 건물이 자리에 따라 `E` 에 닿기도 안 닿기도 한다.** 같은 조작이 될 때도 안 될 때도 있었다. `altarRadius` 를 `Economy.csv` 로 올리고 **2.2 → 3**(링 최대 2.51 을 덮는다) | 판정 — 거리 2.40 에서 `FindAltarClassEvolution = null`(재현) · 0.80 으로 옮기니 `Sentinel > Aegis` 승급 성공 · 반경 3 이 실측 링 거리 전부를 덮는다 · 씬 저장 확인. 🔴 **시험이 한 번 오염됐다** — 앞 판정의 잔여 상태(직업·터렛 3개)가 남아 `False` 와 `Sentinel > Aegis` 가 **동시에** 나왔다. 판을 새로 켜고 다시 쟀다 |
| **D100** | 사용자: *"아직도 warrior 오른쪽으로 이동할때 칼이 왼쪽을 향해"* | ✅ 해결 (2026-09-06 120차 → 2-125) — 🔴 **코드 버그가 아니라 그림이 엇갈렸다.** 걷기 시트가 전부 **정면 그림**이라 "향하는 쪽"이 없고 **무기를 어느 손에 그렸는지**가 곧 기본 방향인데, 그게 직업마다 다르다. Warrior 는 칼이 왼쪽, Ranger 는 활이 오른쪽이라 `flipX = input.x < 0` **한 규칙으로는 둘 다 못 맞춘다.** ⇒ `CharacterClassData.ArtFacesRight` 신설 + `Classes.csv` 열 + 임포터/익스포터 + `PlayerController` 배선. 🔑 **자동 판정을 포기하고 사람 눈으로 표를 만들었다** — 불투명 픽셀 무게중심으로 재 봤더니 Ranger 가 *"중앙"* 으로 나왔다(몸통 질량이지 무기 위치가 아니다). CONTENT 의 꼬리 길이 판정도 Warrior 에서 틀렸다. 사용자 관찰 8종 + 내가 그림을 연 4종이 **4/4 일치**했고, 목록에 없던 `Aegis`(총구 오른쪽)·`Warden`(방패 왼쪽)만 내가 채웠다 | 판정 — 10종 값이 Import 로 애셋에 들어감(**오른쪽 3 / 왼쪽 7**) · 직업을 갈아 끼우면 규칙이 따라온다(Warrior→뒤집는다 / Ranger→그대로) · 씬 저장 확인. 🔵 `ArtFacesRight` = 무기가 **그림 오른쪽**인가 — `Ranger`·`Sentinel`·`Aegis` 만 `1` |
| **D99** | 사용자 잔손 3건: *"아직 흰색 아니고, build 좀 더 아래로"* · *"상점 이제 잘되"* · *"아직도 warrior 칼이 왼쪽"* | ✅ 해결(2/3) (2026-09-06 119차 → 2-124) — ① **레벨업 리롤 글자 흰색.** `ShopUI` 는 이미 `Color.white` 인데 `LevelUpManager` 만 금색(0.96,0.80,0.28)이었다 — 카드 강조색·`LEVEL UP` 제목과 같은 색이라 **버튼인지 장식인지 안 갈렸다.** ② **`[Z] Build` 를 아래로** — 유효 위치 **30 % → 17 %**(336→190px). 🔴 **처음에 앵커만 보고 계산이 틀렸다** — `anchoredPosition y = 90` 이 더해지는 걸 놓쳤다 ⇒ 오프셋을 **0 으로 만들고 앵커 하나로만** 자리를 정했다(둘이 섞이면 또 틀린다). ③ 🟡 **칼 방향은 못 고쳤다 — 코드 문제가 아니다** → `요청-60`, `D100` 에서 데이터로 풀었다 | 판정 — 리롤 `Color.white` · `[Z] Build` 앵커 y 0.17 · 오프셋 (0,0) · 씬 저장 확인 |
| **D98** | 사용자: *"아이템이 꽉차고 레벨업하면 아무것도 안뜨네 / 골드 일정량 or 체력 회복 or 남은 시간 5초 이렇게 추가해"* | ✅ 해결 (2026-09-06 118차 → 2-123) — 🔴 **레벨업을 했는데 아무것도 못 받는 구멍**이었다. `PickItems` 가 0장을 주면 `RefreshPanel` 이 카드를 전부 꺼서 `LEVEL UP` 글자와 리롤만 남았다(리롤해도 뽑을 게 없다). 🔑 **상점이 이미 같은 문제를 풀었다**(`D71`) — 새 개념 없이 그 구조를 따랐다: `LevelUpBonusKind {Gold, Heal, Time}` · `ItemCardUI.SetupBonus`(카드 프리팹 재사용) · `WaveManager.TryCutRemainingTime`. 🔴 **시간 카드는 조건부** — 킬 목표 웨이브엔 타이머가 없어 아예 안 내놓는다(못 쓰는 선택지는 안 주는 것보다 나쁘다). 🔴 치트용 `DevSetRemainingTime` 과 **따로** 뒀다 — 그쪽은 `#if UNITY_EDITOR` 다 | 판정 **7/7** — 꽉 찬 상태에서 **카드 3장**(예전 0장) · 골드 23→83 · HP 89→129 · 시간 35.9→30.9 · 실제 경로 `timeScale 1` 복귀. 🔑 **뜨는 것과 먹히는 것은 다르다** — 셋 다 눌러서 값이 움직이는 걸 봤다. 🔴 시험 중 둘을 잡았다: `B17` 경고는 **내 시험이 상태 전환을 건너뛴 탓**(실제 경로엔 안 뜬다) · **CSV Import 뒤 씬이 저장 안 돼 있었다**(확인 안 했으면 60·40·5 가 사라질 뻔). ⚠️ `Economy.csv` 3행 → 요청-59 |
| **D97** | 사용자: *"꽉찼을때 heal이랑 exchange는 잘보여 근데 글자가 너무 많아 직관적이게 바꿔줘"* | ✅ 해결 (2026-09-06 117차 → 2-122) — 🔑 **긴 건 서비스 칸 둘뿐**이었다(아이템 설명은 `Items.csv` 에서 오고 원래 한 줄). `Exchange` 는 **두 문장이 같은 말을 두 번** 했다 — 뒤 문장은 이미 `+40 meta G` 가 말하고 있어 버렸다. 설명 **107자 → 59자(−45 %)** · 효과값을 `<size=160%>` 로 카드에서 가장 큰 글자로. 🔴 **그 확대가 넘침을 만들었고 커밋 전에 재서 잡았다**(`LevelText` 32 필요 / 26 자리) — `DescText` 가 96 중 43만 쓰기에 남는 자리를 넘겨 **26→38 · 96→84**. 🟢 곁일로 `PLAYTEST_2` 2-E 의 *"소지 제한이 걸리나"* 를 닫았다 — **무기 3/3 · 패시브 5/5 · 건물 5/5** 로 실제로 걸린다 | 판정 — 꽉 찬 상태에서 `Heal · Exchange · Exchange` · **넘치는 글자 0/18**. 🔑 프리팹을 고쳤으므로 **플레이를 껐다 켜고** 다시 쟀다. ⚠️ 사용자가 본 *"꽉 찼는데 살 수 있었다"* 는 **업그레이드**(`Fireball Lv.1→2`)라 정상이다 — 칸을 더 안 먹는다 |
| **D96** | 사용자: *"방금 플레이한 한판으로 playtest2 나머지 채워봐"* — 준 단서는 정산 한 줄뿐이었다(*"메타 골드 +90 · 남은 런 골드 74 소멸"*) | ✅ 해결 (2026-09-06 → `Docs/PLAYTEST_2.md`) — 🟡 **문서만 고쳤다. 코드·애셋 변경 0 이라 별도 `##` 절은 두지 않았다.** 그 한 줄을 `Editor.log` 에서 찾아 **거슬러 올라가 판 전체를 복원**했다(Warrior→Warden · 1스테이지 완주 · 10노드). 🔴 **채운 것은 둘뿐이다** — `2-A`(도달 **Lv 20** · 레벨업 7·7·3·2=19)와 `2-D`(상점 활동 전부 + 정산). **§1 은 안 채웠다** — *"제가 못 보는 것들"* 이라 로그로는 원리적으로 답할 수 없고 사용자가 이미 채워 뒀다. 🔑 **`2-A` 가 통과선(20~28)의 아래 끝에 걸렸는데 원인이 곡선이 아닐 수 있다** — 전투 노드가 10개 중 **4개뿐**이었다(이벤트 4 · 상점 2) ⇒ `xpThresholds` 를 만지기 전에 `weightEvent` 를 먼저 볼 것. 🟢 `D69`·`D71` 이 실측으로 확인됐다 — 남은 골드가 `PLAYTEST.md` 의 **1168 → 74**(16분의 1), 제거 2회로 **+180G** 회수 후 재구매 | 판정 — 로그가 못 답한 것도 같이 적었다(승급까지 몇 분 · 마지막 상점에서 왜 안 샀나 · 기습은 이 판에 안 나옴 · 픽업). 🔵 `직업 승급 — Warden → Warden` 을 확인했더니 **버그가 아니었다** — `EvolutionName`(레시피 이름)을 찍는 것이라 표기 혼동뿐 |
| **D95** / **B17** | 사용자: *"상점을 키고 오래 있으니까 레벨업하면서 게임이 고장났어"* | ✅ 해결 (2026-09-06 116차 → 2-121) — 🔴 **예외가 0 이라 콘솔로는 안 잡혔다.** `Editor.log` 스택으로 원인 셋을 갈랐다: ① `BuildingBase.Update` 가 무조건 돌아 상점에서도 마을이 XP 를 준다 ② `ShopRoot`[2] 가 `LevelUpPanel`[1] 위에 그려져 카드가 숨는다 ③ `OnLevelUpCompleted` 가 **열 때 찍은** 상태를 뒤늦게 되돌려 `Wave` 를 `Shop` 으로 끌어낸다. 🔑 **내 권고가 틀렸고 사용자가 바로잡았다** — 나는 ①을 두자고 했는데(*"막으면 마을이 논다"*) 사용자가 *"전투 노드일 때만 작동해야"* 라고 정정했다. **"지금 이렇게 돼 있다"를 근거로 삼은 것**이 실수의 모양이다. ⇒ 셋 다 고쳤다(원인·표시·복귀). 타이머는 리셋 않고 **멈춘다** | 판정 **5/5** — 🔑 **대조군이 절반**: 전투 아님 + `timeScale 1` 로 20초 → **XP 0**, 전투로 20초 → **XP 2**(과하게 막지 않았다) · 사슬 재현에서 `Wave` 유지(예전엔 `Shop`) · 평범한 레벨업 대조군 정상. ⚠️ `Restaurant`·`Farm` 도 같이 멈춘다(의도) |
| **D94** | 사용자: *"CONTENT 요청 3건 진행해"* — `요청-39`(아이콘 확인) · `요청-40`(스팅어 배선) · `요청-41`(도감 한 줄) | ✅ 해결 (2026-09-06 115차 → 2-120) — 🟢 **CONTENT 열린 항목 0.** ① 도감 `Curve(sb, "HP Regen", p.BonusHpRegen)` — `C49` 진단이 정확했다. **`B13`(Luck)과 같은 원인, 다른 증상**: `B13` 은 합산에서 빠져 **효과가 안 났고**, 이건 표시에서 빠져 **효과는 나는데 안 보인다** ⇒ 🔴 **화면을 봐야만 잡힌다**. 소비처 목록을 주석에 박았다. ② `SfxId.AmbushStart = 34` · Volume 0.90 · Jitter 0. 🔴 **판정 자리를 이름이 아니라 경로로** — 웨이브 이름 비교는 애셋 이름을 바꾸면 조용히 죽으므로 `overrideWave != null`. ③ 아이콘은 **엔진에 물어** 확인 | 판정 **8/9** — 스팅어 `time 0.000s`(그 프레임) · 🔑 **대조군: 같은 노드 노말 웨이브 0건**(이게 판정의 절반이다) · 검증기가 배선 전 `AmbushStart` 를 실제로 잡았다 · 2.40s·모노 · 보스와 다른 클립 · 도감에서 `HP Regen 0.4 -> 2.6` 렌더 확인 · 콘솔 0. 🟡 *"묻히지 않나"* 는 못 닫았다 — 다만 **사용자 `SfxVolume` 이 0.117** 이라 클립을 1.00 으로 올려도 천장이 0.117 이다. 안 들리면 **클립이 아니라 옵션 화면을 먼저 볼 것**. ⚠️ `AudioLibrary.asset`(CONTENT 소유)을 `SerializedObject` 로 건드림 → 요청-58 |
| **D93** | 사용자 요구 3건: *"두박스 하나로 합치고 / 하단에 큰 버튼으로 continue, options, quit, mainmenu / 스탯 글씨, 아이템 크기 키워"* | ✅ 해결 (2026-09-06 114차 → 2-119) — ① `Card` 의 자식을 `Body` 로 옮겨 **1440x1000 한 상자**로 (빈 `Card` 는 지우지 않고 비활성 + 이름표). ② 하단 **325x70 버튼 4개**, 순서는 사용자가 적은 그대로. `Main Menu` 는 `RunEndUI` 와 **같은 경로**(`ReloadScene(false)`)를 쓰고 나가기 전 메타를 저장한다. ③ 스탯 25→**30** · 격자 칸 76x96→**96x104**. 🔴 **칸만 키우면 아이템은 안 커진다** — `B16`(`Icon` 고정 64x64)을 `D91` 이 **HUD 쪽만** 고쳤던 탓에 스탯 창은 그대로였다. `SetCompact(false)` 가 위 74 %/아래 26 % **비율 배치**를 하게 만들고 `StatsPanelUI` 가 부르게 했다 — `RunEndUI` 의 같은 넘침도 같이 고쳐졌다. 조작 단이 488→390 으로 좁아져 `<pos>` 한 줄을 **키/설명 2줄**로(D85 와 같은 이유) | 판정 **9/9** — 활성 상자 **1개** · 화면 안(x[107..749] y[26..472]) · 30pt 342·380/400 · 조작 367/400 · 칩 42.8x46.4px(전 33.9x42.8) · 아이콘이 칸 안 · 버튼 4개 확인 · `Wave→Paused→Wave`. ⚠️ **정정** — 값 위치 72→58 % 를 *"72 % 면 네 자리 체력에서 넘친다"* 로 적었는데 **재 보니 아니었다**(최악 425/460). 진짜 이유는 본문 여백이 **5px → 69px** 로 는 것. 예측을 근거로 적기 전에 그 예측도 재야 한다 |
| **D92** | 사용자 요구 2건: *"아직도 작아 좀더 크기 키워"* · *"TAB 칸은 완벽한데 뒤 랑 겹쳐보이는게 쎄네 / 그냥 ESC 눌렀을때 게임도 멈추고 한번에 보여주는게 나을거 같아"* | ✅ 해결 (2026-09-06 113차 → 2-118) — ① 🔑 **절대 픽셀이 아니라 창 가로 대비로 재야 했다.** 사용자가 게임 뷰를 키워 화면 픽셀은 저절로 커졌는데도 작았다 — 칸 50→**64**(2.6 %→**3.3 %**, 참고 이미지 3.7 %) · 간격 24 · 줄 폭 690. ② 🔴 *"겹쳐 보인다"* 의 원인은 **가림막이 아예 없던 것** — `StatsPanel` 의 자식은 `Body` 하나뿐이라 `DimBG` 가 없고, `UI_Panel` 채움 알파가 **0.92**(D47)라 전투와 HUD 가 글자 위로 비쳤다. `Body` 를 `PausePanel` 로 옮기고 `DimBG` 0.65→**0.92**, `Card` 를 560x720 으로 키워 **조작 안내까지 한 화면**에(참고 이미지 배치). 🟢 **`timeScale` 3단 계약이 통째로 사라졌다** — 저배속을 스스로 걸 필요가 없어져 멈추는 일은 `PauseMenuUI` 한 곳만 한다 | 판정 **8/8** — 손 안 대고 `Body=True` · `Wave→Paused→Wave` · `timeScale 1→0→1` · Body x[134..446] / Card x[473..722] 틈 27px · 글자 316/340 · 316/340 · 189/334. 🔴 **검증이 결함을 잡았다** — `Awake` 의 `SetActive(false)` 를 없애자 씬에 **꺼진 채 저장돼 있던** `Body` 를 아무도 안 켜서 **값은 다 채워졌는데 화면이 비었다**. 로그만 보면 완벽했다(`D86` 의 *"뜬다 ≠ 보인다"* 와 같은 함정) — `activeInHierarchy` 를 같이 쟀기에 잡혔다. 씬을 켜고 `Refresh()` 에 자가복구를 넣었다 |
| **D91** | 사용자 요구: *"TAB에는 두지 말고"* · *"아이템이 제대로 테두리 안에 안들어가 / 프레임으로 만들어서 안에 들어가게해"* | ✅ 해결 (2026-09-06 112차 → 2-117) — 🔴 **원인은 프리팹 치수였다.** `Prefab_ItemChip` 의 `Icon` 은 `sizeDelta 64x64` · `anchor (0.5,1)` · `y -4` 로 **고정**인데, 이는 **TAB 창의 76x96 칸**(아이콘 위·글자 아래)에 맞춘 값이다. HUD 칸은 **50x50** 이라 아이콘이 **칸보다 14px 크고** 위쪽 정렬이라 아래로도 흘러 **테두리를 덮어 그렸다**. 🔑 `D90` 이 테두리를 자식으로 분리한 덕에 비로소 드러난 결함이다(그 전엔 아이콘이 테두리를 아예 대체했다). ⇒ 칸에 맞춰 늘리고 여백 **8**(아이콘 34x34 · 테두리에서 4px) · `preserveAspect`. ② **새 플래그를 안 만들었다** — `SetCompact(bool)` 는 이미 `HUDManager` 만 부르고 있어서 그대로 **"HUD 줄이냐" 스위치**로 승격했다 | 판정 **3/3** — HUD **13/13** 이 사방 여백 3.6px 로 테두리 안쪽 · TAB 경로를 실제로 태워 자식 `Icon`·`Label` **둘뿐** · 임시 오브젝트 제거. 🔴 `ItemGrid 자식 0개` 만 봤으면 **공허한 판정**이었다 — 칩이 없으니 당연히 0이다. 🟢 **`B16` 을 닫는다** — 병렬 세션(`D88`)이 `/screen-check` 로 **같은 결함을 따로 잡아** 두었고 원인·처방이 일치했다. 다만 그쪽 서술이 더 정확하다(*"`D87` 이 만든 게 아니라 드러낸 것` — 칸 36 시절엔 사방 14px 로 더 심했다"*)이고 판정도 **렌더 픽셀 + 빈 칸 대조군**이라 더 낫다. ⇒ **UI 를 고치기 전에 `BUGS.md` 를 먼저 훑는다** |
| **D90** | 사용자 요구: *"아이템을 먹어도 테두리는 있게"* | ✅ 해결 (2026-09-06 111차 → 2-116) — 🔑 **`Image` 한 장이 두 일을 하고 있었다.** `D86` 이 빈 칸 테두리를 `icon` 에 얹은 탓에, `Bind()` 가 `item.Icon` 을 넣는 순간 같은 컴포넌트가 덮여 **테두리가 사라지고 찬 칸만 줄에서 떠 보였다**. ⇒ 겹쳐 그릴 것이 둘이면 `Image` 도 둘이어야 한다. `Frame` 자식 `Image` 를 런타임에 한 번 만들어(**애셋 0 유지**) 칸 전체로 늘리고 `SetAsLastSibling()` 으로 아이콘 위에 얹었다 — 가운데가 뚫려 있어 그림을 안 가린다. 찬 칸 α **0.95** · 빈 칸 α **0.55** 로 갈라 *"몇 칸 남았나"* 를 지켰고, 빈 칸 `icon` 은 α 0 으로 지웠다(🔴 `sprite=null` 만으로는 **흰 쿼드**가 남는다 — D86 에서 겪은 것) | 판정 **4/4** — 활성 13칸 전부 `Frame` 보유 · `Chip_Sword` 가 테두리(0.95)와 `Icon Sword a=1.00` 을 **같이** 그린다 · 빈 칸 `Icon a=0.00` · `Frame` 형제 순서 **2/2**. 🔵 TAB 스탯 창 격자에도 같은 테두리가 생긴다(의도한 부수효과) |
| **D89** | 사용자 스크린샷: *"아직도 딱 붙은것처럼 보이잖아 조금 간격을 띄워"* | ✅ 해결 (2026-09-06 110차 → 2-115) — 🔑 **`D87` 의 4.5px 는 통과선을 넘겼는데 눈에는 안 통했다.** `D88` 의 통과선 20px 은 *"보이는가"* 의 기준이지 *"떨어져 보이는가"* 의 기준이 아니다 — **다른 질문에 같은 자를 댔다.** 칸마다 **1.8px 테두리**가 붙어 있어 두 칸 사이는 `선 1.8 + 틈 4.5 + 선 1.8` = **8.1px 안에 선 두 개**, 안티에일리어싱까지 얹혀 **한 줄의 격자선**으로 뭉쳤다. 사용자가 본 것은 실제로 **표**였다. ⇒ 붙어 보이는지는 **틈이 아니라 테두리를 포함한 덩어리**로 본다. 간격 10→**22** · 줄 간격 64→**76** · 줄 폭 500→**560**(한 줄 최대 8칸이라 `8x50+7x22=554`, 안 늘리면 마지막 칸이 잘린다) | 판정 **3/3** — 세 줄 모두 가로틈 **9.8px**(전 4.5) · 세로틈 **11.6px**(전 6.2) · 칸 22.3px. 🔴 `D86`(뜬다≠보인다) · `D87`(겹침≠틈) 에 이어 **세 번 연속 판정 대상이 틀렸다** — 공통점은 **내 지표가 사용자의 질문과 달랐다**는 것 |
| **D88** | 🔴 **UI 화면을 볼 수단이 없었다.** `ScreenSpaceOverlay` 는 `Camera_Capture` 에 안 잡히고(#120) `ScreenCapture` 는 포커스가 없어 프레임이 안 돌아(#114) 같은 그림만 나왔다 ⇒ 기록 #114 가 *"화면은 사용자 눈에 맡긴다"* 로 끝나 있었다. 그 대가로 **숫자는 통과했는데 화면이 틀린 건이 세 번**(`D56`·`D85`·`D86`) | ✅ 해결 (2026-09-06 108차 → 2-113) — `computer-use` MCP 로 **데스크톱에 합성된 진짜 픽셀**을 찍는 `/screen-check` 스킬 신설. 오버레이는 이미 합성돼 있고 창이 앞에 오므로 프레임도 돈다 ⇒ **두 제약을 동시에 우회한다.** 🔑 **경로를 둘 남겼고 보는 것이 다르다** — `renderMode` 치환(B)은 **렌더 타깃 크기**로 찍혀 게임뷰 실해상도를 재현 못 하므로 `D86` 의 19px 버그를 **못 잡는다.** B 는 권한 거부 시의 차선책. 기록 #114·#120 도 같이 정정했다(옛 결론을 읽고 시도조차 안 하는 걸 막는다). **게임 코드 변경 0** | 검산 — 환산식 `2^(0.5·log2(W/1920)+0.5·log2(H/1080))` 이 과거 기록 **두 건과 자릿수까지 일치**: 856×498 → `0.4532`(`D56` 0.453) · 25pt → **11.33px**(`D56` 11.3) · 44px → **19.9px**(`D86` 19). ⇒ 화면을 보기 전에도 "안 보인다"를 예측할 수 있다. 통과선 **20px**. 🟡 **실캡처는 아직 0회** — 사용자가 이번엔 권한을 거부했다(*"스킬을 만들어라"* 였지 *"지금 찍어라"* 가 아니었다). 🟡 Unity 창 tier 미확인(IDE 로 잡히면 **타이핑이 막힌다** — 캡처는 무관) |
| **D86** | 두 번째 스크린샷 묶음 — 빈 박스가 겹친다 · 흰 박스 말고 테두리만 · **상점 판매가 안 된다** · 화살표가 아직 안 보인다 · HELP 글자/박스 | ✅ 해결 (2026-09-07 107차 → 2-112) — 🔴 **③ 은 진짜 버그였다**: `ConfirmOverlay` 안의 `ConfirmBtn` 에 **아무도 리스너를 안 붙였다**. 게다가 유일한 판매 경로가 *"X 를 한 번 더 누르는 것"* 이라 **취소하려고 X 를 다시 누르면 팔렸다**. 확인 버튼을 잇고 X 는 취소로. 🔴 **④ 는 "뜬다"와 "보인다"가 달랐다** — `D85` 판정도 사용자 판정도 둘 다 맞았다. 캔버스 1920 기준 44px 가 게임뷰(약 850)에서 **19px** 이었다. `arrowSize` **44 → 96**. **내 판정이 `activeSelf` 만 봤지 눈에 띄는 크기인가는 안 봤다.** ① 칩 44 = 줄 간격 44 라 여백 0 → 칩 **36** · 간격 **52**. ② `Image.sprite` 가 null 이면 Unity 가 **흰 쿼드를 그린다** → **8x8 테두리 텍스처 + 9-슬라이스**(애셋 0개). ⑤ **폭을 먼저 넓히고**(340→520) 글자를 키웠다(21→27) — 순서를 바꾸면 줄이 접혀 더 나빠진다 | 판정 — 겹치는 쌍 **0**(칩 13) · 빈 칸 `ItemChipFrame·Sliced` · 판매 **보유 True→False · 골드 4→32** · HELP 668/711 vs 자리 790. 🔑 ③ 은 **배선 여부만 봤으면 통과였다** — 행을 만들어 **버튼을 눌러 봤기 때문에** 잡혔다 |
| **D85** | 사용자가 스크린샷 5장으로 다섯 가지를 짚었다 — 메뉴 버튼이 아래로 많다 · 아이템이 한 줄 · 상점 골드가 카드와 겹친다 · HELP 글씨가 몰렸다 · 보스 화살표가 아직 안 보인다 | ✅ 해결 (2026-09-06 106차 → 2-111) — 🔴 **지적 밖에서 내가 만든 겹침을 찾았다**: `D83` 이 버튼 7개만 45px 올리고 `CurrencyText` 를 안 올려 **`Start Run` 이 골드를 29px 덮고 있었다**. Credits 를 좌하단으로 빼고 기둥을 6개로 재배치(겹침 **0** · 아래 여유 102). 아이템은 **분류별 3줄**(Warrior 3/5/5 전부 상한 일치) · 상점 골드는 카드 아래 빈 자리로. 🔴 **HELP 는 첫 시도가 실패했다** — 줄간격을 키웠더니 **1004 / 자리 708** 로 넘쳤다. 글자를 줄이면 요구(*"가독성 위주"*)와 반대라 **카드의 남는 가로를 써 2단으로**(513 · 545 / 708). 보스 화살표는 **"멀면(7유닛 초과) 화면 안이어도"** 로 조건을 넓혔다 | 판정 — 겹침 0 · 3줄 3/5/5 · HELP 2단 들어감 · 화살표 **9.0 뜬다 / 3.0 안 뜬다**(대조군). 🔴 **화살표를 재는 데 네 번 실패했고 전부 시험 오염이었다**(보스가 다가옴 · 물리로 밀림 · `GameState=LevelUp` · 레벨업 큐). `simulated=false` + `timeScale 0` + `ChangeState(Wave)` 로 셋을 고정하고서야 쟀다 — 한 번이라도 그 자리에서 결론 냈으면 **멀쩡한 코드를 고칠 뻔했다** |
| **D84** | 사용자 플레이 로그의 런타임 예외 49건 (`B14` `EnemyVisual` 매 프레임 · `B15` `ExpDrop` NRE) | ✅ 해결 (2026-09-06 105차 → 2-110) · 🟢 **`B14`·`B15` 닫힘** — 🔑 **원인은 하나였다: 플레이 중 재컴파일(도메인 리로드).** `_sr`(Object 참조)은 복원되는데 `_mpb`(순수 C# 객체)는 null 이 되고 `Awake`/`Setup` 은 다시 안 돈다 — 🔴 **`B14` 등재 때 "설명이 안 된다"고 적은 건 "두 필드가 같이 사라진다"는 잘못된 가정 때문이었다.** 가드를 더 다는 대신 **지연 생성 프로퍼티**로(스스로 낫는다 · `EnemyBase` 가 이미 그 방식이었다) · `PlayerVisual` 도 함께. `B15` 는 **예외가 아니라 순서가 핵심** — `_collected` 가 먼저 서서 반납이 건너뛰어지면 **구슬이 영원히 남는다**. 🟢 재현 중 **세 번째**를 찾았다 — `PlayerStats.Die()` 의 `GameManager.Instance` 무가드 호출(정산·클리어 화면이 통째로 안 뜬다). 🟡 **셋 다 빌드에서는 안 난다**(리로드가 없다) — 개발 중 콘솔을 덮어 진짜 에러를 가리는 문제였다 | 판정 — 리플렉션이 금지돼 `_mpb` 를 못 비우므로 **진짜 원인을 만들었다**: 적 9마리가 살아 있는 상태에서 **플레이 중 재컴파일**을 일으켜 도메인 리로드를 통과 → **`ArgumentNullException` 0건**(전 8초에 47건). 🔑 가짜 조건 대신 실제 조건을 만든 덕에 **세 번째 결함까지 잡혔다** |
| **D83** | `PLAYTEST_2` 답의 나머지 — 승급 파동이 난잡하다 · 기울기가 세다/반대다 · 보스 일직선이 *"그냥 원 여러 개"* · 클리어 화면이 난잡하다 · F7/F8 배율 + 도움말 · `B13` B안 | ✅ 해결 (2026-09-06 104차 → 2-109) — 🔑 **기울기를 세 번 고치고 껐다**(`D65` 부호 → `D79` 기준선 → `D83` off). 답이 *"애니메이션 만들어놨는데 굳이 필요할까"* 였다 — 걷기 시트가 이미 방향을 말하므로 전단은 **겹치는 것**이었다. 🔑 **보스 일직선은 원 7개 → 사각형 1개**(`Texture2D.whiteTexture` · 애셋 0개). **보이는 것만이 아니라 판정도 사각형이 됐다** — 예전엔 원 사이 오목한 자리가 안전지대였다. 🔑 **승급 파동은 수가 아니라 크기로 푼다** — `D75` 가 *"약하다"* 를 3연으로 푼 게 틀렸다. `Count 3→1` · `Growth` 의 뜻을 *"두 번째부터"* → *"첫 파동부터"* (1.25→1.6). 🔑 **F7/F8 은 적을 줄이는 대신 화면을 넓힌다**(4~11 · `PlayerPrefs` 유지 · 키는 `Update` 에서 — `LateUpdate` 는 일시정지 때 안 돈다) + `HelpPanel` 신설(**새 조작을 만들었으면 알 방법도 같이**). 🔴 `B13` 은 **`PlayerStats` 가 아니라 `StatBlock` 을 고쳤다** — `Luck` 한 줄만 더하면 다음 필드에서 또 밟는다 | 판정 — 🔴 **계산이 결함을 둘 잡았다**: ① HELP 버튼을 그냥 붙이면 화면 아래 끝에서 **21px**(기둥이 90 길어졌다) → 전체 45 올려 대칭 ② 클리어 격자가 **한 줄 9칸인데 상한 합 최대 14**(Mage) → 2줄×9=18, 버튼을 -196..-124 → -240..-188. 둘 다 *"최댓값이 몇인가"* 를 먼저 물어서 나왔다 — `D82` 아이템 줄과 같은 종류다 |
| **D82** | `PLAYTEST_2` 를 채우고 추가 요구 4건을 적었다 — 상점 골드가 안 보인다 · `-` 가 남았다 · 상인이 작다 · 리롤 글자 색 · 초상화 박스가 비었다 · 보유 아이템을 아이콘으로 · `[Z] Build` 위치 | ✅ 해결 (2026-09-06 103차 → 2-108) — 🔑 **비어 있던 것들의 이유가 다 달랐다.** 골드는 참조가 **`HUD/CurrencyText`** 라 상점을 열면 HUD 가 꺼져 안 보였고, `-` 는 `WaveProgressText` **하나만 안 꺼져** 있었고, 초상화는 **얼굴 그림 4장이 전부 미할당**이라 매번 `sprite = null` 을 넣고 있었다. 🔑 **⑤⑥ 은 새 애셋 0개** — 초상화는 직업 10종이 이미 가진 `Portrait`, 아이템 줄은 `Prefab_ItemChip` + `SlotLimit`/`CountOwned`. 요구의 핵심이 *"남은 자리"* 라 **상한만큼** 칸을 그린다 ⇒ 🟢 `2-E` 의 *"소지 상한이 도는지 확인해줘"* 에 화면으로 답한다. 🔴 리롤 글자는 **씬을 고쳐도 소용없다**(코드가 매번 덮어쓴다) → `RerollOn` 상수. `[Z] Build` 는 가로 가운데 · **바닥 1/3**(앵커 자체를 1/3 에 뒀다 — 바닥 앵커 + y 계산은 `D81` 이 겹침을 만든 방식이다) | 판정 — 🔴 **계산이 결함을 잡았다**: 칩 76px 그대로면 상한 최대 **14칸(Mage)** 에서 **1116px** 이 필요한데 자리는 460px 이었다. 44px + 아이콘만으로 **655 ≤ 660** · 캔버스 124..784 vs TimerText 810(여유 26). **Warrior(13칸)·Ranger(11칸)로는 안 나타났을 결함이다.** ⚠️ `ScreenSpaceOverlay` 라 캡처가 안 되므로 **화면 확인은 사람 몫** |
| **D81** | 사용자가 작은 창의 직업 선택 화면을 보냈다 — *"화면 줄이면 UI 끼리 겹치는거 해결해"* | ✅ 해결 (2026-09-06 102차 → 2-107) — 🔑 **개별 앵커가 아니라 `ScreenMatchMode` 였다.** `MatchWidthOrHeight 0.5` 는 비율이 바뀌면 **논리 캔버스의 가로를 줄인다**(1920 → 1745 → 1663 → 1610). 화면들이 **왼쪽 고정 + 오른쪽 고정**이라 두 기둥이 서로 다가온다. `Expand` 는 `min(가로,세로)` 배율을 써 **1920x1080 아래로 안 내려간다** ⇒ 비율 때문에 생기는 겹침이 **원리적으로 0**. 곁가지 — `HeaderText` ±700→±620(16:9 에서도 40 겹쳤다) · 안내문 y 66→78(아래 칸이 70까지 차지). 🔴 **프리팹 원본이 `ConstantPixelSize · 800x600` 이었다** — 씬만 오버라이드돼 있어 `B11` 함정이 그대로였다. 같이 맞췄다 | 판정 — UI 는 캡처가 안 되므로 **앵커로 사각형을 다시 풀어** 쟀다. 전 `ClassSelect 95/177/230` → 후 **전 해상도 0**(640x480 · 2560x1080 포함). 🔴 **측정을 한 번 못 믿고 검산했다** — 같은 오브젝트의 앵커가 두 번 다르게 읽혔고, `EvolutionPromptUI` 가 **런타임에 자기 RectTransform 을 코드로 다시 잡는 것**이 원인이었다(그 항목은 씬 값이 정본이 아니다). ⚠️ 사용자 플레이 로그에서 **런타임 예외 49건**을 찾아 등재했다(고치지 않음) → `B14` · `B15` |
| **D80** | CONTENT `요청-38` — `Pickup_Swift.prefab` 의 `m_Sprite` 를 Haste 그림에서 전용 Swift 그림으로. `D78` 때는 `.meta` 가 커밋 안 돼 guid 가 없어 막혀 있었다 | ✅ 해결 (2026-09-05 101차 → 2-106) — **한 줄이다.** `m_Color (0.55,1,0.45)` 는 **그대로 뒀다**(그림이 중립이라 초록으로 칠하면 두 번 곱해져 형광이 된다). 🔑 **CONTENT 가 미리 고친 `.meta` 가 이 작업의 절반이었다** — 기본값 `PPU 100`(20배 크기)·Bilinear·Multiple·압축 그대로였으면 **화면 절반만 한 흐릿한 꺾쇠**가 떴을 것이고 나는 *"배선했다"* 고 보고했을 것이다 | 판정 **5/5** — guid · 크기 **0.650x0.650u 동일** · 모양 **세로 번개 ↔ 가로 꺾쇠(주축 90도)** · 색 보존 · 콘솔 0. 🔴 **첫 캡처가 새까맸다** — 픽업이 `Sprite-Lit-Default`(2D 라이트)라 조명 없는 임시 카메라에선 색이 안 나온다. **대조군(Haste)도 같이 검게 나온 것이 그 증거다.** 🟡 픽업이 자석 반경에 걸려 즉시 회수돼 한 번 헛돌았다(40유닛 밖으로 옮겨 재측정). 🟡 **재다가 찾았다 — 가로 폭만 Swift 가 864px(Haste 352 · 나머지 416~544)**. 세로 480 은 6종 중 5종이 지키는 규격이다. 의도일 수 있어 **고치지 않고** CONTENT 판단으로 넘겼다 → 요청-56 |
| **D79** | 사용자가 스크린샷으로 넷을 짚었다 — *"아직도 너무 픽셀이 커"* · *"몹크기도 좀 키워줘"* · *"특정색이 너무 튀어"* · *"아직도 좌우 이동할때 반대로 몸이 움직이는것처럼 보여"* | ✅ 해결 (2026-09-05 100차 → 2-105) — 🔑 **앞 둘은 한 문제였다**: `Goblin 0.500u == 타일 0.500u` 라 바닥이 배경으로 안 물러나고 **적과 같은 층위의 격자**로 읽혔다. 타일 0.5→**0.25u**(PPU 4096 · cellSize 0.25) + `SizeScale` 전부 **×1.5**. 🔴 타일만 줄이면 `Fill()` 비용이 8배 → 기준점을 **8칸 단위로 양자화**해 다시 채우는 간격을 **2.0유닛으로 고정**(칸 크기와 무관해진다). 🔑 **바닥 색은 손잡이를 둘로 갈랐다** — `C37` 은 층 안의 균일함과 층 사이의 차이를 *같은* 램프로 조절해 서로를 깎고 있었다. 타일 10종을 **한 점**으로 모으고(편차 0) 층 구분은 `Tilemap.color` 로 옮겼다. 🔴 **`D65` 는 부호만 고쳤다** — 전단이 피벗(가운데) 기준이라 **다리가 반대로 밀렸다**. `_LeanPivotY`(밑변) 신설 | 판정 — 기울기 **머리 +38.5/발 −37.5 → 머리 +88.5/발 +12.5**(격리 카메라 실측) · 칸 편차 **0.0138 → 0.0075** · dL **0.059**(통과선 0.04) · 끊김 **음수 사분면 0.00 %** · **최악 위치(나머지 7,7) 테두리 2/4096**. 🔑 최악 위치를 **계산해서 만들었다** — 아무 데나 찍으면 대부분 통과한다. 🔴 `DeepTint` 를 한 번 되돌렸다(층9 가 예전의 절반이 됐다 — 요구는 튐 제거지 후반을 어둡게 하는 게 아니다). ⚠️ 적 스케일이 **히트박스까지** 키운다 → `PLAYTEST_2` §2-L. ⚠️ CONTENT 소유 5종 변경 → 요청-55 |
| **D78** | CONTENT 가 `C44` 커밋에서 짚었다 — *"`Grid.Cell Size` 를 0.5 로 바꿔 달라고 넘겼는데 아직 1,1 이다"* (`요청-35` 이틀째 미처리). 찍어 보니 실제로 **바닥이 드문드문 깔려 있었다** | ✅ 해결 (2026-09-05 99차 → 2-104) — **어긋난 자리가 둘이었다.** ① 씬 `Grid.Cell Size` 1.0 → **0.5** (타일 0.500u 와 일치 · 칸 채움률 25 % → **100 %**) ② `GroundTiler.EnsureBuffer` 가 **월드 유닛을 칸 수로 그대로** 썼다 → 셀 크기로 나눈다. 🔑 **②는 ①을 고쳐야 드러난다** — 셀이 1.0 인 동안은 두 값이 우연히 같았다. ①만 고쳤으면 가장자리가 검게 남았다 | 판정 — 채움률 **25.0 % → 100.0 %** · 버퍼 **20.5x12.0u → 39.0x22.0u**(화면 36.8x20.0u) · `SubmitRenderRequest` 로 전/후 비교(드문드문 → 이음새 없음). 🔴 **CONTENT 의 PPU 를 지적하려다 내 전제가 틀린 것을 찾았다** — 원본은 256px 이 아니라 **1024px** 이고 `maxTextureSize` 256 이 ¼ 로 줄여 `sprite.pixelsPerUnit` 이 2048→**512** 로 따라 내려간다. **임포터 2048 은 처음부터 맞았다.** `.meta` 한 줄만 읽고 플랫폼 오버라이드를 안 본 탓 — 요청은 보내지 않았다 |
| **D76** | 사용자 요구 `9` — 기습 이벤트를 *"짧고 빽빽하게"*. 🔴 확인해 보니 **`eventWaves` 배선이 아예 없어 기습이 그냥 노말 웨이브였다** | ✅ 해결 (2026-09-05 98차 → 2-102) — `Ambush1` 신설(35s · 100마리를 8초에 · 전부 `~Ring` · **Demon 제외**). 🔑 `~Ring`(`D50`)이 *"전맵 테두리에서"* 를 그대로 만족한다 — 새 패턴이 필요 없었다. 🔴 *"기습만"* 이라는 말 때문에 `GameEvent.WaveId` + `StartWave(node, overrideWave)` 를 만들었다 — `eventWaves` 에 넣고 무작위로 뽑으면 **지뢰밭에도 100마리가 쏟아진다**(성격이 정반대다). 못 찾으면 null 이라 예전 동작으로 떨어지고 경고만 찍는다 | 판정 **6/6** — Demon **0** · `Ambush→'Ambush1'` / `Minefield→''` · `FindWave` 대조군 · 소환 거리 평균 **17.2** 최대 **20.0**(반경 20) · **4분면 18/20/20/18**. 🔑 4분면 분포가 *"전맵 테두리"* 의 증거다 — 평균 거리만 보면 한쪽에 몰려도 통과한다. 🔴 **또 "이미 도착한 적을 쟀다"**(`D51` 과 같은 함정) — 첫 측정 0.0~18.2 는 25초 뒤 상태였다. `timeScale 0.25` 로 다시 쟀다 |
| **D75** | 사용자 요구 `B-5` — `D41` 이 승급 파동을 넣었지만 판정이 **"약하다"** 였다 | ✅ 해결 (2026-09-05 97차 → 2-101) — **새 애셋 0개.** 이미 있던 부품 넷을 **같은 순간에** 겹친다 — 히트스톱 0.12s · 충격파(반경 5.5 · 세기 16 · **피해 0**) · 화면 흔들림 · 파동 3연(1.25배씩). 🔴 `TakeDamage(0)` 으로는 안 된다(`Mathf.Max(1, ...)` 가 **최소 1 피해**를 넣는다) → `PushAway` 신설. 🔴 `WaitForSecondsRealtime` — 히트스톱이 `timeScale` 을 0 으로 만들어 **두 번째 파동이 영원히 안 온다** | 판정 **5/7** — 히트스톱 `timeScale=0` → 1 복구 · 충격파 **안 16.0 / 밖 0.0**(대조군) · 첫 파동 크기 1.00. 🟡 **2·3번째 파동과 흔들림은 못 쟀다** — `RunCommand` 왕복(1초+)이 파동 수명(0.55s)보다 길다 ⇒ 화면 확인은 사용자 몫. 🔴 **시험에서 한 번 헛짚었다** — `transform` 을 옮긴 직후엔 물리 브로드페이즈가 옛 위치를 써서 `OverlapCircleAll` 이 아무도 못 잡았다(속도 0.0). `Physics2D.SyncTransforms()` 후 정상. `D51` 의 *"이미 도착한 적을 쟀다"* 와 같은 종류 |
...
114. 🔴 **MCP 로 주입한 키 입력은 게임에 도달하지 않는다** (게임 뷰 포커스 없음).
     ...
     **입력이 필요한 검증은 로직을 직접 호출해서 한다.**
     🟢 **뒷줄(*"화면은 사용자 눈에 맡긴다"*)은 `D88` 이 은퇴시켰다** — `/screen-check` 로
     데스크톱을 직접 찍는다. 다만 **입력 주입이 안 되는 건 그대로다**(화면만 뚫렸다)
...
120. 🔴 **`ScreenSpaceOverlay` 캔버스는 `Unity_Camera_Capture` 에 한 픽셀도 안 잡힌다.** 카메라를
     ...
     🟢 **`D88` 이후로는 이게 차선책이다.** 이 치환은 **렌더 타깃 크기**로 찍히므로
     게임뷰 실해상도를 재현하지 못한다 ⇒ `D86` 의 "19px 라 안 보인다"를 **못 잡는다**.
     1순위는 `/screen-check` 의 데스크톱 캡처, 권한이 거부됐을 때만 이쪽
