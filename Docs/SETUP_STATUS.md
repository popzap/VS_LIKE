...
### 검산 — 환산식이 과거 기록 두 건과 자릿수까지 맞는다

스킬에 "캔버스 px → 실제 화면 px" 환산식을 박아 넣었다
(`ScaleWithScreenSize` · 기준 1920×1080 · `Match 0.5`, 씬 YAML 에서 실측):
...
| 대조 | 식의 결과 | 과거 기록 |
|---|---|---|
| 게임뷰 856×498 | `scaleFactor` **0.4532** | `D56` — 0.453 ✅ |
| 본문 25pt | **11.33px** | `D56` — 11.3px ✅ |
| 화살표 44px | **19.9px** | `D86` — 19px ✅ |

**두 건 다 맞으므로 이 식으로 앞으로는 화면을 보기 전에도 "안 보인다"를 예측할 수 있다.**
통과선은 **20px** 로 정했다 (`D86` 이 19px 에서 실패했고 96px→43.5px 로 고쳐 통과했다).
