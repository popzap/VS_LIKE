...
- **권한 승인 후 실제 캡처를 아직 한 번도 안 했다** (사용자가 이번 요청에서 거부 —
  *"스킬을 만들어라"* 였지 *"지금 찍어라"* 가 아니었다)
- **Unity 창의 tier 를 모른다.** `computer-use` 는 IDE 로 분류된 앱에 **타이핑을 막는다.**
  캡처는 tier 와 무관하지만, 나중에 실조작까지 노린다면 여기서 막힐 수 있다
  → 첫 승인 때 `list_granted_applications` 로 확인해 `SKILL.md` 에 적을 것
