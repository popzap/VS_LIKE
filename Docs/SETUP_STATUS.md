...
>
> **최종 갱신:** 2026-08-30 (25차 — 설치 대기 건물을 HUD 에 표시, D2)
>
...
| **B1** | **엘리트 외곽선이 스프라이트와 어긋난다** — `SpriteOutline.shader` 의 `SampleAlpha` 가 `uv 0~1` 로 아틀라스 밖을 막지만, **시트에서 잘라 온 스프라이트의 `uv` 는 텍스처 전체 기준**이라 검사가 절대 안 걸리고 **옆 칸을 빨아들인다.** I-58 에서 낱장 png → 시트로 바꾼 순간 전제가 깨졌다 | 🔴 **미해결** — 원인 확정(25차). 반경 `14/512 = 28텍셀` vs Wolf 여백 14px · Demon 0px. **로직 버그인데 파일은 CONTENT 소유**라 담당 결정 대기 → `Parallel/BUGS.md` |
