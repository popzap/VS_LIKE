...
🟡 **그리고 또 지적 밖에서 하나 봤다** — 화살표가 **보스 체력 바에 묻힌다**(둘 다 빨강).
`B20` 판정이 **글자만** 보고 `Image` 는 안 봤기 때문이다 → [`BUGS.md` `B21`](Parallel/BUGS.md) (안 고쳤다).
띠를 더 내리면 되지만 같이 잰 왼쪽 아이템 칸 `Frame` 이 **356** 까지 내려와서,
그것까지 피하면 화살표가 **화면 1/3 지점**에 앉아 "테두리 표시"가 아니게 된다.
⇒ 진짜 해법은 **색을 가르는 것**(`요청-61`)에 가깝다.
