...
`Rift` 행은 적이 아니라 **적이 나오는 자리**다. 같은 표에 들어 있지만 열을 다르게 읽는다.

| 열 | 값 | 뜻 |
|---|---|---|
| `MaxHp` | 240 | **닫는 데 드는 화력.** 층 배율·`Bounty` 세금이 그대로 곱해진다 |
| `MoveSpeed` · `ContactDamage` | 0 | 걷지도 때리지도 않는다 (`RiftEnemy` 가 이동을 통째로 덮는다) |
| `SizeScale` | 3.2 | 화면 폭 약 1.5 유닛 — **플레이어(1.0)보다 커야** 자리로 읽힌다 |
| `XpDrop` · `CurrencyDrop` | 30 · 12 | **닫았을 때의 보상.** 상자는 코드가 따로 놓는다(`RiftEnemy.OnDeath`) |
| `AI` | `Chaser` | 🔴 **안 쓴다.** 비워 두지 않은 것은 임포터가 enum 을 읽기 때문이다 |

🔴 **여는 리듬은 CSV 에 없다.** 첫 25초 · 이후 40초 · 최대 2개 · 거리 9~14 는
