...
소환수(Goblin 2.2)도 못 쫓아온다 — 보스 주위에 깔려 **"무기 사거리(3.5~6) 안에 서 있는 것"을 방해**하는 게 역할이고,
...
#### 그래도 남는 문제 — 런 골드는 쓸 곳이 거의 없다

런 골드의 지출처는 **상점·상점 리롤·레벨업 리롤 셋뿐**이다 (건물 설치는 무료).
그런데 상점은 한 판에 **1.2번**, 그때마다 **3칸**만 열린다.

| | 값 |
|---|---:|
| 수입 | ≈ 1,100G |
| 최대 지출 (1.2회 × 3칸 × 평균 75G + 리롤) | ≈ **400G** |

**가격을 10배로 올린 지금도 절반 넘게 남아서 소멸한다.** 이건 가격 문제가 아니라 **배수구 문제**다.
고치려면 다음 중 하나가 필요하고, 셋 다 이 절의 숫자를 다시 계산하게 만든다:

1. `weightShop` 를 올린다 (0.15 → 0.28 이면 방문 2.2회) — `Economy.csv` 에 줄을 옮겨 뒀다
2. `shopSlotCount` 를 3 → 4~5 로 (`ShopUI` 는 슬롯마다 카드를 생성하므로 UI 수정 불필요)
3. 런 골드를 쓰는 소비처를 새로 만든다 (건물 설치 비용 등) — 코드 필요

> 어느 쪽인지는 **플레이해 보고 정한다.** → `TUNING.md`
