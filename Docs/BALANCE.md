...
총 **35종**: 무기 11 + 건물 8 + 패시브 12 + **진화 결과 4** (2026-09-08 · D112 기준).
가격대는 패시브 5~7G, 무기 8~11G, 건물 10~12G.

> 🔴 **이 줄은 오래 낡아 있었다** — `25종 / 무기 5 / 건물 5 / 패시브 10` 으로 적혀 있었는데
> `Items.csv` 를 세어 보니 **35행**이었다(D112 에서 발견). `SceneWiring.csv` 의
> `allItems` **31** = 35 − 진화 결과 4 로 맞는다. 아이템을 늘릴 때 **이 줄도 같이 세어 볼 것.**
