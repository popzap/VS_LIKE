...
> 🔴 **이 두 줄은 `C27`(2026-08-31) 시점 값이다.** 그 뒤로 배수구 양쪽이 다 바뀌었다 —
> `D69` 가 가격을 **층·레벨 스케일**로 만들었고(0층 0.6배 ~ 8층 3.5배),
> `D70` 이 `shopPityLayers 4` 로 바닥을 깔아 방문이 **1.2 → 2.65회**(400판 실측)가 됐다.
> ⇒ **배수구가 대략 3배**다. 🔴 **실측 `M` 은 아직 없다** — `D61` 의 세 판(89/1168/465)은
> Dev 패널 층 스킵이 섞여 오염됐다(`D63`). **`Dev 패널 없이 완주 한 판`이 오면 이 절을 다시 쓴다.**
