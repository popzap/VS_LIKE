...
- 🔴 **착수 전에 [`Docs/Parallel/BUGS.md`](Docs/Parallel/BUGS.md) 를 먼저 훑는다** (B16).
  병렬 세션이 **같은 결함을 이미 진단해 뒀을 수 있다.** 실제로 `D91` 이 HUD 아이콘 넘침을
  처음부터 다시 추적했는데, `D88` 이 원인(`Icon` `sizeDelta 64x64` 고정)과 처방까지
  `B16` 에 적어 둔 뒤였다. **원인 추적을 두 번 한 셈이다.**
- 🔴 **화면에 관한 판정은 "그려진 픽셀"로 한다** — `RectTransform` 좌표는
  *"의도한 사각형이 안에 있나"* 이지 *"보이는 것이 안에 있나"* 가 아니다.
  `D86`(뜬다≠보인다) · `D87`(겹침≠틈) · `D89`(틈≠테두리 포함 덩어리)가 연달아 이것으로 틀렸다.
  ⇒ [`/screen-check`](.claude/skills/screen-check/) 를 쓰고, **대조군을 같이 잰다**
  (빈 칸처럼 *"0이 나와야 하는 것"*. 실제로 안티에일리어싱 오진을 그 대조군이 잡았다).
