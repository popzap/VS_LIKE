...
public class HUDManager : MonoBehaviour
{
    ...
    // 🔴 컴파일 반영 확인용 (D27).
    public const int Version = 12;  // 12 = 색조 흰색 + 뒷판 0.34 (진짜 원인은 Image.color 곱셈이었다) · 11 = 뒷판 0.48 · 10 = 0.20+확대 · 9 = 액자 88x88 · 7 = 칸 64 (D92) · 6 = 간격 22 (D89) · 5 = 칸 키우고 분류색 테두리 (D87) · 4 = 아이템 줄을 분류별 3줄로 (D85) · 3 = 초상화+아이템 줄 (D82) · 2 = 레벨업 파동 (D43)
    ...
    ///
    /// <para>🔴 <b>그런데 진짜 원인은 뒷판이 아니었다.</b> <c>PortraitImage.color</c> 에
    /// <c>(0.25, 0.28, 0.34)</c> 색조가 걸려 있었고 <c>Image.color</c> 는 <b>곱셈</b>이라
    /// (<c>D56</c> 에서 겪은 그것) 밝은 은색 투구가 <b>25 % 로 깎여</b> 어두운 남색이 됐다.
    /// 흰색으로 바꾸니 원 안 휘도가 <b>0.215 → 0.33</b> 로 뛰었다 —
    /// 뒷판을 0.10~0.48 전 구간 굴려도 0.316~0.343 밖에 안 움직인다.
    /// <b>뒷판을 만지기 전에 색조부터 볼 것.</b></para>
    ///
    /// <para>색조를 고친 뒤 다시 골랐다. <b>밝은 초상(Warrior)은 어두운 뒷판에서,
    /// 어두운 초상(Assassin)은 밝은 뒷판에서 산다</b> — 양쪽이 다 사는 지점이 <b>0.34</b> 다.</para>
    /// </summary>
    [Tooltip("초상 뒷판 원의 색. 0.34 는 밝은 초상과 어두운 초상 양쪽을 렌더해서 고른 값.")]
    [SerializeField] private Color portraitBackColor = new(0.34f, 0.35f, 0.316f, 0.95f);
    ...
    /// 한 줄 최대는 Mage 패시브 8칸이다.</para>
    ///
    /// <para>🔴 <b>틈만 벌려서는 안 붙어 보이지 않는다</b> (D89 · 사용자 스크린샷).
